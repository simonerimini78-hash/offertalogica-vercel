export const PDF_AI_CONFIG_VERSION = "step8-clean-budget-v2-preview-active";
...
export function pdfAiConfig(env = process.env) {
  ...
  const requestedMode = String(
    isPreview
      ? env.PDF_AI_PREVIEW_MODE || "fallback"
      : env.PDF_AI_MODE || "off",
  ).trim().toLowerCase();
  return {
    ...
    questionConcurrency: integerSetting(env.PDF_AI_QUESTION_CONCURRENCY, 10, { min: 2, max: 12 }),
    questionTimeoutMs: integerSetting(env.PDF_AI_QUESTION_TIMEOUT_MS, 14_000, { min: 6_000, max: 20_000 }),
    questionUploadTimeoutMs: integerSetting(env.PDF_AI_QUESTION_UPLOAD_TIMEOUT_MS, 12_000, { min: 5_000, max: 20_000 }),
  };
}
