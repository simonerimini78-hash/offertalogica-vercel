export const PDF_AI_FOUNDATION_VERSION = "8.8.8.10";
...
export function pdfAiConfig(env = process.env) {
  ...
  const normalizedMode = requestedMode === "fallback" ? "shadow" : requestedMode;
  const mode = PDF_AI_MODES.includes(requestedMode) ? normalizedMode : "off";
  ...
  return Object.freeze({
    ...
    mode,
    requested_mode: requestedMode || "off",
    legacy_mode_normalized: requestedMode === "fallback",
    enabled: mode !== "off",
    model,
    max_pages: boundedInteger(env.PDF_AI_MAX_PAGES, DEFAULTS.maxPages, 1, 8),
    max_bytes: boundedInteger(env.PDF_AI_MAX_BYTES, DEFAULTS.maxBytes, 1_000_000, 25_000_000),
    timeout_ms: boundedInteger(env.PDF_AI_TIMEOUT_MS, DEFAULTS.timeoutMs, 20_000, 40_000),
