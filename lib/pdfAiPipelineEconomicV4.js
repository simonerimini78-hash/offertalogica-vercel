...
import { runPdfAiFallbackImages } from "./pdfAiRasterBatchedReaderEconomicV4.js";
import {
  canonicalPdfField,
...
export const PDF_AI_FALLBACK_PIPELINE_VERSION = "v106.8.8.7.4-separated-economic-recovery-1";
...
const DEDICATED_ECONOMIC_SOURCE_PATTERN = /economic-recovery/i;
...
const EXCLUDED_ECONOMIC_COMPONENT_PATTERN = /\b(?:prezzo\s+medio|costo\s+medio|spesa\s+media|quota\s+(?:per\s+)?consumi(?:\s+totale)?|totale\s+(?:quota\s+)?consumi|costo\s+unitario\s+totale|rete|oneri|trasporto|distribuzione|contatore|potenza|imposte?|accise|iva|canone|acqua|servizi?)\b/i;
...
function candidateRejection(candidate, aiConflictFields) {
  ...
  if (["prezzo_luce_eur_kwh", "prezzo_gas_eur_smc"].includes(candidate.field)) {
    if (AVERAGE_UNIT_COST_LABEL_PATTERN.test(visualContext) || TOTAL_OR_AVERAGE_CONSUMPTION_PATTERN.test(visualContext)) return "average_unit_cost_not_contract_price";
