...
import { runPdfAiQuestionSession } from "./pdfAiQuestionSession.js";
import { mergePdfAiQuestionSession, PDF_AI_QUESTION_PIPELINE_VERSION } from "./pdfAiQuestionMerge.js";

export const PDF_AI_PIPELINE_VERSION = PDF_AI_QUESTION_PIPELINE_VERSION;
const PDF_AI_FILE_READER_VERSION = "step8-clean-targeted-file-v1";
...
const CONTRACTUAL_SALES_PRICE_PATTERN = /\b(?:prezzo\s+(?:applicato|contrattuale|energia|gas|materia)|costo\s+per\s+consumi|corrispettivo\s+(?:energia|gas|consumo|materia)|componente\s+(?:energia|gas|materia)|materia\s+(?:prima|gas|energia)|prezzo\s+di\s+vendita|vendita\s+di\s+(?:energia\s+elettrica|gas\s+naturale)|condizioni\s+economiche|quota\s+(?:per\s+)?(?:energia|consumi)|formula\s+(?:del\s+)?prezzo)\b/i;
const BILL_SUMMARY_SECTION_PATTERN = /\b(?:scorporo|riepilogo|sintesi|composizione|come\s+si\s+compone|totale\s+(?:bolletta|fattura)|dettaglio\s+(?:degli\s+)?importi|spesa\s+del\s+periodo)\b/i;
const FIXED_SALES_PATTERN = /\b(?:quota\s+fissa|corrispettivo\s+fisso|commercializzazione|vendita\s+fissa|ccv)\b/i;
const EXPLICIT_SALES_CHILD_PATTERN = /\bdi\s+cui\s+(?:spesa\s+)?per\s+la\s+vendita\s+di\s+(?:energia\s+elettrica|gas\s+naturale)\b/i;
...
const INDEX_FORMULA_PATTERN = /\b(?:pun(?:\s+index\s+gme)?|psv(?:\s+day\s+ahead)?|psbil|ttf|indice)\b/i;
const EXPLICIT_TOTAL_PRICE_PATTERN = /\b(?:prezzo|costo|corrispettivo)\s+(?:unitario\s+)?(?:applicato|complessivo|totale|finale|per\s+consumi|energia|gas|materia)\b/i;
const FUTURE_PRICE_PATTERN = /\b(?:dal|dopo)\s+(?:\d{1,2}[°º]?\s+)?(?:mese|anno)|rinnovo|scadenza|successiv[oa]\b/i;
...
function economicCandidateFromRow(row, index) {
  if (economicInventoryReason(row)) return null;
  ...
  const priceBasis = compact(row?.price_basis, 80);
  ...
  if (role === "sales_variable") {
    ...
    const isSpread = priceBasis === "spread";
    const field = isSpread
      ? commodity === "luce" ? "spread_luce_eur_kwh" : "spread_gas_eur_smc"
      : commodity === "luce" ? "prezzo_luce_eur_kwh" : "prezzo_gas_eur_smc";
    return createPdfCandidate({
      id: `ai:economic_inventory:${isSpread ? "spread" : "price"}:${commodity}:${index}`,
      field,
      value_number: rate,
      ...
      commodity,
      page: row?.page,
      label: compact([row?.section_label, row?.row_label].filter(Boolean).join(" — "), 180)
        || (isSpread ? "Spread contrattuale" : "Prezzo unitario complessivo"),
      evidence,
...
function structuredConsumptionField(observation) {
  const commodity = normalizedCommodity(observation?.commodity);
  if (commodity === "luce") return "consumo_luce_kwh";
  if (commodity === "gas") return "consumo_gas_smc";
  return null;
}

function structuredEconomicField(row) {
  const commodity = normalizedCommodity(row?.commodity);
  if (!["luce", "gas"].includes(commodity)) return null;
  const role = compact(row?.component_role, 80);
  const priceBasis = compact(row?.price_basis, 80);
  if (role === "sales_variable") {
    if (priceBasis === "spread") {
      return commodity === "luce" ? "spread_luce_eur_kwh" : "spread_gas_eur_smc";
    }
    return commodity === "luce" ? "prezzo_luce_eur_kwh" : "prezzo_gas_eur_smc";
  }
  if (role === "sales_fixed") {
...
function sameStructuredValue(candidateValue, inventoryValue) {
  const left = Number(candidateValue);
  const right = Number(inventoryValue);
  if (Number.isFinite(left) && Number.isFinite(right)) {
    return Math.abs(left - right) <= 0.00000001;
  }
  return valueKey(candidateValue) === valueKey(inventoryValue);
}

function structuredInventoryBlockReason(candidate, ai = {}) {
  ...
  if (!field || !page) return null;

  for (const observation of ai.consumption_observations || []) {
    if (structuredConsumptionField(observation) !== field) continue;
    if (Number(observation?.page || 0) !== page) continue;
    if (!sameStructuredValue(value, observation?.value_number)) continue;
    const reason = consumptionInventoryReason(observation);
    if (reason) return `structured_consumption_${reason}`;
  }

  for (const row of ai.economic_rows || []) {
    if (structuredEconomicField(row) !== field) continue;
    if (Number(row?.page || 0) !== page) continue;
    if (!sameStructuredValue(value, row?.unit_rate_number)) continue;
    const reason = economicInventoryReason(row);
    if (reason) return `structured_economic_${reason}`;
  }
  return null;
}
...
function economicInventoryReason(row) {
  ...
  const priceBasis = compact(row?.price_basis, 80);
  const validityRole = compact(row?.validity_role, 80);
  if (["future_or_renewal", "expired"].includes(validityRole)) return `informational_${validityRole}`;
  const rate = finitePositive(row?.unit_rate_number);
  const rateUnit = compact(row?.unit_rate_unit, 80);
  ...
  const context = structuredEvidence([row?.section_label, row?.row_label, row?.evidence]);
  if (!context) return "evidence_missing";
  if (BILL_SUMMARY_SECTION_PATTERN.test(section)) return "bill_summary_section_not_contract_terms";
  if (role === "sales_variable") {
    if (!["total_unit_price", "spread"].includes(priceBasis)) {
      return `informational_${priceBasis || "unknown_price_basis"}`;
    }
    const expected = commodity === "luce" ? /(?:€|eur)\s*\/?\s*kwh/i : /(?:€|eur)\s*\/?\s*smc/i;
    if (!expected.test(rateUnit)) return "sales_price_unit_mismatch";
    if (priceBasis === "spread") {
      if (!SPREAD_PATTERN.test(context)) return "spread_label_missing";
      return null;
    }
    if (!CONTRACTUAL_SALES_PRICE_PATTERN.test(context)) return "contract_sales_price_context_missing";
    if (priceBasis === "total_unit_price"
      && (SPREAD_PATTERN.test(context) || INDEX_FORMULA_PATTERN.test(context))
      && !EXPLICIT_TOTAL_PRICE_PATTERN.test(context)) {
      return "indexed_component_not_total_unit_price";
    }
    return null;
  }
  if (priceBasis !== "fixed_charge") return `informational_${priceBasis || "unknown_price_basis"}`;
  const monthly = row?.period_unit === "month" || /(?:€|eur)\s*\/?\s*(?:mese|month)/i.test(rateUnit);
  const annual = row?.period_unit === "year" || /(?:€|eur)\s*\/?\s*(?:anno|year|pod\s*\/\s*anno|pdr\s*\/\s*anno)/i.test(rateUnit);
...
export function structuredInventoryDiagnostics(ai = {}, { promoted = [], rejected = [] } = {}) {
  ...
  const economics = (ai.economic_rows || []).slice(0, 30).map((row, index) => {
    ...
    return {
      ...
      price_basis: compact(row?.price_basis, 60) || "unknown",
      validity_role: compact(row?.validity_role, 60) || "unclear",
      formula_text: compact(row?.formula_text, 180) || null,
      index_reference: compact(row?.index_reference, 80) || null,
      decision,
      candidate_field: candidate?.field || null,
      reason: conversionReason || safetyRejection?.reason || (decision === "accepted_not_promoted" ? "arbitration_or_existing_value" : null),
...
function economicCandidateReason(candidate) {
  ...
  if (field === "prezzo_luce_eur_kwh" || field === "prezzo_gas_eur_smc") {
    ...
    if (BILL_SUMMARY_SECTION_PATTERN.test(evidence)) return "bill_summary_section_not_contract_terms";
    if (NON_SALES_PATTERN.test(evidence)) return "regulated_or_non_sales_component";
    if (FUTURE_PRICE_PATTERN.test(evidence)) return "future_or_renewal_price_not_current";
    if ((SPREAD_PATTERN.test(evidence) || INDEX_FORMULA_PATTERN.test(evidence))
      && !EXPLICIT_TOTAL_PRICE_PATTERN.test(evidence)) {
      return "indexed_component_not_total_unit_price";
    }
    if (!CONTRACTUAL_SALES_PRICE_PATTERN.test(evidence)) return "contract_sales_price_context_missing";
  ...
  if (field.includes("quota_fissa_vendita")) {
    ...
    if (NON_SALES_PATTERN.test(evidence.replace(FIXED_SALES_PATTERN, ""))
      && !EXPLICIT_SALES_CHILD_PATTERN.test(evidence)) {
...
function mergeCompletedPdfAi(base, ai, parserCandidates) {
  ...
  const structuredRejections = [];
  const directCandidates = (ai.candidates || []).filter((candidate) => {
    const field = canonicalPdfField(candidate?.field);
    if (structuredFields.has(field)) return false;
    const reason = structuredInventoryBlockReason(candidate, ai);
    if (!reason) return true;
    structuredRejections.push({
      field,
      reason,
      value: candidate?.normalized_value ?? candidate?.value_number ?? candidate?.value_text,
      page: candidate?.page,
      evidence: compact(candidate?.evidence, 240),
    });
    return false;
  ...
  const filtered = filterSafePdfAiCandidates(aiWithStructuredCandidates);
  const accepted = filtered.accepted;
  const rejected = [...structuredRejections, ...filtered.rejected];
  const modelConflictFields = filtered.modelConflictFields;
...
export function publicPdfAiStatus(audit = {}) {
  ...
  return {
    ...
    batches: (audit?.ai?.batches || []).map((batch) => ({
      ...
      targeted_answer_count: Number(batch.targeted_answer_count || 0),
      targeted_answers: Array.isArray(batch.targeted_answers) ? batch.targeted_answers : [],
      document_commodity: batch.document_commodity || null,
      page_map_count: Number(batch.page_map_count || 0),
      page_map: Array.isArray(batch.page_map) ? batch.page_map : [],
    })),
  };
}

function mergeFilePageMaps(results = []) {
  const seen = new Set();
  return results.flatMap((item) => item?.page_map || []).filter((item) => {
    const key = `${Number(item?.page || 0)}|${compact(item?.role, 100)}|${compact(item?.summary, 220)}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
...
function mergedFileDocument(results = [], normalized = {}) {
  const documents = results.map((item) => item?.document).filter(Boolean);
  const suppliers = unique(documents.map((item) => compact(item?.supplier, 180)).filter(Boolean));
  const documentTypes = unique(
    documents.map((item) => compact(item?.document_type, 80)).filter((item) => item && item !== "unknown"),
  );
  const customerTypes = unique(
    documents.map((item) => compact(item?.customer_type, 80)).filter((item) => item && item !== "unknown"),
  );
  let hasLuce = false;
  let hasGas = false;
  for (const result of results) {
    const documentCommodity = compact(result?.document?.commodity, 80).toLowerCase();
    if (["electricity", "dual"].includes(documentCommodity)) hasLuce = true;
    if (["gas", "dual"].includes(documentCommodity)) hasGas = true;
    for (const candidate of result?.candidates || []) {
      const field = canonicalPdfField(candidate?.field);
      const commodity = normalizedCommodity(candidate?.commodity);
      if (field?.includes("_luce") || ["pod", "potenza_impegnata_kw", "potenza_disponibile_kw"].includes(field)
        || commodity === "luce") hasLuce = true;
      if (field?.includes("_gas") || field === "pdr" || commodity === "gas") hasGas = true;
    }
    for (const item of result?.consumption_observations || []) {
      if (normalizedCommodity(item?.commodity) === "luce") hasLuce = true;
      if (normalizedCommodity(item?.commodity) === "gas") hasGas = true;
    }
    for (const item of result?.economic_rows || []) {
      if (normalizedCommodity(item?.commodity) === "luce") hasLuce = true;
      if (normalizedCommodity(item?.commodity) === "gas") hasGas = true;
    }
  }
  const fallbackCommodity = normalizedCommodity(normalized?.commodity);
  const commodity = hasLuce && hasGas
    ? "dual"
    : hasLuce
      ? "electricity"
      : hasGas
        ? "gas"
        : fallbackCommodity === "luce"
          ? "electricity"
          : fallbackCommodity;
  return {
    document_type: documentTypes.length === 1 ? documentTypes[0] : "unknown",
    supplier: suppliers.length === 1 ? suppliers[0] : null,
    commodity: ["electricity", "gas", "dual"].includes(commodity) ? commodity : "unknown",
    customer_type: customerTypes.length === 1 ? customerTypes[0] : "unknown",
    page_count: Number(normalized?.page_count || 0) || null,
  };
}

async function runTargetedPdfAiFile({
  filePath,
  filename,
  legacyNormalized,
  parserCandidates,
  ...
  const config = pdfAiConfig(env);
  const budget = createPdfAiBudgetPlan({ deadlineAt, raster: false, env });
  if (!budget.sufficient) {
    return {
      ...
      batches: [],
      reader_version: PDF_AI_FILE_READER_VERSION,
    };
  }
  const profiles = ["critical_luce", "critical_gas"];
  const results = await Promise.all(profiles.map(async (profile) => {
    ...
    return {
      ...result,
      batch_id: `file-${profile.replace("critical_", "")}`,
      phase: "critical",
      elapsed_ms: Date.now() - startedAt,
    };
  }));
  const completed = results.filter((item) => item.status === "completed");
  const batches = results.map((item) => ({
    id: item.batch_id,
    phase: item.phase,
    profile: item.profile,
    model: item.model,
    pages: [],
    page_selection: "complete_original_pdf",
    status: item.status,
    reason: item.reason || null,
    timeout_ms: item.timeout_ms || null,
    elapsed_ms: item.elapsed_ms,
    candidate_count: Array.isArray(item.candidates) ? item.candidates.length : 0,
    consumption_observation_count: Array.isArray(item.consumption_observations)
      ? item.consumption_observations.length
      : 0,
    economic_row_count: Array.isArray(item.economic_rows) ? item.economic_rows.length : 0,
    document_commodity: item.document?.commodity || null,
  ...
  return {
    ...
    candidates: completed.flatMap((item) => item.candidates || []),
    economic_rows: completed.flatMap((item) => item.economic_rows || []),
    consumption_observations: completed.flatMap((item) => item.consumption_observations || []),
    conflicts: completed.flatMap((item) => item.conflicts || []),
    review_reasons: unique(completed.flatMap((item) => item.review_reasons || [])),
    document: mergedFileDocument(completed, legacyNormalized),
    page_map: mergeFilePageMaps(completed),
    batches,
    reader_version: PDF_AI_FILE_READER_VERSION,
...
async function runLegacyPdfAiPipeline({
  filePath = "",
  imageFiles = [],
...
export async function runPdfAiPipeline({
  ...
  // New Step 8 architecture: upload the complete PDF once, then ask one
  // independent literal question for each requested field. The previous
  // candidate/inventory/arbitration path is intentionally not used here.
  const session = await runPdfAiQuestionSession({
    filePath,
    imageFiles,
    filename,
    normalized,
    pageCount: Number(normalized?.page_count || imageFiles.length || 0),
    deadlineAt,
    env,
    transport,
    apiKey,
  });
  const merged = mergePdfAiQuestionSession({ normalized, session, env });
  ...
  return {
    ...
    audit: {
      ...merged.audit,
      mode: policy.config.mode,
      reason: merged.audit?.reason || policy.reason,
    },
  };
}
