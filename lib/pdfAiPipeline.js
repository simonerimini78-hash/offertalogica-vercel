...
export const PDF_AI_PIPELINE_VERSION = "step8-clean-single-pipeline-v2-preview-active";
...
function mergeCompletedPdfAi(base, ai, parserCandidates) {
  ...
  const recognizedByClassification = ["bolletta", "scheda_offerta"].includes(fieldValidated.kind)
    && ["luce", "gas", "dual"].includes(fieldValidated.commodity);
  if (recognizedByClassification) {
    fieldValidated.recognized = true;
    if (!["high", "medium"].includes(fieldValidated.confidence)) fieldValidated.confidence = "medium";
  }
