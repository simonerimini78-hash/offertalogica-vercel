...
function makeFieldMeta(input, field) {
  ...
  const decision = autofillDecision(input, field, value, status, reviewSource);
  return {
    field,
    ...
    provenance: {
      ...
      method: aiField ? "openai_visual_semantic" : fieldMethod(input, field, diagnostic),
      confidence: aiField
...
export function buildPdfDataContract(input = {}) {
  ...
  return {
    ...
    autofill_plan: {
      ...
      requires_user_confirmation: true,
      policy: "complete_native_fields_plus_canonical_explicitly_selected_ocr_or_ai_fields",
