export const PDF_DATA_CONTRACT_VERSION = "1.0.0";
export const PDF_AUTOFILL_POLICY_VERSION = "1.0.0";
...
function fieldValue(input, field) {
  if (field === "fornitore_luce") {
    return ["luce", "dual"].includes(input.commodity) ? input.fornitore : null;
  }
  if (field === "fornitore_gas") {
    return ["gas", "dual"].includes(input.commodity) ? input.fornitore : null;
  }
...
function autofillDecision(input, field, value, status) {
  ...
  if (!targets.length) return { allowed: false, reason: "campo_non_mappato", targets: [], use };
  if (!hasValue(value)) return { allowed: false, reason: "valore_assente", targets, use };
  if (status.status !== "completo") return { allowed: false, reason: `stato_${status.status}`, targets, use };
  if (sharedPriceTypeConflict(input, field, value)) return { allowed: false, reason: "target_condiviso_con_valori_in_conflitto", targets, use };
  if (!targetSupportsValue(field, value)) return { allowed: false, reason: "valore_non_supportato_dal_modulo", targets, use };
  return { allowed: true, reason: "campo_completo_con_conferma_utente", targets, use };
}
...
function makeFieldMeta(input, field) {
  ...
  const decision = autofillDecision(input, field, value, status);
  return {
    field,
    ...
    provenance: {
      source: "parser",
      origin: "pdf_native_text",
      source_version: input.parser_version || "unknown",
      method: fieldMethod(input, field, diagnostic),
      confidence: diagnostic?.confidence || input.confidence || "unknown",
    },
    evidence,
    autofill: decision,
  };
...
function blockedFieldRows(fields) {
  return Object.values(fields)
    .filter((entry) => entry.status !== "non_applicabile" && entry.autofill.targets.length && !entry.autofill.allowed)
    .map((entry) => ({
      source_field: entry.field,
...
export function buildPdfDataContract(input = {}) {
  ...
  return {
    ...
    customer: {
      ...
      customer_code: input.codice_cliente || null,
    },
    supplies: {
    ...
    autofill_plan: {
      ...
      requires_user_confirmation: true,
      policy: "only_complete_validated_fields",
