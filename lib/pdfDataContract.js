...
function isAiField(input, field, diagnostic = null) {
  if (input?.ai?.applied) {
    const filled = new Set(Array.isArray(input.ai.filled_fields) ? input.ai.filled_fields : []);
    const aliases = DIAGNOSTIC_ALIASES[field] || [field];
    if (aliases.some((alias) => filled.has(alias))) return true;
  }
  const method = compact(diagnostic?.method, 120).toLowerCase();
  return /^(?:gpt41_visual|openai_visual)/.test(method);
}
...
function autofillDecision(input, field, value, status, sourceReview = null) {
  ...
  if (sourceReview && ["completo", "da_verificare"].includes(status.status)) {
    return {
      allowed: false,
      review_selectable: true,
      requires_explicit_selection: true,
      reason: sourceReview === "ai"
        ? "ia_da_verificare_con_conferma_esplicita"
...
function makeFieldMeta(input, field) {
  ...
  const aiField = isAiField(input, field, diagnostic);
  const sourceReview = aiField ? "ai" : ocrField ? "ocr" : null;
  const decision = autofillDecision(input, field, value, status, sourceReview);
  return {
    field,
    ...
    original_value_kind: diagnostic?.derivation
      ...
      : evidence.literal_value
        ? "source_literal"
        : diagnostic
          ? "parser_diagnostic_value"
          : "parser_output_value",
    ...
    provenance: {
      ...
      origin: aiField ? "pdf_image_visual" : ocrField ? "pdf_image_ocr" : "pdf_native_text",
      source_version: aiField
        ? input.ai?.pipeline_version || diagnostic?.source_version || input.parser_version || "unknown"
        : ocrField
      ...
      confidence: (aiField || ocrField) ? diagnostic?.confidence || "medium" : diagnostic?.confidence || input.confidence || "unknown",
    },
