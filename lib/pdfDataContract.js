export const PDF_DATA_CONTRACT_VERSION = "1.2.0";
export const PDF_AUTOFILL_POLICY_VERSION = "1.2.0";
...
const UNIT_BY_FIELD = Object.freeze({
  ...
  consumo_luce_f1_kwh: "kWh/anno F1",
  consumo_luce_f2_kwh: "kWh/anno F2",
  consumo_luce_f3_kwh: "kWh/anno F3",
  consumo_luce_f23_kwh: "kWh/anno F23",
  prezzo_luce_eur_kwh: "EUR/kWh",
  prezzo_luce_f0_eur_kwh: "EUR/kWh",
  prezzo_luce_f1_eur_kwh: "EUR/kWh",
  prezzo_luce_f2_eur_kwh: "EUR/kWh",
  prezzo_luce_f3_eur_kwh: "EUR/kWh",
  prezzo_luce_f23_eur_kwh: "EUR/kWh",
  ...
  moltiplicatore_indice_luce: "coefficiente",
  ...
  moltiplicatore_indice_gas: "coefficiente",
  consumo_gas_mc: "mc/anno",
  coefficiente_conversione_gas_c: "coefficiente C",
  consumi_mensili_luce_kwh: "kWh/mese",
  potere_calorifico_gas_gj_smc: "GJ/Smc",
});
...
const COMMODITY_BY_FIELD = Object.freeze({
  ...
  consumo_luce_f1_kwh: "luce",
  consumo_luce_f2_kwh: "luce",
  consumo_luce_f3_kwh: "luce",
  consumo_luce_f23_kwh: "luce",
  prezzo_luce_eur_kwh: "luce",
  prezzo_luce_f0_eur_kwh: "luce",
  prezzo_luce_f1_eur_kwh: "luce",
  prezzo_luce_f2_eur_kwh: "luce",
  prezzo_luce_f3_eur_kwh: "luce",
  prezzo_luce_f23_eur_kwh: "luce",
  ...
  moltiplicatore_indice_luce: "luce",
  ...
  residenza_luce: "luce",
  consumi_mensili_luce_kwh: "luce",
  classe_contatore_gas: "gas",
  potere_calorifico_gas_gj_smc: "gas",
  ambito_tariffario_gas: "gas",
  regime_accisa_gas: "gas",
  regime_arisgam_gas: "gas",
});
...
const COMPARISON_TARGETS = Object.freeze({
  ...
  consumo_luce_f1_kwh: ["pdf-current-consumo-luce-f1"],
  consumo_luce_f2_kwh: ["pdf-current-consumo-luce-f2"],
  consumo_luce_f3_kwh: ["pdf-current-consumo-luce-f3"],
  consumo_luce_f23_kwh: ["pdf-current-consumo-luce-f23"],
  prezzo_luce_eur_kwh: ["in-luce-prezzo-att"],
  prezzo_luce_f1_eur_kwh: ["pdf-current-prezzo-luce-f1"],
  prezzo_luce_f2_eur_kwh: ["pdf-current-prezzo-luce-f2"],
  prezzo_luce_f3_eur_kwh: ["pdf-current-prezzo-luce-f3"],
  prezzo_luce_f23_eur_kwh: ["pdf-current-prezzo-luce-f23"],
  ...
  indice_riferimento_luce: ["pdf-current-indice-luce"],
  spread_luce_eur_kwh: ["pdf-current-spread-luce"],
  moltiplicatore_indice_luce: ["pdf-current-moltiplicatore-luce"],
  formula_prezzo_luce: ["pdf-current-formula-luce"],
  struttura_prezzo_luce: ["pdf-current-struttura-luce"],
  indice_riferimento_gas: ["pdf-current-indice-gas"],
  spread_gas_eur_smc: ["pdf-current-spread-gas"],
  moltiplicatore_indice_gas: ["pdf-current-moltiplicatore-gas"],
  formula_prezzo_gas: ["pdf-current-formula-gas"],
  residenza_luce: ["master-luce-residenza"],
  classe_contatore_gas: ["master-gas-contatore"],
  potere_calorifico_gas_gj_smc: ["master-gas-pcs"],
  ambito_tariffario_gas: ["master-gas-regione"],
  regime_accisa_gas: ["master-gas-accisa"],
  regime_arisgam_gas: ["master-gas-arisgam"],
});
...
const OFFER_TARGETS = Object.freeze({
  ...
  prezzo_luce_f1_eur_kwh: ["pdf-offer-prezzo-luce-f1"],
  prezzo_luce_f2_eur_kwh: ["pdf-offer-prezzo-luce-f2"],
  prezzo_luce_f3_eur_kwh: ["pdf-offer-prezzo-luce-f3"],
  prezzo_luce_f23_eur_kwh: ["pdf-offer-prezzo-luce-f23"],
  ...
  tipo_prezzo_luce: ["master-luce-tipo"],
  tipo_prezzo_gas: ["master-luce-tipo"],
  indice_riferimento_luce: ["pdf-offer-indice-luce"],
  spread_luce_eur_kwh: ["pdf-offer-spread-luce"],
  moltiplicatore_indice_luce: ["pdf-offer-moltiplicatore-luce"],
  formula_prezzo_luce: ["pdf-offer-formula-luce"],
  struttura_prezzo_luce: ["pdf-offer-struttura-luce"],
  indice_riferimento_gas: ["pdf-offer-indice-gas"],
  spread_gas_eur_smc: ["pdf-offer-spread-gas"],
  moltiplicatore_indice_gas: ["pdf-offer-moltiplicatore-gas"],
  formula_prezzo_gas: ["pdf-offer-formula-gas"],
});
...
function fieldEvidence(input, field, diagnostic, value) {
  ...
  return {
    ...
    note: !available
      ? "evidenza_testuale_non_disponibile"
      : literalValuePresent
        ? null
...
function isAiField(input, field) {
  if (!input?.ai?.applied) return false;
  const filled = new Set(Array.isArray(input.ai.filled_fields) ? input.ai.filled_fields : []);
  const aliases = DIAGNOSTIC_ALIASES[field] || [field];
  return aliases.some((alias) => filled.has(alias));
}
...
function autofillDecision(input, field, value, status, sourceType = null) {
  ...
  if (sourceType === "ai") {
    return {
      ...
      review_selectable: false,
      requires_explicit_selection: false,
      reason: "ia_visuale_completa_con_conferma_utente",
      targets,
      use,
    };
  }
...
function makeFieldMeta(input, field) {
  ...
  const aiField = isAiField(input, field);
  const sourceType = aiField ? "ai" : ocrField ? "ocr" : null;
  const decision = autofillDecision(input, field, value, status, sourceType);
  return {
    field,
    commodity: COMMODITY_BY_FIELD[field] || "common",
    normalized_value: value,
    original_value: diagnostic?.derivation?.original_value ?? evidence.literal_value ?? diagnostic?.value ?? value,
    original_value_kind: diagnostic?.derivation
      ? "derived_from_source_literal"
      : evidence.literal_value ? "source_literal" : diagnostic ? "parser_diagnostic_value" : "parser_output_value",
    unit: UNIT_BY_FIELD[field] || null,
    derivation: diagnostic?.derivation || null,
    status: status.status,
    status_reason: status.reason,
    provenance: {
      source: aiField ? "ai" : ocrField ? "ocr" : "parser",
      origin: aiField ? "pdf_visual_ai" : ocrField ? "pdf_image_ocr" : "pdf_native_text",
      source_version: aiField
        ? input.ai?.reader_version || input.parser_version || "unknown"
        : ocrField
      ...
      confidence: aiField ? diagnostic?.confidence || "medium" : ocrField ? "medium" : diagnostic?.confidence || input.confidence || "unknown",
    },
    review_required: Boolean(aiField || ocrField),
    evidence,
    autofill: decision,
  };
}
...
function supplyObject(input, commodity) {
  ...
  return {
    ...
    annual_consumption_bands: isLuce ? {
      f1: input.consumo_luce_f1_kwh ?? null,
      f2: input.consumo_luce_f2_kwh ?? null,
      f3: input.consumo_luce_f3_kwh ?? null,
      f23: input.consumo_luce_f23_kwh ?? null,
      unit: "kWh/anno",
    } : null,
    ...
    offer: {
      ...
      index_multiplier: input[isLuce ? "moltiplicatore_indice_luce" : "moltiplicatore_indice_gas"] ?? null,
      ...
      price_bands: isLuce ? {
        f0: input.prezzo_luce_f0_eur_kwh ?? null,
        f1: input.prezzo_luce_f1_eur_kwh ?? null,
        f2: input.prezzo_luce_f2_eur_kwh ?? null,
        f3: input.prezzo_luce_f3_eur_kwh ?? null,
        f23: input.prezzo_luce_f23_eur_kwh ?? null,
        unit: "EUR/kWh",
      } : null,
    },
    readiness: {
...
export function buildPdfDataContract(input = {}) {
  ...
  return {
    ...
    parser: {
      mode: input.ai?.applied
        ? "visual_ai_only"
        : input.ocr?.applied ? "deterministic_with_controlled_ocr" : "deterministic",
      parser_version: input.parser_version || "unknown",
      ocr_pipeline_version: input.ocr?.applied ? input.ocr.pipeline_version || null : null,
      ai_reader_version: input.ai?.applied ? input.ai.reader_version || null : null,
      ai_model: input.ai?.applied ? input.ai.model || null : null,
    ...
    autofill_plan: {
      ...
      requires_user_confirmation: true,
      policy: "validated_native_or_ai_fields_plus_explicitly_selected_ocr_fields",
