export const PDF_DATA_CONTRACT_VERSION = "1.3.1";
export const PDF_AUTOFILL_POLICY_VERSION = "1.3.0";
...
const DIAGNOSTIC_ALIASES = Object.freeze({
  ...
  codice_prodotto_fornitore_luce: ["codice_prodotto_fornitore_luce", "codice_prodotto_fornitore"],
  codice_prodotto_fornitore_gas: ["codice_prodotto_fornitore_gas", "codice_prodotto_fornitore"],
...
const COMMODITY_BY_FIELD = Object.freeze({
  ...
  codice_prodotto_fornitore_luce: "luce",
  ...
  codice_prodotto_fornitore_gas: "gas",
  tipo_prezzo_gas: "gas",
...
function fieldFilledBy(input, field, source) {
  const meta = input?.[source];
  if (!meta?.applied) return false;
  const filled = new Set(Array.isArray(meta.filled_fields) ? meta.filled_fields : []);
  const aliases = DIAGNOSTIC_ALIASES[field] || [field];
  return aliases.some((alias) => filled.has(alias));
}

function isOcrField(input, field) {
  return fieldFilledBy(input, field, "ocr");
}

function isAiField(input, field) {
  return fieldFilledBy(input, field, "ai");
}
...
function autofillDecision(input, field, value, status, reviewSource = null) {
  if (field === "codice_cliente" && input.commodity === "luce" && hasValue(fieldValue(input, "codice_cliente_luce"))) {
    return deniedAutofill("rappresentato_da_codice_cliente_luce", [], "activation_helper");
  }
  if (field === "codice_cliente" && input.commodity === "gas" && hasValue(fieldValue(input, "codice_cliente_gas"))) {
    return deniedAutofill("rappresentato_da_codice_cliente_gas", [], "activation_helper");
  }
  ...
  const sourceReviewStatus = reviewSource && status.status === "da_verificare"
    && ["ai_visuale_da_confermare", "ocr_da_confermare"].includes(status.reason);
  if (status.status !== "completo" && !sourceReviewStatus) return deniedAutofill(`stato_${status.status}`, targets, use);
  ...
  if (reviewSource) {
    return {
      ...
      reason: reviewSource === "ai"
        ? "ai_da_verificare_con_conferma_esplicita"
        : "ocr_da_verificare_con_conferma_esplicita",
      targets,
      use,
    };
  }
...
function makeFieldMeta(input, field) {
  ...
  const reviewSource = aiField ? "ai" : ocrField ? "ocr" : null;
  const effectiveStatus = aiField && status.status === "completo"
    ? { status: "da_verificare", reason: "ai_visuale_da_confermare" }
    : status;
  const decision = autofillDecision(input, field, value, effectiveStatus, reviewSource);
  return {
    field,
    ...
    status_reason: effectiveStatus.reason,
    provenance: {
      ...
      source_version: aiField
        ? input.ai?.pipeline_version || input.ai?.model || "unknown"
        : ocrField
          ? input.ocr?.pipeline_version || input.parser_version || "unknown"
          : input.parser_version || "unknown",
      method: aiField ? diagnostic?.method || "openai_visual_semantic" : fieldMethod(input, field, diagnostic),
      confidence: aiField
        ? input.ai?.field_meta?.[field]?.confidence || "medium"
        : ocrField
          ? "medium"
          : diagnostic?.confidence || input.confidence || "unknown",
    },
    review_required: Boolean(ocrField || aiField),
    evidence,
    autofill: decision,
  };
}
...
function supplyObject(input, commodity) {
  ...
  return {
    ...
    offer: {
      ...
      provider_product_code: input[`codice_prodotto_fornitore_${commodity}`] || null,
  ...
  return {
    ...
    parser: {
      mode: input.ai?.applied
        ? "deterministic_with_controlled_visual_ai"
        : input.ocr?.applied
          ? "deterministic_with_controlled_ocr"
          : "deterministic",
      parser_version: input.parser_version || "unknown",
      ocr_pipeline_version: input.ocr?.applied ? input.ocr.pipeline_version || null : null,
      ai_pipeline_version: input.ai?.applied ? input.ai.pipeline_version || null : null,
    ...
    autofill_plan: {
      ...
      requires_user_confirmation: true,
      policy: "verified_native_fields_plus_explicitly_selected_review_fields",
