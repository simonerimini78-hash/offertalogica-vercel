...
function photovoltaicEmailContent(lead) {
  ...
  const text = [
    ...
    "",
    `IMPORTANTE PRIVACY: il cliente ha acconsentito alla comunicazione di questi dati al consulente tecnico esterno per il solo ricontatto e approfondimento fotovoltaico richiesto. Se dopo il ricontatto il cliente vuole essere messo in contatto con un installatore o un'altra azienda identificata, raccogli e documenta una nuova conferma specifica prima di comunicare i dati. Elimina il messaggio e le eventuali copie operative entro ${photovoltaicRetentionDeadline(lead)} salvo un diverso obbligo di legge documentato.`,
  ].join("\n");
