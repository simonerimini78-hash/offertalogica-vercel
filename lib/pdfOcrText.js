...
function applyCommonOcrRecovery(candidate, text) {
  ...
  if (address) {
    setWhenMissing(candidate, "indirizzo_fornitura", address);
    if (["luce", "dual"].includes(candidate.commodity)) setWhenMissing(candidate, "indirizzo_fornitura_luce", address);
    if (["gas", "dual"].includes(candidate.commodity)) setWhenMissing(candidate, "indirizzo_fornitura_gas", address);
  }
