export const PDF_AI_PREVIEW_VERSION = "8.8.8.10";
...
export function buildPdfAiStatus(aiShadow) {
  const client = aiShadow?.diagnostics?.client;
  return {
    status_version: PDF_AI_PREVIEW_VERSION,
    endpoint_version: compact(aiShadow?.endpoint_version, 40) || null,
    shadow_version: compact(aiShadow?.shadow_version, 40) || null,
    mode: compact(aiShadow?.mode, 30) || "off",
    attempted: Boolean(aiShadow?.attempted),
    status: compact(aiShadow?.status, 40) || "unknown",
    reason: compact(aiShadow?.reason, 120) || "unknown",
    elapsed_ms: safeNumber(client?.elapsed_ms),
    error_code: compact(client?.error?.code, 120) || null,
    http_status: Number.isInteger(Number(client?.error?.http_status))
      ? Number(client.error.http_status)
      : null,
  };
}

/**
 * Builds the only AI data intentionally returned to a Vercel Preview.
