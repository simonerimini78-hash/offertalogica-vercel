...
export const PREMIUM_RED_VERIFIER_VERSION = "premium-red-verifier-v0.36.31";
...
export function routePremiumRedReasons(reasons = []) {
  const codes = [...new Set(safeArray(reasons).map((reason) => cleanCode(reason?.code)).filter(Boolean))];
  let route = "auto_ai";
...
export async function loadPremiumRedVerificationSnapshot({ config, billId, userId, fetchImpl = fetch } = {}) {
  const billQuery = new URLSearchParams({
    select: "id,user_id,automatic_screening_status,automatic_screening_reasons,automatic_analysis_run_id,red_verification_state,red_verification_result,red_verification_run_id,red_verified_at",
    id: `eq.${billId}`,
    user_id: `eq.${userId}`,
    limit: "1",
  });
...
export function normalizePremiumRedVerification(raw, { routeInfo, firstAnalysisRunId = null } = {}) {
  const verificationResult = ["confirmed", "not_confirmed", "inconclusive", "needs_data"].includes(raw?.verification_result)
  ...
  const missingData = safeArray(raw?.missing_data).slice(0, 8).map((item) => compact(item, 220)).filter(Boolean);
  const confidence = ["low", "medium", "high"].includes(raw?.confidence) ? raw.confidence : "low";
  const requestedCanResolve = raw?.can_resolve_alone === "yes" ? "yes" : "no";
  ...
  return {
    ...
    issue: compact(raw?.issue, 500) || "Anomalia rossa da verificare",
    evidence,
    verification_result: verificationResult,
    confidence,
    can_resolve_alone: autoResolved ? "yes" : requestedCanResolve,
    customer_reply: compact(raw?.customer_reply, 900),
    escalation_reason: compact(raw?.escalation_reason, 900),
    missing_data: missingData,
    decision,
    agreement_with_first_check: verificationResult === "confirmed" && missingData.length === 0,
  };
}
...
export async function verifyPremiumRedPdf({
  ...
  try {
    ...
    const context = {
      ...
      first_red_reasons: reasonContext(reasons),
    ...
    try {
      ...
      return {
        result: normalizePremiumRedVerification(parsed, { routeInfo, firstAnalysisRunId }),
