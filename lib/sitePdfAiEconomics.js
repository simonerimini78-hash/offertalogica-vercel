...
async function latestVerifiedPremiumFx(config, fetchImpl = fetch, nowMs = Date.now()) {
  if (premiumFxCache && premiumFxCache.configKey === config.supabaseUrl && nowMs - premiumFxCache.cachedAtMs < FX_CACHE_MS) return premiumFxCache;
  ...
  return premiumFxCache;
}

function ecbXmlRate(xml = "") {
  const referenceDate = String(xml).match(/<Cube\s+time=["']([^"']+)["']\s*>/i)?.[1] || "";
  const eurToUsd = Number(String(xml).match(/<Cube\s+currency=["']USD["']\s+rate=["']([^"']+)["']/i)?.[1]);
  if (!referenceDate || !Number.isFinite(eurToUsd) || eurToUsd <= 0) return null;
  const usdToEur = 1 / eurToUsd;
  if (!Number.isFinite(usdToEur) || usdToEur <= 0) return null;
  return { referenceDate, eurToUsd, usdToEur };
}

async function latestEcbUsdToEur(fetchImpl = fetch, nowMs = Date.now()) {
  if (ecbFxCache && nowMs - ecbFxCache.cachedAtMs < FX_CACHE_MS) return ecbFxCache;
  try {
    const response = await fetchWithTimeout(
      fetchImpl,
      ECB_DAILY_FX_URL,
    ...
    if (!response.ok) throw new Error(`site_pdf_ai_ecb_fx_http_${response.status}`);
    const parsed = ecbXmlRate(await response.text());
    if (!parsed) throw new Error("site_pdf_ai_ecb_fx_invalid_payload");
    ecbFxCache = {
      usdToEur: parsed.usdToEur,
      eurToUsd: parsed.eurToUsd,
      ecbReferenceDate: parsed.referenceDate,
      observedAt: new Date(nowMs).toISOString(),
      stale: false,
      cachedAtMs: nowMs,
      source: "ecb_reference_rate_direct",
    };
    return ecbFxCache;
  } catch (error) {
    if (ecbFxCache) return { ...ecbFxCache, stale: true, errorCode: String(error?.message || "ecb_fx_unavailable") };
    return null;
  }
}

async function latestSiteFx(config, fetchImpl = fetch, nowMs = Date.now()) {
  let premiumFx = null;
  try {
    premiumFx = await latestVerifiedPremiumFx(config, fetchImpl, nowMs);
  } catch {
    premiumFx = null;
  }
  if (premiumFx && !premiumFx.stale) return premiumFx;

  const directEcbFx = await latestEcbUsdToEur(fetchImpl, nowMs);
  return directEcbFx || premiumFx || null;
}
...
export function buildSitePdfAiEconomicEntry({
  ...
  const normalizedInputKind = String(inputKind || "pdf").trim().toLowerCase() === "photo" ? "photo" : "pdf";
  const sourceSystem = normalizedInputKind === "photo" ? "site_photo_ai" : "site_pdf_ai";
  const category = `${sourceSystem}_${normalizedCustomerType}`;
  ...
  return {
    ...
    notes: `${normalizedInputKind === "photo" ? "Lettura foto bolletta" : "Analisi IA calcolatore sito"} · ${label}`,
    metadata: {
      accounting_version: normalizedInputKind === "photo" ? "site-photo-ai-accounting-v1" : "site-pdf-ai-accounting-v2",
...
export async function recordSitePdfAiEconomicEvent({
  ...
  const fx = await latestSiteFx(config, fetchImpl, nowMs);
...
export async function resolveEconomicUsdToEurFx({
  env = process.env,
  fetchImpl = fetch,
  nowMs = Date.now(),
} = {}) {
  ...
  return latestSiteFx(config, fetchImpl, nowMs);
}
