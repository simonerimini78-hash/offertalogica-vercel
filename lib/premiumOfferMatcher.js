...
function economicVersionVerified(input, version) {
  if (!input || !version) return false;
  const inputType = canonicalPriceType(input.priceType);
  const versionType = canonicalPriceType(version.priceType);
  if (inputType === "unknown" || versionType === "unknown" || inputType !== versionType) return false;

  const fixedFeeClose = relativeClose(input.annualFixedFee, version.annualFixedFee, 5);
  if (fixedFeeClose === false) return false;

  if (versionType === "fisso") {
    const priceFloor = input.commodity === "luce" ? 0.005 : 0.02;
    return relativeClose(input.price, version.price, priceFloor) === true;
  }

  if (versionType === "variabile") {
    const inputIndex = trimText(input.indexName, 80);
    const versionIndex = trimText(version.indexName, 80);
    if (!inputIndex || !versionIndex || similarity(inputIndex, versionIndex) < 0.8) return false;
    const spreadFloor = input.commodity === "luce" ? 0.002 : 0.01;
    return relativeClose(input.spread, version.spreadEstimate, spreadFloor) === true;
  }

  // Le offerte ibride/miste richiedono condizioni più articolate di un singolo
  // prezzo o spread. Senza una versione economica inequivocabile restano non verificate.
  return false;
}
...
function matchSupply(input, records, onDate) {
  ...
  if (top.exactCode) {
    return {
      ...
      confidence: 100,
      method: "offer_code",
      verified: economicVersionVerified(input, top.version),
      candidate: top,
      candidates: candidates.slice(0, 3).map(publicCandidate),
    };
  }

  const strongIdentity = Boolean(input.offerName)
    && top.providerSimilarity >= 0.65
    && top.nameSimilarity >= 0.65;
  if (top.score >= 90 && margin >= 8 && strongIdentity && economicVersionVerified(input, top.version)) {
    return {
      commodity: input.commodity,
      status: "matched",
...
function findSingleByCode(records, input, code, onDate) {
  ...
  return version ? {
    ...
    verified: economicVersionVerified(input, version),
    candidate: {
      record,
      version,
      score: 100,
...
function dualCodeMatches(inputs, records, onDate) {
  ...
  for (const code of codes) {
    ...
    if (!version) continue;
    const lightInput = inputs.find(input => input.commodity === "luce") || null;
    const gasInput = inputs.find(input => input.commodity === "gas") || null;
    const light = findSingleByCode(records, lightInput, version.electricityOfferCode, onDate);
    const gas = findSingleByCode(records, gasInput, version.gasOfferCode, onDate);
...
export function buildPremiumContractValues(normalized = {}, match = {}, sourceUrl = "") {
  ...
  // Nome/fornitore compatibili non bastano a trasformare una versione storica
  // simile nelle condizioni contrattuali del cliente. I valori di catalogo sono
  // usati soltanto quando anche la specifica versione economica è verificata.
  const lightVersion = verified ? light?.candidate?.version || null : null;
  const gasVersion = verified ? gas?.candidate?.version || null : null;
  const useMatched = verified;
  ...
  return {
    ...
    arera_offer_code_electricity: verified ? trimText(light?.candidate?.record?.offerCode, 240) : "",
    arera_offer_code_gas: verified ? trimText(gas?.candidate?.record?.offerCode, 240) : "",
    arera_history_key_electricity: verified ? trimText(light?.candidate?.record?.key, 300) : "",
    arera_history_key_gas: verified ? trimText(gas?.candidate?.record?.key, 300) : "",
    electricity_arera_valid_from: verified ? areraDateToIso(lightVersion?.validFrom) : null,
    electricity_arera_valid_to: verified ? areraDateToIso(lightVersion?.validTo) : null,
    gas_arera_valid_from: verified ? areraDateToIso(gasVersion?.validFrom) : null,
    gas_arera_valid_to: verified ? areraDateToIso(gasVersion?.validTo) : null,
    ...
    // Le alternative restano un dettaglio interno del matcher: se la versione
    // economica non è verificata non vengono persistite come proposte al cliente.
    automatic_match_candidates: verified ? match.supplies.map(item => ({
    ...
    // La memoria dell'utente non viene usata per certificare una versione tariffaria.
    // Se il matcher non ha certezza economica, non viene aperta alcuna conferma.
    customer_confirmation_status: verified ? "not_required" : "not_available",
...
async function persistContract({ config, bill, values, fetchImpl }) {
  ...
  const legacyCustomerConfirmed = current?.verification_status === "verified"
    && current?.source === "import"
    && current?.automatic_match_method === "customer_confirmed";
  ...
  if (current && (
    legacyCustomerConfirmed
    || sameAutomaticIdentity(current, values)
    || ["unverified", "needs_review"].includes(current.verification_status)
  )) {
...
function publicSummary(match, contract, sourceUrl, preserved = false) {
  const exposeCandidates = contract?.verification_status === "verified";
  return {
    ...
    supplies: match.supplies.map(item => ({
      ...
      confidence: item.confidence,
      method: item.method,
      selected: exposeCandidates ? publicCandidate(item.candidate) : null,
      candidates: exposeCandidates ? item.candidates : [],
    })),
  };
}
