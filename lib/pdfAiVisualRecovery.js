export const PDF_AI_VISUAL_RECOVERY_VERSION = "v106.8.8.3.1-single-pass-review-safe-1";
...
export function prepareSafeAiVisualCandidates(ai = {}) {
  ...
  for (const field of ["pod", "pdr", "codice_fiscale"]) {
    ...
    for (const candidate of candidates.filter((item) => item?.field === field)) {
      ...
      // Un identificativo formalmente valido e chiaramente etichettato resta sempre
      // revisionabile, anche quando compare in un solo passaggio visuale.
      candidate.warnings = unique([
        ...candidateWarnings(candidate),
        "requires_explicit_user_confirmation",
        "single_pass_identifier_review",
      ]);
...
export function safeVisualFieldThreshold(candidate) {
  ...
  if (field === "pod" && EXPLICIT_POD_LABEL.test(context) && normalizedPod(raw)) {
    return warnings.includes(CONSENSUS_WARNING) ? 90 : 88;
  }
  if (field === "pdr" && EXPLICIT_PDR_LABEL.test(context) && normalizedPdr(raw)) {
    return warnings.includes(CONSENSUS_WARNING) ? 90 : 88;
  }
