export const PDF_AI_VISUAL_RECOVERY_VERSION = "v106.8.8-visual-direct-fields-recovery-1";
...
export function italianTaxIdNameCodes(holder) {
  ...
  const possibilities = [];
  const lastAsSurname = tokens[tokens.length - 1];
  const firstAsGiven = tokens.slice(0, -1).join("");
  possibilities.push(`${surnameCode(lastAsSurname)}${givenNameCode(firstAsGiven)}`);

  const firstAsSurname = tokens[0];
  const restAsGiven = tokens.slice(1).join("");
  possibilities.push(`${surnameCode(firstAsSurname)}${givenNameCode(restAsGiven)}`);
...
export function recoverItalianTaxIdCandidate({ candidate, holder = "" } = {}) {
  ...
  const raw = candidate.value_text ?? candidate.normalized_value ?? candidate.value_number;
  ...
  return {
    ...
    evidence: compact(`${candidate.evidence || context}; recupero controllato da caratteri OCR ambigui e coerenza con intestatario`, 360),
...
export function explicitVisualIdentifierThreshold(candidate) {
  ...
  const context = `${candidate?.label || ""} ${candidate?.evidence || ""}`;
  const raw = candidate?.normalized_value ?? candidate?.value_text ?? candidate?.value_number;
  if (field === "pod" && EXPLICIT_POD_LABEL.test(context) && normalizedPod(raw)) return 88;
  if (field === "pdr" && EXPLICIT_PDR_LABEL.test(context) && normalizedPdr(raw)) return 88;
