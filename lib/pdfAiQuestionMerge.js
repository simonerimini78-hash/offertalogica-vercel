...
export const PDF_AI_QUESTION_PIPELINE_VERSION = "step8-question-pipeline-v2-page-isolated-session";
...
function diagnosticFor(result) {
  return {
    ...
    method: "gpt41_targeted_single_page_verified_question",
...
function publicQuestionBatch(item) {
  ...
  return {
    ...
    page_selection: "single_page_selected_by_literal_index",
...
export function mergePdfAiQuestionSession({
    ...
    return {
      ...
      audit: {
        ...
        ai: {
          ...
          plan: {
            mode: "page_upload_once_sequential_independent_questions",
  ...
  for (const item of session.results || []) {
    ...
    merged.diagnostics.push(diagnosticFor({ ...result, question_id: item.id }));
    promoted.push({
      field,
      value: selectedValue,
      status: "review",
    ...
    audit: {
      ...
      ai: {
        ...
        plan: {
          ...
          page_isolated: true,
          question_count: Number(session.results?.length || 0),
          concurrency: "browser_sequential",
          completed_question_count: Number(session.results?.filter((item) => item.status === "completed").length || 0),
          session_version: session.session_version || null,
          catalog_version: session.catalog_version || null,
          validation_version: session.validation_version || null,
