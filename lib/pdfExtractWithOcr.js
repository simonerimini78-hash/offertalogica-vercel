  ...
  try {
    const ocr = await runControlledPdfOcr(filePath, {
      ...
      onPageText: ({ pageTexts }) => {
        const interim = extractPdfDataFromText(pageTexts.join("\n"));
        // Interrompe presto quando la prima pagina ha già prodotto un risultato robusto.
        return scorePdfResult(interim) < 12;
      },
    });
    const ocrText = ocr.pageTexts.join("\n");
    const candidate = extractPdfDataFromText(ocrText);
    candidate.page_count = base.page_count;
    candidate.diagnostics = buildPdfDiagnostics(candidate, ocr.pageTexts);
