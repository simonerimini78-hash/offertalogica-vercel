...
const DEFAULT_PHOTOVOLTAIC_RETENTION_DAYS = 120;
const RETENTION_CLEANUP_INTERVAL_MS = 6 * 60 * 60 * 1000;
let lastPhotovoltaicRetentionCleanupAt = 0;

function photovoltaicRetentionDays() {
  const requested = Number(process.env.CUSTOMER_DB_PHOTOVOLTAIC_RETENTION_DAYS || DEFAULT_PHOTOVOLTAIC_RETENTION_DAYS);
  if (!Number.isFinite(requested)) return DEFAULT_PHOTOVOLTAIC_RETENTION_DAYS;
...
function hashValue(value) {
  const secret = process.env.CUSTOMER_DB_HASH_SECRET || "";
...
async function deletePhotovoltaicLeadsCreatedBefore(cutoffIso) {
  const query = new URLSearchParams({
    created_at: `lt.${cutoffIso}`,
    "calculation->>requestType": "eq.photovoltaic_consulting",
  });
  const response = await fetch(tableUrl(LEADS_TABLE, `?${query.toString()}`), {
    method: "DELETE",
    headers: headers("return=minimal"),
  });
  if (!response.ok) throw new Error(`Customer DB photovoltaic retention error ${response.status}`);
}

async function maybePrunePhotovoltaicLeads(lead) {
  if (!configured()) return { ok: true, skipped: true, reason: "not_configured" };
  if (lead?.calculation?.requestType !== "photovoltaic_consulting") {
    return { ok: true, skipped: true, reason: "not_photovoltaic" };
  }
  const now = Date.now();
  if (now - lastPhotovoltaicRetentionCleanupAt < RETENTION_CLEANUP_INTERVAL_MS) {
    return { ok: true, skipped: true, reason: "recently_checked" };
  }
  lastPhotovoltaicRetentionCleanupAt = now;
  const retentionDays = photovoltaicRetentionDays();
  const cutoffIso = new Date(now - retentionDays * 86_400_000).toISOString();
  try {
    await deletePhotovoltaicLeadsCreatedBefore(cutoffIso);
    return { ok: true, skipped: false, retentionDays, cutoffIso };
  } catch (error) {
    console.warn("customer_db_photovoltaic_retention_cleanup_failed", String(error?.message || "retention_cleanup_error").slice(0, 240));
    return { ok: false, skipped: false, retentionDays, cutoffIso, error: error?.message || "retention_cleanup_error" };
  }
}
...
export async function persistLeadSnapshot(lead, eventType, payload = {}) {
  ...
  try {
    ...
    await maybePrunePhotovoltaicLeads(lead);
...
export async function checkCustomerDb() {
  ...
  try {
    ...
    return {
      ...
      photovoltaicRetentionDays: photovoltaicRetentionDays(),
    };
  } catch (error) {
    return {
      ok: false,
...
const EVENT_LABELS_IT = Object.freeze({
  ...
  comparison_missing_current_price: "Confronto: dati tariffa mancanti",
...
function eventLabelIt(eventType) {
  const key = String(eventType || "").trim();
  return EVENT_LABELS_IT[key] || key;
}

function engagementStageLabelIt(stage) {
  const labels = {
    landing: "Landing",
...
function dataOriginLabelIt(origin) {
  const labels = {
...
function summaryFromEvents(events) {
  ...
  events.forEach((event) => {
    ...
    if (event.eventType === "offer_redirect" || event.eventType === "offer_switcho_redirect") funnel.redirects += 1;
    if (event.eventType === "switcho_landing_opened") funnel.switchoLandings += 1;
