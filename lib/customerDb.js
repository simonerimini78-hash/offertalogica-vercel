...
function safeLeadIds(values = []) {
  return [...new Set((Array.isArray(values) ? values : [])
    .map(value => String(value || "").trim().slice(0, 100))
    .filter(value => /^[A-Za-z0-9_-]+$/.test(value)))];
}

function safeEventIds(values = []) {
  return [...new Set((Array.isArray(values) ? values : [])
...
export async function deleteCustomerLeads(options = {}) {
  ...
  const ids = safeLeadIds(options.ids);
  if (id && /^[A-Za-z0-9_-]+$/.test(id) && !ids.includes(id)) ids.push(id);
  const resetAll = options.all === true;
  if (!ids.length && !resetAll) return { ok: false, configured: true, status: "invalid_request", deletedCount: 0 };

  try {
    let leadIds = ids;
    if (resetAll) leadIds = await leadIdsForDeletion("");
    if (resetAll) {
      await deleteRows(EVENTS_TABLE, "lead_id=not.is.null");
      await deleteRows(LEADS_TABLE, "id=not.is.null");
    } else if (leadIds.length) {
      const filter = inFilter(leadIds);
      await deleteRows(EVENTS_TABLE, `lead_id=${filter}`);
      await deleteRows(LEADS_TABLE, `id=${filter}`);
    }
    return {
      ok: true,
      configured: true,
...
export async function deleteCustomerAnalytics(options = {}) {
  if (!configured()) return { ok: false, configured: false, status: "not_configured", deletedCount: 0 };
  const ids = safeEventIds(options.ids);
  ...
  try {
    if (resetAll) {
      await deleteRows(EVENTS_TABLE, "id=not.is.null");
      return { ok: true, configured: true, status: "ready", deletedCount: null, deletedIds: [], resetAll: true };
    }
    await deleteRows(EVENTS_TABLE, `id=${inFilter(ids)}`);
    return { ok: true, configured: true, status: "ready", deletedCount: ids.length, deletedIds: ids, resetAll: false };
  } catch (error) {
    return {
      ok: false,
      configured: true,
      status: "error",
      error: error.message || "customer_db_analytics_delete_error",
      deletedCount: 0,
    };
  }
}

const EVENT_LABELS_IT = Object.freeze({
  ...
  activation_channel_choice_opened: "Percorso offerta aperto",
  activation_channel_selected: "Percorso offerta selezionato",
  provider_site_redirect: "Sito fornitore non affiliato",
...
function eventLabelIt(value) {
  const key = String(value || "").trim();
  return EVENT_LABELS_IT[key] || key;
}

function dataOriginLabelIt(value) {
  const labels = {
...
function engagementStageLabelIt(value) {
  const labels = { landing: "Landing", calculator: "Calcolatore", offers: "Offerte", otp: "Verifica numero", other: "Altro" };
  const key = String(value || "").trim();
  return labels[key] || key;
}
...
function mapEventRow(row) {
  ...
  return {
    ...
    eventLabel: eventLabelIt(row.event_type || ""),
    createdAt: row.created_at,
    sessionId: payload.sessionId || "",
    page: payload.page || "",
    customerType: payload.customerType || "",
    dataOrigin: payload.dataOrigin || "",
    dataOriginLabel: dataOriginLabelIt(payload.dataOrigin || ""),
    source: payload.source || payload.leadSource || "",
    leadSource: payload.leadSource || "",
    trafficSource: payload.trafficSource || "",
    trafficMedium: payload.trafficMedium || "",
    trafficCampaign: payload.trafficCampaign || "",
    trafficTerm: payload.trafficTerm || "",
    trafficContent: payload.trafficContent || "",
    trafficCampaignId: payload.trafficCampaignId || "",
    trafficAdGroupId: payload.trafficAdGroupId || "",
    trafficCreativeId: payload.trafficCreativeId || "",
    trafficMatchType: payload.trafficMatchType || "",
    trafficDevice: payload.trafficDevice || "",
    trafficNetwork: payload.trafficNetwork || "",
    trafficReferrer: payload.trafficReferrer || "",
    trafficLandingPage: payload.trafficLandingPage || "",
    trafficClickIdType: payload.trafficClickIdType || "",
    trafficClickId: payload.trafficClickId || "",
    articleSlug: payload.articleSlug || "",
    articleTitle: payload.articleTitle || "",
    articleCategory: payload.articleCategory || "",
    articleType: payload.articleType || "",
    articleCanonical: payload.articleCanonical || "",
    consentAction: payload.consentAction || "",
    consentSource: payload.consentSource || "",
    trafficAgent: payload.trafficAgent || "",
    trafficReason: payload.trafficReason || "",
    clientTimestamp: payload.client_timestamp || payload.clientTimestamp || "",
    sessionEventSeq: payload.session_event_seq == null && payload.sessionEventSeq == null
      ? null
      : numberOrNull(payload.session_event_seq ?? payload.sessionEventSeq),
    routingVersion: payload.routingVersion || "",
    route: payload.route || "",
    channel: payload.channel || "",
    tipoPrezzo: payload.tipoPrezzo || "",
    tipoFornitura: payload.tipoFornitura || "",
    regioneGas: payload.regioneGas || "",
    potenzaKw: numberOrNull(payload.potenzaKw),
    luceConsumoKwh: numberOrNull(payload.luceConsumoKwh),
    gasConsumoSmc: numberOrNull(payload.gasConsumoSmc),
    fornitoreAttuale: payload.fornitoreAttuale || "",
    fornitoreLuceAttuale: payload.fornitoreLuceAttuale || "",
    fornitoreGasAttuale: payload.fornitoreGasAttuale || "",
    fornitoreNuovaOfferta: payload.fornitoreNuovaOfferta || "",
    context: payload.context || "",
    field: payload.field || "",
    mode: payload.mode || "",
    hasPendingUrl: typeof payload.hasPendingUrl === "boolean" ? payload.hasPendingUrl : null,
    assisted: typeof payload.assisted === "boolean" ? payload.assisted : null,
    businessCatalogOffersCount: numberOrNull(payload.businessCatalogOffersCount),
    businessCatalogDualOffersCount: numberOrNull(payload.businessCatalogDualOffersCount),
    staffMode: typeof payload.staffMode === "boolean" ? payload.staffMode : null,
    demoMode: typeof payload.demoMode === "boolean" ? payload.demoMode : null,
    toolCode: payload.toolCode || "",
    toolAction: payload.toolAction || "",
    toolOutcome: payload.toolOutcome || "",
    toolContext: payload.toolContext || "",
    toolVersion: payload.toolVersion || "",
    rankingOffersCount: numberOrNull(payload.rankingOffersCount),
    bestPartnerSaving: numberOrNull(payload.bestPartnerSaving),
    engagementStage: payload.engagementStage || "",
    engagementStageLabel: engagementStageLabelIt(payload.engagementStage || ""),
    engagementReason: payload.engagementReason || "",
    engagementActiveSeconds: numberOrNull(payload.engagementActiveSeconds),
    engagementElapsedSeconds: numberOrNull(payload.engagementElapsedSeconds),
    engagementLandingSeconds: numberOrNull(payload.engagementLandingSeconds),
    engagementCalculatorSeconds: numberOrNull(payload.engagementCalculatorSeconds),
    engagementOffersSeconds: numberOrNull(payload.engagementOffersSeconds),
    engagementOtpSeconds: numberOrNull(payload.engagementOtpSeconds),
    engagementFirstActionSeconds: numberOrNull(payload.engagementFirstActionSeconds),
    engagementOffersReachedSeconds: numberOrNull(payload.engagementOffersReachedSeconds),
    ...
    pdfUploadSource: payload.pdfUploadSource || "",
    pdfUploadOfferId: payload.pdfUploadOfferId || "",
    pdfUploadOfferName: payload.pdfUploadOfferName || "",
    pdfUploadProvider: payload.pdfUploadProvider || "",
    ...
    unrecognizedCount: numberOrNull(payload.unrecognizedCount),
    selectedCount: numberOrNull(payload.selectedCount),
    acceptedCount: numberOrNull(payload.acceptedCount),
    duplicateCount: numberOrNull(payload.duplicateCount),
    fieldCount: numberOrNull(payload.fieldCount),
    protectedCount: numberOrNull(payload.protectedCount),
    ocrReviewCount: numberOrNull(payload.ocrReviewCount),
    skippedCount: numberOrNull(payload.skippedCount),
    protectedSkippedCount: numberOrNull(payload.protectedSkippedCount),
    missingFieldCount: numberOrNull(payload.missingFieldCount),
    reviewFieldCount: numberOrNull(payload.reviewFieldCount),
    activeSlot: payload.activeSlot || "",
    retainedCurrentDocuments: numberOrNull(payload.retainedCurrentDocuments),
    retainedOfferDocuments: numberOrNull(payload.retainedOfferDocuments),
    pathChoice: payload.pathChoice || "",
    trigger: payload.trigger || "",
    analysisStatus: payload.analysisStatus || "",
    diagnosticCode: payload.diagnosticCode || "",
    analysisStage: payload.analysisStage || "",
    ingressMode: payload.ingressMode || "",
    diagnosticCodes: Array.isArray(payload.diagnosticCodes) ? payload.diagnosticCodes : [],
    analysisStages: Array.isArray(payload.analysisStages) ? payload.analysisStages : [],
    ingressModes: Array.isArray(payload.ingressModes) ? payload.ingressModes : [],
    documentKinds: Array.isArray(payload.documentKinds) ? payload.documentKinds : [],
    ...
    pdfAnalysisIds: Array.isArray(payload.pdfAnalysisIds) ? payload.pdfAnalysisIds : [],
    pdfArchiveStored: typeof payload.pdfArchiveStored === "boolean" ? payload.pdfArchiveStored : null,
    pdfContractVersion: payload.pdfContractVersion || "",
    pdfParserMode: payload.pdfParserMode || "",
    pdfParserVersion: payload.pdfParserVersion || "",
    pdfReaderConfidence: payload.pdfReaderConfidence || "",
    pdfPageCount: numberOrNull(payload.pdfPageCount),
    inputSource: payload.inputSource || "",
    photoReadable: typeof payload.photoReadable === "boolean" ? payload.photoReadable : null,
    comparisonAutoStarted: typeof payload.comparisonAutoStarted === "boolean" ? payload.comparisonAutoStarted : null,
    pdfFieldDiagnostics: Array.isArray(payload.pdfFieldDiagnostics) ? payload.pdfFieldDiagnostics : [],
    mixedDocuments: Boolean(payload.mixedDocuments),
    mergeBlocked: Boolean(payload.mergeBlocked),
    gasDecision: payload.gasDecision || "",
    electricityDecision: payload.electricityDecision || "",
    hasNewOffer: typeof payload.hasNewOffer === "boolean" ? payload.hasNewOffer : null,
...
export async function listCustomerAnalytics(options = {}) {
  ...
  const sessionId = String(options.sessionId || "").trim().slice(0, 160);
  const limit = limitValue(options.limit, 100, sessionId ? 5000 : 2000);
  const query = new URLSearchParams({
    select: "id,lead_id,event_type,created_at,payload",
    order: "created_at.desc",
    limit: String(limit),
  });
  if (sessionId) query.set("payload->>sessionId", `eq.${sessionId}`);

  try {
    const response = await fetch(tableUrl(EVENTS_TABLE, `?${query.toString()}`), {
      method: "GET",
      headers: headers("return=representation"),
    });
    if (!response.ok) {
