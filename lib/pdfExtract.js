...
export function extractPdfDataFromText(text = "") {
  ...
  const hasLuceData = Boolean(consumoLuce || prezzoLuce || pod);
  const hasGasData = Boolean(consumoGas || prezzoGas || pdr);
  const hasLuce = hasLuceData || (!hasGasData && (lower.includes("energia elettrica") || lower.includes("kwh")));
  const hasGas = hasGasData || (!hasLuceData && (lower.includes("gas naturale") || lower.includes("smc")));
