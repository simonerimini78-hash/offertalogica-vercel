...
import { scopedCommodityText, segmentPdfText } from "./pdfSemanticSegments.js";
import { extractOfferDetails, hasGroundedOfferUnit } from "./pdfOfferDetails.js";
import { applyPdfFieldValidation } from "./pdfFieldValidation.js";
import { applyPdfDataContract } from "./pdfDataContract.js";

export const PDF_PARSER_VERSION = "v103-safe-data-contract-step5";
...
function detectProvider(text) {
  const providers = [
    ...
    [/\bestra(?:\s+energie)?\b/i, "Estra Energie"],
...
function extractExplicitAnnualConsumption(text, unit) {
  const normalizedUnit = String(unit || "").toLowerCase();
  if (!["kwh", "smc"].includes(normalizedUnit)) return null;

  const source = String(text || "");
  const valuePattern = "([0-9]{1,3}(?:[.\\u00A0][0-9]{3})+(?:,[0-9]+)?|[0-9]+(?:[,.][0-9]+)?)";
  const definitions = [
    { score: 165, pattern: new RegExp(`(?:consumo\\s+(?:annuo|annuale)(?:\\s+aggiornato)?)\\s*\\(\\s*${normalizedUnit}\\s*\\)\\s*[:：]?\\s*${valuePattern}`, "gi") },
    { score: 155, pattern: new RegExp(`in\\s+un\\s+anno\\s+hai\\s+consumato[\\s\\S]{0,120}?${valuePattern}\\s*${normalizedUnit}\\b`, "gi") },
    { score: 150, pattern: new RegExp(`totale\\s+consumo\\s+annuo[\\s\\S]{0,120}?${valuePattern}\\s*${normalizedUnit}\\b`, "gi") },
    { score: 145, pattern: new RegExp(`consumo\\s+annuo\\s+aggiornato[\\s\\S]{0,120}?${valuePattern}\\s*${normalizedUnit}\\b`, "gi") },
    { score: 140, pattern: new RegExp(`(?:consumo\\s+annuo|consumo\\s+annuale)\\s*[:：]?[\\s\\S]{0,100}?${valuePattern}\\s*${normalizedUnit}\\b`, "gi") },
    { score: 130, pattern: new RegExp(`progressivo\\s+consumo\\s+annuo[\\s\\S]{0,100}?${valuePattern}\\s*${normalizedUnit}\\b`, "gi") },
    { score: 155, pattern: new RegExp(`totale\\s+consumo\\s+annuo\\s+(?:in\\s+)?${normalizedUnit}\\b[\\s\\S]{0,100}?${valuePattern}`, "gi") },
  ];
  ...
  for (const definition of definitions) {
    for (const match of source.matchAll(definition.pattern)) {
      ...
      if (/consumo\s+annuo\s+stimato\s+compreso|compreso\s+fra|fascia\s+di\s+consumo|valore\s+di\s+soglia|scaglione|caso\s+[a-z]\b/i.test(context)) continue;
      if (normalizedUnit === "smc" && /consumo\s+(?:annuo|annuale)[\s\S]{0,30}?\(\s*mc\s*\)/i.test(context)) continue;
      if (/quota\s+per\s+consumi|consumo\s+del\s+periodo|scontrino\s+dell['’]energia|consumo\s+effettivo/i.test(context)) continue;
      const value = numberFromItalian(match[1]);
      if (!Number.isFinite(value) || value <= 0) continue;
      candidates.push({ value, score: definition.score });
    }
  }
  ...
  for (const candidate of candidates) {
    ...
    const current = grouped.get(key) || { value: candidate.value, score: 0, count: 0 };
  ...
  if (ranked.length > 1 && ranked[0].score === ranked[1].score && ranked[0].count === ranked[1].count) return null;
  return ranked[0].value;
}

function annualConsumption(text, unit) {
  return extractExplicitAnnualConsumption(text, unit)
    ?? selectAnnualConsumptionFromText(text, unit);
}
...
function extractBillPrice(text, commodity) {
  ...
  return matchNumber(compact, [
    commodity === "luce"
      ? /di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:vendita\s+(?:di\s+)?energia\s+elettrica|materia\s+energia)[\s\S]{0,140}?([0-9]+[,.][0-9]{3,})\s*€?\s*\/?\s*kwh/i
      : /di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:vendita\s+(?:di\s+)?gas\s+naturale|materia\s+gas\s+naturale)[\s\S]{0,140}?([0-9]+[,.][0-9]{3,})\s*€?\s*\/?\s*smc/i,
    commodity === "luce"
    ...
    commodity === "gas"
      ? /comp\.?\s*sost\.?\s*mat\.?\s*prima(?:\s+contratto)?[\s\S]{0,80}?([0-9]+[,.][0-9]{3,})\s*(?:€|euro)\s*\/?\s*smc/i
      : /$a/,
    commodity === "luce"
      ? /di\s+cui\s+spesa\s+per\s+(?:la\s+)?vendita\s+(?:di\s+)?energia\s+elettrica\s+([0-9]+[,.][0-9]{3,})\s*€\/?kwh/i
      : /di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:(?:vendita\s+(?:di\s+)?)|materia\s+)gas\s+naturale\s+([0-9]+[,.][0-9]{3,})\s*€\/?smc/i,
    commodity === "luce"
...
function extractMonthlyFixed(text, commodity) {
  ...
  return matchNumber(compact, [
    commodity === "luce"
      ? /quota\s+fissa[\s\S]{0,700}?di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:(?:vendita\s+(?:di\s+)?)|materia\s+)energia\s+(?:elettrica\s+)?[\s\S]{0,160}?([0-9]+[,.][0-9]{2,8})\s*€?\s*\/?\s*mese/i
      : /quota\s+fissa[\s\S]{0,700}?di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:(?:vendita\s+(?:di\s+)?)|materia\s+)gas\s+(?:naturale\s+)?[\s\S]{0,160}?([0-9]+[,.][0-9]{2,8})\s*€?\s*\/?\s*mese/i,
    commodity === "luce"
      ? /quota\s+fissa[\s\S]{0,700}?di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:(?:vendita\s+(?:di\s+)?)|materia\s+)energia\s+(?:elettrica\s+)?(?:\d+\s+)?([0-9]+[,.][0-9]{2,6})[\s\S]{0,100}?€\s*\/?mese/i
      : /quota\s+fissa[\s\S]{0,700}?di\s+cui\s+spesa\s+per\s+(?:la\s+)?(?:(?:vendita\s+(?:di\s+)?)|materia\s+)gas\s+(?:naturale\s+)?(?:\d+\s+)?([0-9]+[,.][0-9]{2,6})[\s\S]{0,100}?€\s*\/?mese/i,
...
function normalizeOfferName(value) {
  const normalized = String(value || "")
    .replace(/\s+(?:codice|data\s+scadenza|mercato\s+libero|tipologia\s+cliente|tipologia\s+di\s+uso|tipologia\s+pagamento|data\s+di\s+attivazione)\b.*$/i, "")
    .replace(/\.{4,}.*$/, "")
    .replace(/\s+/g, " ")
    .trim();
...
function extractOfferPriceTypeEvidence(section, type) {
  const source = String(section || "");
  const patterns = type === "ibrido"
    ? [/struttura\s+di\s+prezzo\s+non\s+convenzionale/i, /struttura\s+ibrida/i, /prezzo\s+ibrido/i]
    : type === "variabile"
      ? [/tipologia\s+(?:di\s+)?offerta\s*:\s*(?:a\s+)?prezzo\s+variabile/i, /tipologia\s+offerta\s*:\s*variabile/i, /prezzo\s+(?:variabile|indicizzato)/i, /indice\s+di\s+riferimento\s*:\s*(?:pun|psv)/i]
      : type === "fisso"
        ? [/tipologia\s+(?:di\s+)?offerta\s*:\s*(?:a\s+)?prezzo\s+fisso/i, /tipologia\s+offerta\s*:\s*fisso/i, /prezzo\s+fisso/i]
        : [];
  for (const pattern of patterns) {
    const match = source.match(pattern);
    if (match?.[0]) return match[0].replace(/\s+/g, " ").trim().slice(0, 160);
  }
  return null;
}
...
function extractOfferSpread(section, commodity) {
  ...
  return { value: inferredValue, explicitUnit: inferredValue !== null && hasGroundedOfferUnit(section, commodity, "spread") };
}
...
function extractLegacyOfferData(text, lower) {
  const nomeOfferta = normalizeOfferName(matchText(text, [
    ...
    /nome\s+dell['’]offerta\s+commerciale\s*:\s*([^\n\r]{2,100})/i,
    /nome\s+offerta\s*:\s*([^\n\r]{2,100})/i,
    /denominazione\s+contratto\s*:?\s*([^\n\r]{2,100})/i,
    /\bofferta\s*:\s*([^\n\r]{2,100})/i,
  ]));
  const codiceOfferta = matchText(text, [/codice\s+offerta\s*:\s*([A-Z0-9_-]{12,})/i]);
  const tipoPrezzo = detectOfferPriceType(lower);
  const tipoPrezzoEvidence = extractOfferPriceTypeEvidence(text, tipoPrezzo);
  ...
  const detailsCommodity = likelyLuce && !likelyGas ? "luce" : likelyGas && !likelyLuce ? "gas" : null;
  const details = extractOfferDetails(text, detailsCommodity);
  return {
    nomeOfferta,
    codiceOfferta,
    tipoPrezzo,
...
function extractOfferData(text, lower) {
  const source = String(text || "");
  const matches = [...source.matchAll(/(?:\bnome\s+dell['’]offerta\s+commerciale|\bnome\s+offerta|\bdenominazione\s+commerciale\s+offerta|\bdenominazione\s+contratto|(?:^|\n)\s*offerta)\s*:?\s*([^\n\r]{2,180})/gim)];
  const entries = matches.map((match, index) => {
    ...
    const typeEvidence = extractOfferPriceTypeEvidence(section, type);
    ...
    const details = extractOfferDetails(section, commodity);
    const score = [name, code, commodity, type, offerIndex, spread, fixedAnnual, details.validFrom, details.validTo, details.formula].filter((value) => value !== null && value !== "").length;
    return { name, code, commodity, type, typeEvidence, index: offerIndex, spread, spreadExplicitUnit: spreadResult.explicitUnit, fixedAnnual, ...details, score };
  });
  return {
...
function commonOfferArray(luce, gas, key) {
  const left = luce?.[key];
  const right = gas?.[key];
  if (!Array.isArray(left) || !Array.isArray(right)) return [];
  return JSON.stringify(left) === JSON.stringify(right) ? left : [];
}
...
function finalizeOfferData(offer, commodity) {
  ...
  if (commodity === "luce" && !luce && fallback.nomeOfferta) {
    luce = { name: fallback.nomeOfferta, code: fallback.codiceOfferta, type: fallback.tipoPrezzo, typeEvidence: fallback.tipoPrezzoEvidence, index: fallback.indice, spread: fallback.spreadLuce, spreadExplicitUnit: fallback.spreadLuceExplicitUnit, fixedAnnual: fallback.costoFissoAnno, ...fallback };
  }
  if (commodity === "gas" && !gas && fallback.nomeOfferta) {
    gas = { name: fallback.nomeOfferta, code: fallback.codiceOfferta, type: fallback.tipoPrezzo, typeEvidence: fallback.tipoPrezzoEvidence, index: fallback.indice, spread: fallback.spreadGas, spreadExplicitUnit: fallback.spreadGasExplicitUnit, fixedAnnual: fallback.costoFissoAnno, ...fallback };
  }

  const mono = commodity === "luce" ? luce : commodity === "gas" ? gas : null;
  const shared = (key) => mono?.[key] ?? (commodity === "dual" ? commonOfferValue(luce, gas, key) : fallback?.[key] ?? null);
  return {
    luce,
    gas,
    ...
    tipoPrezzoEvidence: mono?.typeEvidence ?? (commodity === "dual" ? commonOfferValue(luce, gas, "typeEvidence") : fallback.tipoPrezzoEvidence),
    ...
    priceStructure: shared("priceStructure"),
    indexPeriodicity: shared("indexPeriodicity"),
    billingFrequency: shared("billingFrequency"),
    formula: shared("formula"),
    terminationFee: shared("terminationFee"),
    economicConditionsCode: shared("economicConditionsCode"),
    otherFeatures: shared("otherFeatures"),
    contractExpiry: shared("contractExpiry"),
    components: mono?.components ?? (commodity === "dual" ? commonOfferArray(luce, gas, "components") : fallback.components || []),
    discounts: mono?.discounts ?? (commodity === "dual" ? commonOfferArray(luce, gas, "discounts") : fallback.discounts || []),
    discountValues: mono?.discountValues ?? (commodity === "dual" ? commonOfferArray(luce, gas, "discountValues") : fallback.discountValues || []),
  };
}
...
function extractServiceAddress(text, commodity) {
  const source = String(text || "");
  const marker = commodity === "gas" ? "(?:punto\\s+di\\s+riconsegna(?:\\s*\\(PDR\\))?|PDR)" : "(?:punto\\s+di\\s+prelievo(?:\\s*\\(POD\\))?|POD)";
  const street = "(?:via|viale|piazza|piazzale|corso|vicolo|largo|strada|contrada|localit[aà])";
  return normalizeAddress(matchText(source, [
    new RegExp(`servizio\\s+fornito\\s+in\\s*[:：]?\\s*(${street}[\\s\\S]{0,150}?)(?=\\s*${marker}\\b|\\n|$)`, "i"),
    new RegExp(`indirizzo\\s+di\\s+fornitura\\s*[:：]?\\s*(${street}[\\s\\S]{0,150}?)(?=\\s*${marker}\\b|\\n|$)`, "i"),
  ]));
}

function extractAddressBeforeSupplyCode(text, commodity, code) {
  const source = String(text || "");
  const normalizedCode = String(code || "").replace(/[^A-Z0-9]/gi, "");
  if (!normalizedCode) return null;
  const escaped = normalizedCode.split("").map((char) => String.raw`${char}[\s.-]*`).join("");
  ...
  const street = "(?:via|viale|piazza|piazzale|corso|vicolo|largo|strada|contrada|localit[aà])";
  const patterns = [
    new RegExp(`(${street}[\\s\\S]{0,150}?)(?=\\s+(?:codice\\s+cliente[\\s\\S]{0,90}?)?${marker}\\s*[:：]?\\s*${escaped})`, "i"),
    new RegExp(`(${street}[\\s\\S]{0,150}?)\\s+${marker}\\s*[:：]?\\s*${escaped}`, "i"),
  ];
  return normalizeAddress(matchText(source, patterns));
}
...
function normalizeCustomerName(value) {
  ...
  if (/\b(?:unoenergy|energia|fattura|bolletta|mercato|codice|fornitura|totale|pagina|documento|informazioni|pagamenti?|scadenza|servizio\s+clienti|riepilogo|importi)\b/i.test(normalized)) return null;
  const words = normalized.split(/\s+/).filter(Boolean);
  const companyMarker = /\b(?:soc\.?|societ[aà]|s\.?r\.?l\.?|s\.?p\.?a\.?|s\.?n\.?c\.?|s\.?a\.?s\.?|cooperativa|studio)\b/i.test(normalized);
  if (/\bcliente\b/i.test(normalized) && !companyMarker && words.length <= 3) return null;
  return words.length >= 2 && words.length <= 8 ? normalized : null;
}

function customerContextSegments(source, customerHeader = {}) {
  const text = String(source || "");
  const segments = [];
  const markers = /dati\s+(?:identificativi\s+del\s+)?cliente|fornitura\s+e\s+riepilogo\s+degli\s+importi|contratto\s+intestato\s+a|gentile\s+cliente/gi;
  ...
  if (customerHeader.intestatario) {
    ...
    if (index >= 0) segments.push(text.slice(Math.max(0, index - 300), Math.min(text.length, index + 1200)));
  }
...
function extractCustomerTaxId(source, customerHeader = {}) {
  ...
  const customerBlockVat = matchText(source, [
    /dati\s+cliente[\s\S]{0,500}?\b(\d{11})\b[\s\S]{0,120}?\bP\.?\s*Iva\b/i,
  ]);
  if (customerBlockVat) return customerBlockVat.replace(/\D/g, "");
...
function normalizeCustomerCode(value) {
  ...
  if (!compact || compact.length < 6 || compact.length > 20 || !/\d/.test(compact)) return null;
  if (/^(?:IMPORTO|DATA|EMISSIONE|SERVIZIO|FATTURA|CLIENTE)$/i.test(compact)) return null;
  return compact;
}
...
function extractCustomerCode(source, supplyPoints = {}) {
  ...
  const addCandidates = (pattern, score) => {
    for (const match of text.matchAll(pattern)) {
      const value = normalizeCustomerCode(match[1]);
      if (!value) continue;
      const compact = value.replace(/[^A-Z0-9]/g, "");
      if (pdr && /^\d+$/.test(compact) && editDistanceAtMostOne(compact, pdr)) continue;
      if (pod && editDistanceAtMostOne(compact, pod)) continue;
      candidates.push({ value, score, index: match.index ?? Number.MAX_SAFE_INTEGER });
    }
  };

  // Intestazioni esplicite dei fornitori: il numero può essere separato da una
  // breve istruzione oppure da una riga tabellare con altre etichette.
  addCandidates(/codice\s+cliente\s+usalo\s+per\s+comunicare\s+con\s+noi\s*[:：]?\s*(\d{6,20})/gi, 100);
  addCandidates(/codice\s+cliente\s*[:：]?\s*([A-Z0-9]{6,20})/gi, 80);
  addCandidates(/codice\s+cliente\s*[:：]\s*c\.?f\.?\s*\/\s*p\.?iva\s*[:：]?[\s\S]{0,100}?servizio\s*[:：]?\s*(\d{6,20})/gi, 70);
...
function extractCustomerHeaderData(text) {
  ...
  const intestatario = normalizeCustomerName(matchText(source, [
    new RegExp(String.raw`dati\s+identificativi\s+del\s+cliente\s+([A-ZÀ-ÖØ-Ý' .&-]{4,120}?)\s+(?=${street}\b)`, "i"),
    /intestatario\s+fornitura\s*[:：]?\s*([A-ZÀ-ÖØ-Ý' .&-]{4,100})/i,
...
function extractActivationData(text, supplyPoints = {}, segments = null) {
  const source = String(text || "");
  const customerSource = segments?.customer?.text || source;
  const gasSource = scopedCommodityText(segments, "gas", source);
  const luceSource = scopedCommodityText(segments, "luce", source);
  const customerHeader = extractCustomerHeaderData(customerSource);
  const legacyCustomerHeader = extractCustomerHeaderData(source);
  // Merge additivo: il valore già selezionato dal parser Step 1 resta prioritario.
  // Il segmento cliente interviene soltanto quando il documento completo non basta.
  const codiceFiscale = extractCustomerTaxId(source, legacyCustomerHeader)
    || extractCustomerTaxId(customerSource, customerHeader);
  const codiceCliente = extractCustomerCode(source, supplyPoints)
    || extractCustomerCode(customerSource, supplyPoints);
  const indirizzoGas = extractAddressNearSupplyHeader(gasSource, "gas")
    || extractServiceAddress(gasSource, "gas")
    || extractAddressBeforeSupplyCode(gasSource, "gas", supplyPoints.pdr)
    || extractAddressNearSupplyPoint(gasSource, "gas")
    || extractAddressNearKnownSupplyCode(gasSource, supplyPoints.pdr)
    || extractAddressNearSupplyHeader(source, "gas")
    || extractAddressNearSupplyPoint(source, "gas")
    || extractAddressNearKnownSupplyCode(source, supplyPoints.pdr);
  const indirizzoLuce = extractAddressNearSupplyHeader(luceSource, "luce")
    || extractServiceAddress(luceSource, "luce")
    || extractAddressBeforeSupplyCode(luceSource, "luce", supplyPoints.pod)
    || extractAddressNearSupplyPoint(luceSource, "luce")
    || extractAddressNearKnownSupplyCode(luceSource, supplyPoints.pod)
    || extractAddressNearSupplyHeader(source, "luce")
  ...
  const indirizzoFornitura = indirizzoLuce || indirizzoGas || normalizeAddress(fallbackAddress) || legacyCustomerHeader.address || customerHeader.address;
  const customerNamePatterns = [
    /intestatario\s+fornitura\s*[:：]?\s*([A-ZÀ-ÖØ-Ý' .&-]{4,100})/i,
    /contratto\s+intestato\s+a\s*[:：]?\s*([A-ZÀ-ÖØ-Ý' .&-]{4,100})/i,
    /dati\s+identificativi\s+del\s+cliente\s+([A-ZÀ-ÖØ-Ý' .&-]{4,120}?)\s+(?=(?:via|viale|piazza|piazzale|corso|vicolo|largo|strada|contrada|localit[aà])\b)/i,
  ...
  // Il risultato legacy sul documento completo resta prioritario: la segmentazione
  // può aggiungere dati mancanti, ma non deve sostituire un intestatario già valido.
  const intestatario = normalizeCustomerName(matchText(source, customerNamePatterns))
    || normalizeCustomerName(matchText(customerSource, customerNamePatterns))
    || legacyCustomerHeader.intestatario
...
export function buildPdfDiagnostics(normalized, pageTexts = []) {
  ...
  const configs = [
    { key: "fornitore", label: "Fornitore", labels: [/dolomiti\s+energia|eni\s+plenitude|plenitude|edison(?:\s+energia)?|estra(?:\s+energie)?|unoenergy|free\s+luce\s*(?:&|e)\s*gas|hera(?:\s+comm)?|e[.\s-]*on|acea\s+energia|pulsee|illumia|enel\s+energia|butangas/i], required: true },
...
function pdfPageSizeAtScaleOne(pageData) {
  const viewportCandidates = [];
  try {
    viewportCandidates.push(pageData?.getViewport?.({ scale: 1 }));
  } catch {
    // pdf.js recent versions use the object form; older versions use a number.
  }
  try {
    viewportCandidates.push(pageData?.getViewport?.(1));
  } catch {
    // Dimensioni non disponibili: l'OCR userà la scala standard.
  }
  for (const viewport of viewportCandidates) {
...
export function extractPdfDataFromText(text = "") {
  ...
  const segments = segmentPdfText(sourceText);
  const luceSource = scopedCommodityText(segments, "luce", sourceText);
  const gasSource = scopedCommodityText(segments, "gas", sourceText);

  const rawConsumoLuce = annualConsumption(luceSource, "kwh")
    ?? annualConsumption(sourceText, "kwh");
  let rawConsumoGas = annualConsumption(gasSource, "smc")
    ?? annualConsumption(sourceText, "smc");
  const consumoGasMc = matchNumber(gasSource, [
    /consumo\s+annuo\s*\(mc\)\s*[:：]?[\s\S]{0,45}?([\d\s.,]+)/i,
    /consumo\s+annuo\s+mc[\s\S]{0,60}?([\d\s.,]+)(?:\s+fino|\n)/i,
  ]);
  const coefficienteC = matchNumber(gasSource, [/coefficiente(?:\s+correttivo)?\s*\(C\)\s*[:：]?\s*([\d.,]+)/i, /coefficiente\s+C\*?\s*[:：]?\s*([\d.,]+)/i], { preferDecimal: true });
  ...
  const rawPrezzoLuce = kind === "scheda_offerta" ? null : (extractBillPrice(luceSource, "luce") ?? extractBillPrice(sourceText, "luce"));
  const rawPrezzoGas = kind === "scheda_offerta" ? null : (extractBillPrice(gasSource, "gas") ?? extractBillPrice(sourceText, "gas"));
  const fissoLuceMese = kind === "scheda_offerta" ? null : (extractMonthlyFixed(luceSource, "luce") ?? extractMonthlyFixed(sourceText, "luce"));
  const fissoGasMese = kind === "scheda_offerta" ? null : (extractMonthlyFixed(gasSource, "gas") ?? extractMonthlyFixed(sourceText, "gas"));
  const offer = extractOfferData(sourceText, lower);

  const pod = normalizePod(matchText(luceSource, [
  ...
  const pdr = normalizePdr(matchText(gasSource, [
  ...
  const rawPotenzaImpegnata = matchNumber(luceSource, [
  ...
  const rawPotenzaDisponibile = matchNumber(luceSource, [
    /\bimp\.?\s*[\d.,]+\s*kW\s*-?\s*dis\.?\s*([\d.,]+)\s*kW/i,
  ...
  const activation = extractActivationData(sourceText, { pod, pdr }, segments);
  ...
  const normalized = {
    ...
    tipo_prezzo_evidenza: finalizedOffer.tipoPrezzoEvidence,
    ...
    tipo_prezzo_evidenza_luce: finalizedOffer.luce?.typeEvidence ?? null,
    ...
    tipo_prezzo_evidenza_gas: finalizedOffer.gas?.typeEvidence ?? null,
    indice_riferimento_gas: finalizedOffer.gas?.index ?? null,
    spread_gas_eur_smc: finalizedOffer.spreadGas,
    decorrenza_condizioni_economiche: finalizedOffer.validFrom,
    scadenza_condizioni_economiche: finalizedOffer.validTo,
    struttura_prezzo: finalizedOffer.priceStructure,
    periodicita_aggiornamento_indice: finalizedOffer.indexPeriodicity,
    frequenza_fatturazione: finalizedOffer.billingFrequency,
    formula_prezzo: finalizedOffer.formula,
    componenti_prezzo: finalizedOffer.components,
    sconti_offerta: finalizedOffer.discounts,
    valori_sconti_offerta: finalizedOffer.discountValues,
    onere_recesso_anticipato: finalizedOffer.terminationFee,
    codice_condizioni_economiche: finalizedOffer.economicConditionsCode,
    altre_caratteristiche_offerta: finalizedOffer.otherFeatures,
    scadenza_contratto: finalizedOffer.contractExpiry,
    decorrenza_condizioni_economiche_luce: finalizedOffer.luce?.validFrom ?? null,
    scadenza_condizioni_economiche_luce: finalizedOffer.luce?.validTo ?? null,
    struttura_prezzo_luce: finalizedOffer.luce?.priceStructure ?? null,
    periodicita_aggiornamento_indice_luce: finalizedOffer.luce?.indexPeriodicity ?? null,
    frequenza_fatturazione_luce: finalizedOffer.luce?.billingFrequency ?? null,
    formula_prezzo_luce: finalizedOffer.luce?.formula ?? null,
    componenti_prezzo_luce: finalizedOffer.luce?.components ?? [],
    sconti_offerta_luce: finalizedOffer.luce?.discounts ?? [],
    valori_sconti_offerta_luce: finalizedOffer.luce?.discountValues ?? [],
    onere_recesso_anticipato_luce: finalizedOffer.luce?.terminationFee ?? null,
    codice_condizioni_economiche_luce: finalizedOffer.luce?.economicConditionsCode ?? null,
    altre_caratteristiche_offerta_luce: finalizedOffer.luce?.otherFeatures ?? null,
    scadenza_contratto_luce: finalizedOffer.luce?.contractExpiry ?? null,
    decorrenza_condizioni_economiche_gas: finalizedOffer.gas?.validFrom ?? null,
    scadenza_condizioni_economiche_gas: finalizedOffer.gas?.validTo ?? null,
    struttura_prezzo_gas: finalizedOffer.gas?.priceStructure ?? null,
    periodicita_aggiornamento_indice_gas: finalizedOffer.gas?.indexPeriodicity ?? null,
    frequenza_fatturazione_gas: finalizedOffer.gas?.billingFrequency ?? null,
    formula_prezzo_gas: finalizedOffer.gas?.formula ?? null,
    componenti_prezzo_gas: finalizedOffer.gas?.components ?? [],
    sconti_offerta_gas: finalizedOffer.gas?.discounts ?? [],
    valori_sconti_offerta_gas: finalizedOffer.gas?.discountValues ?? [],
    onere_recesso_anticipato_gas: finalizedOffer.gas?.terminationFee ?? null,
    codice_condizioni_economiche_gas: finalizedOffer.gas?.economicConditionsCode ?? null,
    altre_caratteristiche_offerta_gas: finalizedOffer.gas?.otherFeatures ?? null,
    scadenza_contratto_gas: finalizedOffer.gas?.contractExpiry ?? null,
...
export async function extractPdfWithPages(filePath) {
  ...
  const parsed = await pdfParse(buffer, {
    pagerender: async (pageData) => {
      pageSizes.push(pdfPageSizeAtScaleOne(pageData));
  ...
  return { normalized: applyPdfDataContract(normalized), pageTexts, pageSizes };
}
