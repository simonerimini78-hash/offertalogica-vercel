...
function extractActivationData(text) {
  const codiceFiscale = matchText(text, [
    /codice\s+fiscale(?:\s*\/\s*partita\s+iva)?\s*[:：]?\s*([A-Z0-9]{11,16})/i,
  ]);
  const codiceCliente = matchText(text, [
    /codice\s+cliente\s*[:：]\s*([A-Z0-9]{6,20})/i,
    /numero\s+cliente[\s\S]{0,300}?\b([0-9]{8,20})\b/i,
  ]);
  let indirizzoFornitura = matchText(text, [
    /indirizzo\s+di\s+fornitura\s*[:：]\s*([^\n\r]{5,120})/i,
    /indirizzo\s+di\s+fornitura\s+PDR[^\n]*\n\s*([^\n]{5,90}?)\s{2,}\d{14}/i,
    /indirizzo\s+di\s+fornitura\s+POD[^\n]*\n\s*([^\n]{5,90}?)\s{2,}IT/i,
    /servizio\s+fornito\s+in\s*[:：]?[\s\S]{0,120}?\n\s*([^\n]{5,90}?)\s{2,}(?:punto|box|codice)/i,
  ...
  const intestatario = matchText(text, [
    /contratto\s+intestato\s+a\s*[:：]?\s*([A-ZÀ-ÖØ-Ý' -]{4,80})/i,
    /i\s+tuoi\s+dati\s+identificativi[\s\S]{0,100}?\n\s*([A-ZÀ-ÖØ-Ý' -]{4,80})\s*\n/i,
    /intestata\s+a\s+([A-ZÀ-ÖØ-Ý' -]{4,80})/i,
  ]);
  return { codiceFiscale, codiceCliente, indirizzoFornitura, intestatario };
}
...
export function extractPdfDataFromText(text = "") {
  ...
  const pod = normalizePod(matchText(sourceText, [
    ...
    /indirizzo\s+di\s+fornitura\s+POD[^\n]*\n[^\n]*?\b(IT(?:[\s.-]*[A-Z0-9]){12})\b/i,
    /punto\s+di\s+prelievo\s*\(POD\)[\s\S]{0,180}?(IT(?:[\s.-]*[A-Z0-9]){12})/i,
  ]));
  const pdr = normalizePdr(matchText(sourceText, [
    /(?:codice\s+)?PDR\s*[:：]?\s*((?:\d[\s.-]*){14})/i,
    /indirizzo\s+di\s+fornitura\s+PDR[^\n]*\n[^\n]*?\b((?:\d[\s.-]*){14})\b/i,
    /punto\s+di\s+riconsegna\s*\(PDR\)[\s\S]{0,180}?((?:\d[\s.-]*){14})/i,
  ]));
  const rawPotenzaImpegnata = matchNumber(sourceText, [/potenza\s+impegnata\s*[:：]?\s*([\d.,]+)\s*kW/i, /potenza\s+contrattualmente\s+impegnata\s*[:：]?\s*([\d.,]+)\s*kW/i], { preferDecimal: true });
  const rawPotenzaDisponibile = matchNumber(sourceText, [/potenza\s+disponibile\s*[:：]?\s*([\d.,]+)\s*kW/i], { preferDecimal: true });
