...
export const PDF_PARSER_VERSION = "v91-dual-offer-regression-1";
...
function extractOfferSpread(section, commodity) {
  ...
  return matchNumber(section, [
    new RegExp(`(?:energia\\s+)?spread(?:\\s+${index})?\\s*[:=]?\\s*([+-]?[0-9]+[,.][0-9]{3,})\\s*€?\\s*\/?${unit}`, "i"),
    new RegExp(`\\bspread\\b\\s*[:=]?\\s*([+-]?[0-9]+[,.][0-9]{3,})`, "i"),
...
function extractLegacyOfferData(text, lower) {
  ...
  const spreadLuce = likelyLuce ? extractOfferSpread(text, "luce") : null;
  const spreadGas = likelyGas ? extractOfferSpread(text, "gas") : null;
...
function extractOfferData(text, lower) {
  ...
  const entries = matches.map((match, index) => {
    ...
    const spread = commodity ? extractOfferSpread(section, commodity) : null;
    ...
    return { name, code, commodity, type, index: offerIndex, spread, fixedAnnual, score };
  });
  return {
...
function finalizeOfferData(offer, commodity) {
  ...
  if (commodity === "luce" && !luce && fallback.nomeOfferta) {
    luce = { name: fallback.nomeOfferta, code: fallback.codiceOfferta, type: fallback.tipoPrezzo, index: fallback.indice, spread: fallback.spreadLuce, fixedAnnual: fallback.costoFissoAnno };
  }
  if (commodity === "gas" && !gas && fallback.nomeOfferta) {
    gas = { name: fallback.nomeOfferta, code: fallback.codiceOfferta, type: fallback.tipoPrezzo, index: fallback.indice, spread: fallback.spreadGas, fixedAnnual: fallback.costoFissoAnno };
  }
...
function extractActivationData(text) {
  ...
  const indirizzoFornitura = indirizzoLuce || indirizzoGas || normalizeAddress(fallbackAddress);
...
export function buildPdfDiagnostics(normalized, pageTexts = []) {
  ...
  const configs = [
    ...
    { key: "spread_luce_eur_kwh", label: "Spread luce", labels: [/spread|pun[\s\S]{0,80}\+/i], applicable: (n) => hasLuce(n) && (Boolean(n.spread_luce_eur_kwh) || (isOffer(n) && n.tipo_prezzo_luce === "variabile")), required: (n) => isOffer(n) && hasLuce(n) && n.tipo_prezzo_luce === "variabile" },
    ...
    { key: "spread_gas_eur_smc", label: "Spread gas", labels: [/spread|psv[\s\S]{0,80}\+/i], applicable: (n) => hasGas(n) && (Boolean(n.spread_gas_eur_smc) || (isOffer(n) && n.tipo_prezzo_gas === "variabile")), required: (n) => isOffer(n) && hasGas(n) && n.tipo_prezzo_gas === "variabile" },
  ];
...
export function extractPdfDataFromText(text = "") {
  ...
  if (kind === "scheda_offerta") {
    ...
    if (/energia elettrica|kwh|luce/i.test(offerOpening) && Number.isFinite(luceFixedAnnual)) fixedLuceAnnual = luceFixedAnnual;
    if (/gas naturale|smc/i.test(offerOpening) && Number.isFinite(gasFixedAnnual)) fixedGasAnnual = gasFixedAnnual;
  }
