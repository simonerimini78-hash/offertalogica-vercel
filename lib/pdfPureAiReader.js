...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.20-ia-libera";
...
const QUESTION_SPECS = Object.freeze([
  ...
  { id: "prezzo_luce_eur_kwh", field: "prezzo_luce_eur_kwh", kind: "number", question: "Qual è il prezzo unitario confrontabile della sola componente commerciale di vendita o materia energia in EUR/kWh? Nelle condizioni economiche usa il prezzo unico/monorario corrente. Nelle bollette accetta anche il prezzo o costo medio unitario esplicitamente riferito soltanto alla vendita, materia energia o componente energia, purché distinto da rete, trasporto, oneri, imposte e IVA. Non usare il prezzo medio complessivo della quota consumi o della bolletta. Se sono presenti soltanto prezzi F1/F2/F3/F23 senza un prezzo commerciale unico o medio esplicito, usa found=false: la media ponderata sarà calcolata dal sistema solo con consumi annui di fascia compatibili." },
  ...
  { id: "prezzo_gas_eur_smc", field: "prezzo_gas_eur_smc", kind: "number", question: "Qual è il prezzo unitario confrontabile della sola componente commerciale di vendita o materia gas in EUR/Smc? Nelle condizioni economiche usa il prezzo corrente o il riepilogo dei corrispettivi per il consumo definiti dal venditore. Nelle bollette accetta anche il prezzo o costo medio unitario esplicitamente riferito soltanto alla vendita, materia prima gas o componente gas, purché distinto da rete, trasporto, distribuzione, oneri, imposte e IVA. Non usare il prezzo medio complessivo della quota consumi o della bolletta, valori storici, esempi o condizioni di rinnovo futuro." },
...
// La richiesta OpenAI resta una sola, ma non usa più una lista piatta di
// decine di domande obbligatorie. I dati economici necessari al confronto
// sono raggruppati per fornitura e hanno precedenza; gli altri dati sono
// restituiti soltanto quando vengono individuati chiaramente.
const REQUEST_PROFILE = "ia_libera_comparison_form_v1";

const REQUEST_IDENTITY_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["provider", "offer_name", "offer_code", "page", "evidence", "confidence"],
  properties: {
...
const REQUEST_ANNUAL_CONSUMPTION_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["total", "f1", "f2", "f3", "f23", "unit", "page", "label", "evidence", "confidence"],
  properties: {
    total: { type: ["number", "null"] },
    f1: { type: ["number", "null"] },
    f2: { type: ["number", "null"] },
    f3: { type: ["number", "null"] },
    f23: { type: ["number", "null"] },
...
const REQUEST_PRICE_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: [
    "type", "single", "f0", "f1", "f2", "f3", "f23",
    "index", "multiplier", "spread", "formula", "periodicity",
    "unit", "page", "label", "evidence", "confidence",
  ],
  properties: {
    type: { type: "string", enum: ["fixed", "variable", "hybrid", "unknown"] },
    ...
    page: { type: ["integer", "null"], minimum: 1 },
    label: { type: ["string", "null"], maxLength: 180 },
    evidence: { type: ["string", "null"], maxLength: 900 },
    confidence: { type: "integer", minimum: 0, maximum: 100 },
  },
};

const REQUEST_FIXED_FEE_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: [
    "value", "value_text", "unit", "period", "page", "label", "evidence", "confidence",
    "section_total_value", "section_total_value_text", "section_total_unit", "section_total_period",
    "section_total_page", "section_total_label", "section_total_evidence", "section_total_confidence",
  ],
  properties: {
    // Componente commerciale esplicita: vendita, commercializzazione, PCV/CCV/QVD o equivalente.
    value: { type: ["number", "null"] },
    value_text: { type: ["string", "null"], maxLength: 120 },
    unit: { type: ["string", "null"], maxLength: 60 },
    ...
    // Totale netto della sezione QUOTA FISSA, separato dalla componente commerciale.
    // Serve come valore confrontabile soltanto quando la componente commerciale è negativa.
    section_total_value: { type: ["number", "null"] },
    section_total_value_text: { type: ["string", "null"], maxLength: 120 },
    section_total_unit: { type: ["string", "null"], maxLength: 60 },
    section_total_period: { type: "string", enum: ["none", "month", "year"] },
    section_total_page: { type: ["integer", "null"], minimum: 1 },
    section_total_label: { type: ["string", "null"], maxLength: 180 },
    section_total_evidence: { type: ["string", "null"], maxLength: 650 },
    section_total_confidence: { type: "integer", minimum: 0, maximum: 100 },
  },
};
...
const REQUEST_SUPPLY_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["commodity", "identity", "annual_consumption", "price", "fixed_fee"],
  properties: {
    commodity: { type: "string", enum: ["electricity", "gas"] },
    identity: REQUEST_IDENTITY_SCHEMA,
    annual_consumption: REQUEST_ANNUAL_CONSUMPTION_SCHEMA,
    price: REQUEST_PRICE_SCHEMA,
    fixed_fee: REQUEST_FIXED_FEE_SCHEMA,
  },
};

const REQUEST_ADDITIONAL_DATA_SCHEMA = {
  type: "array",
  maxItems: 20,
  items: {
    type: "object",
    ...
    properties: {
      ...
      value_text: { type: ["string", "null"], maxLength: 320 },
...
const REQUEST_OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    supplies: {
      ...
      minItems: 1,
      maxItems: 2,
      items: REQUEST_SUPPLY_SCHEMA,
    },
    additional_data: REQUEST_ADDITIONAL_DATA_SCHEMA,
  },
};

const SYSTEM_PROMPT = `Sei il lettore visuale di OffertaLogica. Devi leggere integralmente bollette e schede luce/gas italiane come farebbe una persona competente e compilare il modulo di confronto con i dati presenti nel PDF.

OBIETTIVO PRINCIPALE: COMPILARE IL MODULO DI CONFRONTO
Per ogni fornitura realmente presente, riempi i campi dello schema supplies usando i valori che leggi nel documento, senza cercare di adattarli a un fornitore o a un'impaginazione specifica.

BLOCCO LUCE
- annual_consumption.total: consumo annuo totale in kWh. Non inserire qui il consumo del mese o del periodo fatturato.
- annual_consumption.f1/f2/f3/f23: consumi annui per fascia, quando presenti.
- price.type: fixed, variable, hybrid oppure unknown.
- price.single: prezzo energia unico/commerciale quando il documento mostra un solo prezzo confrontabile.
- price.f0: prezzo F0 quando è indicato esplicitamente come F0.
- price.f1/f2/f3/f23: prezzi delle fasce corrispondenti quando sono indicati esplicitamente.
- price.index, multiplier, spread, formula, periodicity: riportali quando presenti.
- fixed_fee: quota fissa così come compare nel documento, conservando numero, segno, unità e periodo. Non cambiare il segno. Non convertire mese/anno. Se il documento mostra anche un totale della sezione quota fissa, conservalo separatamente nei campi section_total_*.

BLOCCO GAS
- annual_consumption.total: consumo annuo totale in Smc. Non inserire qui il consumo del mese o del periodo fatturato.
- price.type: fixed, variable, hybrid oppure unknown.
- price.single: prezzo commerciale gas in EUR/Smc.
- price.index, multiplier, spread, formula, periodicity: riportali quando presenti.
- fixed_fee: quota fissa così come compare nel documento, conservando numero, segno, unità e periodo. Non convertire mese/anno.

COME SCEGLIERE LE CASELLE DEL PREZZO LUCE
- Se il documento mostra un prezzo unico/commerciale senza etichetta di fascia, mettilo in price.single.
- Se mostra esplicitamente F0, mettilo in price.f0.
- Se mostra esplicitamente F1, F2 e F3, metti ogni valore nella propria casella.
- La presenza delle parole F1/F2/F3 in una formula vicina non trasforma automaticamente un prezzo generale in prezzo di fascia: usa l'etichetta riferita direttamente al valore.
- Non calcolare medie: il sistema calcolerà dopo una media ponderata, oppure una media aritmetica indicativa quando mancano i consumi per fascia.

REGOLE DI FEDELTÀ
- Copia i dati come sono scritti. Non correggere, arrotondare o cambiare segno.
- Non inventare valori e non ricavare numeri per sottrazione.
- Puoi interpretare liberamente la struttura grafica e la posizione dei dati nel PDF: non esiste una pagina o una sezione obbligatoria.
- Per ogni gruppo riporta page, label ed evidence con la riga o le righe che permettono di controllare il dato.
- Se un campo non è presente, lascialo null. Non inserire un dato diverso solo per riempire la casella.
- Tratta luce e gas separatamente nei documenti dual.

DATI DA CONSERVARE PER L'ATTIVAZIONE SUCCESSIVA
Dopo aver compilato il modulo di confronto, inserisci in additional_data tutti gli altri dati utili che hai già trovato durante la lettura: venditore, nome e codice offerta, POD, PDR, codice cliente, intestatario, codice fiscale o partita IVA, indirizzo di fornitura, potenza, codice contratto, matricola contatore, date, periodicità di fatturazione, contatti, metodo di pagamento o IBAN e altri dati contrattuali visibili.
- Per i dati comuni usa questi nomi quando applicabili: provider, offer_name, offer_code, pod, pdr, customer_name, tax_code, customer_code, supply_address, committed_power_kw, available_power_kw. Per gli altri usa un nome breve e stabile in snake_case.
- Non omettere un dato già individuato.
- Non dedicare tempo aggiuntivo a cercare dati secondari assenti prima di aver completato il modulo di confronto.
- additional_data è un archivio per il passaggio successivo e non deve modificare i valori del modulo di confronto.

OUTPUT
Inserisci in supplies una voce electricity e/o gas soltanto per le forniture presenti. Restituisci esclusivamente JSON conforme allo schema.`;

const USER_PROMPT = `Leggi tutto il PDF. Compila il modulo di confronto supplies con i dati esattamente come li trovi; poi archivia in additional_data gli altri dati utili già letti per l'eventuale attivazione successiva.`;
...
export async function buildPdfPureAiRequest({
  ...
  return {
    ...
    text: {
      format: {
        type: "json_schema",
        name: "offertalogica_priority_economic_reading",
        description: "Lettura prioritaria dei dati economici luce/gas con dati accessori opportunistici",
        strict: true,
...
function localeNumberCandidates(value) {
  const text = String(value ?? "")
    .replace(/\b\d{1,2}[\/.\-]\d{1,2}[\/.\-](?:\d{2}|\d{4})\b/g, " ");
  const matches = text.match(/[-+]?(?:(?:\d{1,3}(?:[.\s]\d{3})+(?:,\d+)?)(?!\d)|\d+(?:[.,]\d+)?)/g) || [];
  return matches
    .map((token) => ({ token: compact(token, 60), value: parseLocaleNumber(token) }))
    .filter((item) => Number.isFinite(item.value));
}

function fixedFeeNumberFromEvidence(declaredValue, evidence) {
  if (!Number.isFinite(declaredValue)) return null;
  const candidates = localeNumberCandidates(evidence);
  if (!candidates.length) return { value: declaredValue, token: null, reconciled: false };

  const tolerance = Math.max(0.25, Math.abs(declaredValue) * 0.05);
  const ranked = candidates
    .map((candidate) => ({ ...candidate, distance: Math.abs(candidate.value - declaredValue) }))
    .filter((candidate) => candidate.distance <= tolerance)
    .sort((a, b) => a.distance - b.distance || b.token.length - a.token.length);

  if (!ranked.length) return null;
  const best = ranked[0];
  const equallyCloseDifferent = ranked.some((candidate, index) => index > 0
    && Math.abs(candidate.distance - best.distance) < 1e-9
    && Math.abs(candidate.value - best.value) > 1e-9);
  if (equallyCloseDifferent) return null;
  return {
    value: best.value,
    token: best.token,
    reconciled: Math.abs(best.value - declaredValue) > 1e-9,
  };
}
...
function hasCommercialSalesComponent(text) {
  const normalized = semanticText(text, 1800);
  if (!normalized) return false;
  const generalSupplyAverage = /\b(?:costo|prezzo)\s+medi[oa](?:\s+unitari[oa])?\s+(?:della|del)\s+fornitura\b|\bprezzo\s+medio\s+(?:totale|complessivo)\b/.test(normalized);
  if (generalSupplyAverage) return false;
  return /\b(?:vendita\s+(?:di\s+)?(?:energia(?:\s+elettrica)?|gas(?:\s+naturale)?)|materia\s+(?:prima\s+)?(?:energia|gas)|componente\s+(?:di\s+)?(?:vendita|energia|materia|gas)|quota\s+(?:materia|energia)|corrispettiv[oi]\s+(?:per\s+)?(?:il\s+)?consumo\s+definit[oi]\s+dal\s+venditore|costo\s+per\s+consumi\s+definit[oi]\s+dal\s+venditore)\b/.test(normalized);
}

function inferSourceRole(answer) {
  const text = answerSemanticText(answer);
  if (!text) return "unknown";
  const saleSpecificAverage = (/\b(costo|prezzo|spesa)\s+medi[oa]\b|\bmedi[oa]\s+unitari[oa]\b|\bcosto\s+unitario\b/.test(text))
    && hasCommercialSalesComponent(text)
  ...
  // Una riga può appartenere al riepilogo del periodo e contenere comunque il
  // prezzo commerciale confrontabile. La natura commerciale prevale sulla
  // sola presenza di riferimenti temporali, mentre restano attivi i filtri
  // successivi contro costo medio complessivo, rete, oneri, imposte ed esempi.
  if (hasCommercialSalesComponent(text)
      && (/\b(?:kwh|smc)\b/.test(text) || /\bformula\s+prevista\b/.test(text))) return "contract_term";
  ...
  if (/\b(consum[oi]\s+annu(?:o|i|ale|ali)|in\s+un\s+anno|ultim[oi]\s+12\s+mesi|dodici\s+mesi|12\s+mesi)\b/.test(text)) return "annual_total";
  if (
...
function semanticValidation(spec, answer, document = {}) {
  ...
  if (LIGHT_BAND_CONSUMPTION_FIELDS.includes(spec.field)) {
    const explicitAnnual = /\b(consum[oi]\s+annu(?:o|i|ale|ali)|in\s+un\s+anno|ultim[oi]\s+12\s+mesi|12\s+mesi|dodici\s+mesi)\b/.test(text);
  ...
  if (["consumo_luce_kwh", "consumo_gas_smc"].includes(spec.field)) {
    const explicitAnnual = /\b(consum[oi]\s+annu(?:o|i|ale|ali)|in\s+un\s+anno|annu(?:o|ale|alizzat[oa])|ultim[oi]\s+12\s+mesi|12\s+mesi|dodici\s+mesi)\b/.test(text);
    const contradictoryPeriod = /\b(da\s+inizio\s+fornitura|consum[oi]\s+fatturat[oi]|periodo\s+fatturat|nel\s+periodo|mese\s+di|bimestre|trimestre|consuntivo)\b/.test(text);
  ...
  if (["prezzo_luce_eur_kwh", ...LIGHT_BAND_PRICE_FIELDS, "prezzo_gas_eur_smc"].includes(spec.field)) {
    ...
    const saleSpecificAverage = hasCommercialSalesComponent(text) && expectedUnit;
...
function normalizeAnswer(spec, answer, document = {}) {
  ...
  const semantic = answer.form_entry === true
    ? {
        accepted: true,
        role: LIGHT_BAND_CONSUMPTION_FIELDS.includes(spec.field) || ["consumo_luce_kwh", "consumo_gas_smc"].includes(spec.field)
          ? "annual_total"
          : comparisonEconomicField(spec.field) ? "contract_term" : sourceRole(answer),
        certainty: "certain",
        usable: true,
      }
    : semanticValidation(spec, answer, document);
  if (!semantic.accepted) return semantic;
  if (answer.form_entry === true && ["consumo_luce_kwh", ...LIGHT_BAND_CONSUMPTION_FIELDS, "consumo_gas_smc"].includes(spec.field)) {
    const fieldEvidence = semanticText([answer?.evidence, answer?.value_text, answer?.unit].filter(Boolean).join(" | "));
    const annualCue = /\b(consum[oi]\s+annu(?:o|i|ale|ali)|in\s+un\s+anno|ultim[oi]\s+12\s+mesi|12\s+mesi|dodici\s+mesi)\b/.test(fieldEvidence);
    if (!annualCue) return { accepted: false, reason: "form_consumption_not_annual" };
  }
...
function fixedFeeCandidate({
  ...
  const normalizedPeriod = ["month", "year"].includes(period) ? period : "none";
  const evidenceNumber = numeric === null ? null : fixedFeeNumberFromEvidence(numeric, evidence);
  const resolvedNumeric = evidenceNumber?.value ?? null;
  const valid = resolvedNumeric !== null && normalizedPeriod !== "none";
  const normalizedUnit = valid ? (compact(unit, 60) || null) : null;
  const normalizedValueText = valid
    ? (evidenceNumber?.reconciled && evidenceNumber.token
      ? compact(`${evidenceNumber.token}${normalizedUnit ? ` ${normalizedUnit}` : ""}`, 120)
      : (compact(valueText, 120) || String(resolvedNumeric)))
    : null;
  return {
    value: valid ? resolvedNumeric : null,
    value_text: normalizedValueText,
    unit: normalizedUnit,
    period: valid ? normalizedPeriod : "none",
    page: valid && Number.isInteger(Number(page)) && Number(page) > 0 ? Number(page) : null,
    label: valid ? (compact(label, 180) || null) : null,
    evidence: valid ? (compact(evidence, 650) || null) : null,
    confidence: valid ? Math.max(0, Math.min(100, Number(confidence || 0))) : 0,
  };
}

function compactFixedFeeDetails(fixed = {}) {
  const commercial = fixedFeeCandidate({
    value: fixed.value,
    valueText: fixed.value_text,
    unit: fixed.unit,
    period: fixed.period,
    page: fixed.page,
    label: fixed.label,
  ...
  const sectionTotal = fixedFeeCandidate({
    value: fixed.section_total_value,
    valueText: fixed.section_total_value_text,
    unit: fixed.section_total_unit,
    period: fixed.section_total_period,
    page: fixed.section_total_page,
    label: fixed.section_total_label,
    evidence: fixed.section_total_evidence,
    confidence: fixed.section_total_confidence,
  });

  let selectedForComparison = null;
  if (commercial.value !== null) selectedForComparison = "commercial_component";
  else if (sectionTotal.value !== null) selectedForComparison = "section_total";

  return {
    commercial_component: commercial,
    section_total: sectionTotal,
    selected_for_comparison: selectedForComparison,
  };
}

function selectedFixedFeeCandidate(fixed = {}) {
  const details = compactFixedFeeDetails(fixed);
  if (details.selected_for_comparison === "commercial_component") {
    return { ...details.commercial_component, source: "commercial_component", details };
  }
  if (details.selected_for_comparison === "section_total") {
    return { ...details.section_total, source: "section_total", details };
  }
  return { value: null, period: "none", source: null, details };
}

function compactSupplyAnswers(supply, commodity) {
  if (!supply || typeof supply !== "object" || supply.present === false) return [];
  const answers = [];
  ...
  if (identity.offer_code) {
    answers.push(compactAnswer(`codice_offerta_${prefix}`, {
      valueText: compact(identity.offer_code, 160), page: identity.page, label: "Codice offerta",
      evidence: identityEvidence, confidence: identity.confidence, sourceRole: "contract_term",
    }));
  }
  ...
  const fixed = selectedFixedFeeCandidate(supply.fixed_fee || {});
  if (fixed.value !== null && ["month", "year"].includes(fixed.period)) {
    answers.push(compactAnswer(`quota_fissa_vendita_${prefix}`, {
      valueNumber: fixed.value,
      valueText: fixed.value_text || String(fixed.value),
      unit: fixed.unit,
      period: fixed.period,
      page: fixed.page,
      label: fixed.label || (fixed.source === "section_total" ? "Totale quota fissa" : "Quota fissa commerciale"),
      evidence: fixed.evidence,
      confidence: fixed.confidence,
      sourceRole: "contract_term",
    }));
  }
  return answers;
}

function additionalDataQuestionId(item) {
  const commodity = item?.commodity;
  switch (item?.field) {
    case "provider":
      return commodity === "electricity" ? "fornitore_luce" : commodity === "gas" ? "fornitore_gas" : "fornitore";
    case "offer_name":
      return commodity === "electricity" ? "nome_offerta_luce" : commodity === "gas" ? "nome_offerta_gas" : null;
    case "offer_code":
      return commodity === "electricity" ? "codice_offerta_luce" : commodity === "gas" ? "codice_offerta_gas" : null;
    case "pod":
      return "pod";
    case "pdr":
      return "pdr";
    ...
    case "customer_code":
      return commodity === "electricity" ? "codice_cliente_luce" : commodity === "gas" ? "codice_cliente_gas" : "codice_cliente";
    case "supply_address":
      return commodity === "electricity" ? "indirizzo_fornitura_luce" : commodity === "gas" ? "indirizzo_fornitura_gas" : null;
    case "committed_power_kw":
      return "potenza_impegnata_kw";
    case "available_power_kw":
      return "potenza_disponibile_kw";
    default:
      return null;
  }
}

function additionalDataAnswers(items) {
  if (!Array.isArray(items)) return [];
  const answers = [];
  for (const item of items) {
    const questionId = additionalDataQuestionId(item);
    const spec = QUESTION_BY_ID.get(questionId);
    if (!spec) continue;
    const numeric = finiteNumberOrNull(item?.value_number);
    const textValue = compact(item?.value_text, 320) || (numeric !== null ? String(numeric) : null);
    if (textValue === null && numeric === null) continue;
    const sourceRole = ["provider"].includes(item.field)
      ? "identity"
      : ["offer_name", "offer_code"].includes(item.field)
        ? "contract_term"
        : "customer_data";
    answers.push(compactAnswer(questionId, {
      valueText: textValue,
      valueNumber: ["number", "fixed"].includes(spec.kind) ? numeric : null,
      unit: compact(item?.unit, 60) || null,
      page: Number.isInteger(Number(item?.page)) && Number(item.page) > 0 ? Number(item.page) : null,
      label: compact(item?.label, 180) || questionId,
      evidence: compact(item?.evidence, 500) || textValue,
...
function compactOutputToLegacy(parsed) {
  ...
  const supplies = Array.isArray(parsed.supplies) ? parsed.supplies : [];
  const electricity = supplies.find((supply) => supply?.commodity === "electricity") || parsed.electricity || null;
  const gas = supplies.find((supply) => supply?.commodity === "gas") || parsed.gas || null;
  if (!electricity && !gas) return parsed;

  const electricityAnswers = compactSupplyAnswers(electricity, "luce");
  const gasAnswers = compactSupplyAnswers(gas, "gas");
  const accessoryAnswers = additionalDataAnswers(parsed.additional_data);
  const providers = [electricity?.identity?.provider, gas?.identity?.provider].filter(Boolean);
  const combined = [...electricityAnswers, ...gasAnswers, ...accessoryAnswers];
  if (providers.length && !combined.some((answer) => answer.question_id === "fornitore")) {
    combined.unshift(compactAnswer("fornitore", {
      valueText: compact(providers[0], 300), page: electricity?.identity?.page || gas?.identity?.page || null,
      label: "Venditore", evidence: compact(electricity?.identity?.evidence || gas?.identity?.evidence, 450) || compact(providers[0], 300),
      confidence: Math.max(Number(electricity?.identity?.confidence || 0), Number(gas?.identity?.confidence || 0)), sourceRole: "identity",
    }));
  }
  const seen = new Set();
  const answers = combined.filter((answer) => {
    if (!answer?.question_id || seen.has(answer.question_id)) return false;
  ...
  return {
    ...
    answers,
    fixed_fee_details: {
      luce: electricity ? compactFixedFeeDetails(electricity.fixed_fee || {}) : null,
      gas: gas ? compactFixedFeeDetails(gas.fixed_fee || {}) : null,
    },
    comparison_form_raw: { electricity, gas },
    additional_data_archive: Array.isArray(parsed.additional_data) ? parsed.additional_data : [],
  };
}
...
export function normalizePureAiOutput(parsed, {
  ...
  const normalized = {
    ...
    textExtracted: 0,
    needsReview: true,
    quota_fissa_dettaglio_luce: parsed.fixed_fee_details?.luce || null,
    quota_fissa_dettaglio_gas: parsed.fixed_fee_details?.gas || null,
    comparison_form_raw: parsed.comparison_form_raw || null,
    activation_data_archive: Array.isArray(parsed.additional_data_archive) ? parsed.additional_data_archive : [],
  };
  const filledFields = [];
  const rejected = [];
  for (const spec of QUESTION_SPECS) {
    ...
    if (!result.accepted) {
      if (result.raw_fixed && spec.kind === "fixed") {
        const commodityKey = spec.field.includes("_luce_") ? "luce" : "gas";
        const detailField = `quota_fissa_dettaglio_${commodityKey}`;
        const existing = normalized[detailField] || {
          commercial_component: null,
          section_total: null,
          selected_for_comparison: null,
        };
        normalized[detailField] = {
          ...existing,
          commercial_component: result.raw_fixed,
          selected_for_comparison: existing.selected_for_comparison || null,
        };
      }
  ...
  const f1Price = finiteNumberOrNull(normalized.prezzo_luce_f1_eur_kwh);
  const f2Price = finiteNumberOrNull(normalized.prezzo_luce_f2_eur_kwh);
  const f3Price = finiteNumberOrNull(normalized.prezzo_luce_f3_eur_kwh);
  const f23Price = finiteNumberOrNull(normalized.prezzo_luce_f23_eur_kwh);
  const normalizedF23Consumption = finiteNumberOrNull(normalized.consumo_luce_f23_kwh);
  let comparableLightPrice = null;
  let comparableLightDiagnostic = null;
  const explicitComparableLightPrice = finiteNumberOrNull(normalized.prezzo_luce_eur_kwh);

  if (explicitComparableLightPrice !== null && explicitComparableLightPrice > 0) {
    comparableLightPrice = explicitComparableLightPrice;
  } else if (validatedF0 !== null && validatedF0 > 0) {
    comparableLightPrice = validatedF0;
    const f0Diagnostic = normalized.diagnostics.find((item) => item.field === "prezzo_luce_f0_eur_kwh");
    comparableLightDiagnostic = {
      ...(f0Diagnostic || {}),
      field: "prezzo_luce_eur_kwh",
      label: `${f0Diagnostic?.label || "F0"} (prezzo confrontabile monorario)`,
      value: comparableLightPrice,
      status: "review",
      confidence: Number(f0Diagnostic?.confidence || 100),
      source_snippet: f0Diagnostic?.source_snippet || "Prezzo F0 validato",
      source_match: f0Diagnostic?.source_match || null,
      source_role: f0Diagnostic?.source_role || "contract_term",
      certainty: "derived",
      usable_for_comparison: true,
      method: "deterministic_comparable_price",
      source_version: PDF_PURE_AI_READER_VERSION,
      derivation: {
        type: "monoraria_f0_to_sales_price",
        source_field: "prezzo_luce_f0_eur_kwh",
        derived_value: comparableLightPrice,
      },
    };
  } else {
    const completeF123 = [f1Price, f2Price, f3Price].every((value) => value !== null && value > 0)
      && [f1Consumption, f2Consumption, f3Consumption].every((value) => value !== null && value >= 0);
    const totalF123 = completeF123 ? f1Consumption + f2Consumption + f3Consumption : 0;
    const completeF1F23 = [f1Price, f23Price].every((value) => value !== null && value > 0)
      && [f1Consumption, normalizedF23Consumption].every((value) => value !== null && value >= 0);
    const totalF1F23 = completeF1F23 ? f1Consumption + normalizedF23Consumption : 0;

    if (completeF123 && totalF123 > 0) {
      comparableLightPrice = Number(((f1Price * f1Consumption + f2Price * f2Consumption + f3Price * f3Consumption) / totalF123).toFixed(6));
      comparableLightDiagnostic = {
        field: "prezzo_luce_eur_kwh", label: "Prezzo confrontabile ponderato F1/F2/F3", value: comparableLightPrice,
        status: "review", confidence: 100, page: null,
        source_snippet: "Media ponderata deterministica dei prezzi F1/F2/F3 sui consumi annui delle stesse fasce già validati",
        source_match: null, source_role: "sales_component_rate", certainty: "derived", usable_for_comparison: true,
        verification_reason: null, coverage_months: 12, method: "deterministic_weighted_average", source_version: PDF_PURE_AI_READER_VERSION,
        derivation: {
          type: "annual_band_weighted_sales_price_f1_f2_f3",
          price_fields: ["prezzo_luce_f1_eur_kwh", "prezzo_luce_f2_eur_kwh", "prezzo_luce_f3_eur_kwh"],
          consumption_fields: ["consumo_luce_f1_kwh", "consumo_luce_f2_kwh", "consumo_luce_f3_kwh"],
          total_consumption: totalF123,
          derived_value: comparableLightPrice,
        },
      };
    } else if (completeF1F23 && totalF1F23 > 0) {
      comparableLightPrice = Number(((f1Price * f1Consumption + f23Price * normalizedF23Consumption) / totalF1F23).toFixed(6));
      comparableLightDiagnostic = {
        field: "prezzo_luce_eur_kwh", label: "Prezzo confrontabile ponderato F1/F23", value: comparableLightPrice,
        status: "review", confidence: 100, page: null,
        source_snippet: "Media ponderata deterministica dei prezzi F1/F23 sui consumi annui delle stesse fasce già validati",
        source_match: null, source_role: "sales_component_rate", certainty: "derived", usable_for_comparison: true,
        verification_reason: null, coverage_months: 12, method: "deterministic_weighted_average", source_version: PDF_PURE_AI_READER_VERSION,
        derivation: {
          type: "annual_band_weighted_sales_price_f1_f23",
          price_fields: ["prezzo_luce_f1_eur_kwh", "prezzo_luce_f23_eur_kwh"],
          consumption_fields: ["consumo_luce_f1_kwh", "consumo_luce_f23_kwh"],
          total_consumption: totalF1F23,
          derived_value: comparableLightPrice,
        },
      };
    } else if ([f1Price, f2Price, f3Price].every((value) => value !== null && value > 0)) {
      comparableLightPrice = Number(((f1Price + f2Price + f3Price) / 3).toFixed(6));
      comparableLightDiagnostic = {
        field: "prezzo_luce_eur_kwh", label: "Prezzo indicativo medio F1/F2/F3", value: comparableLightPrice,
        status: "review", confidence: 85, page: null,
        source_snippet: "Media aritmetica deterministica dei prezzi F1/F2/F3 letti nel modulo; usata soltanto perché i consumi annui per fascia non sono disponibili",
        source_match: null, source_role: "sales_component_rate", certainty: "derived", usable_for_comparison: true,
        verification_reason: "stima_media_semplice_fasce", coverage_months: null, method: "deterministic_arithmetic_average", source_version: PDF_PURE_AI_READER_VERSION,
        derivation: {
          type: "arithmetic_average_sales_price_f1_f2_f3",
          price_fields: ["prezzo_luce_f1_eur_kwh", "prezzo_luce_f2_eur_kwh", "prezzo_luce_f3_eur_kwh"],
          divisor: 3,
          derived_value: comparableLightPrice,
          estimated: true,
  ...
  if (comparableLightPrice !== null && comparableLightPrice > 0) {
    normalized.prezzo_luce_eur_kwh = comparableLightPrice;
    if (!filledFields.includes("prezzo_luce_eur_kwh")) filledFields.push("prezzo_luce_eur_kwh");
    normalized.diagnostics = normalized.diagnostics.filter((item) => item.field !== "prezzo_luce_eur_kwh");
    if (comparableLightDiagnostic) normalized.diagnostics.push(comparableLightDiagnostic);
  ...
  normalized.ai = {
    ...
    verification_protocol: "ia_libera_comparison_form_v1",
    activation_archive_count: normalized.activation_data_archive.length,
  };
