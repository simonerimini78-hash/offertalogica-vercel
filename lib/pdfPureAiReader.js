...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v2.0.11-light-period-guard";
...
export const PDF_PURE_AI_QUESTION_IDS = Object.freeze([
  ...
  "residenza_luce",
  "consumi_mensili_luce_kwh",
  "classe_contatore_gas",
  "potere_calorifico_gas_gj_smc",
  "ambito_tariffario_gas",
  "regime_accisa_gas",
  "regime_arisgam_gas"
]);
export const PDF_PURE_AI_REQUEST_QUESTION_IDS = Object.freeze([]);

const REQUEST_PROFILE = "ia_libera_compact_form_v3_7_technical";
const FIELD_PURPOSES = Object.freeze([
  ...
  "power_committed", "power_available", "periodicity", "conditions_start", "conditions_end",
  "residence_status", "monthly_consumption", "meter_class", "calorific_value",
  "gas_tariff_area", "gas_excise_regime", "gas_regional_tax_profile", "other",
]);
...
- price_component: restituisci tutte le componenti unitarie necessarie a ricostruire il prezzo commerciale e a separare la sola materia confrontabile. In particolare non omettere il corrispettivo materia/consumo energia o gas, spread, C.DISP.D./dispacciamento o componenti analoghe del venditore, sconti/riduzioni unitari, perdite e componenti legate a PCS/potere calorifico quando sono esplicite. Il coefficiente correttivo C del gas serve invece alla conversione dei volumi mc→Smc e non è una componente del prezzo. Mantieni sempre l'etichetta originale e il segno del valore;
...
- formula: espressione o testo che descrive come si forma il prezzo; copia anche i riferimenti a sconti, C.DISP.D., perdite e PCS/potere calorifico quando fanno parte della formula economica. Non inserire il coefficiente correttivo C nella formula prezzo quando serve soltanto a convertire mc in Smc;
- index, spread, multiplier, periodicity. Usa multiplier per un moltiplicatore numerico della componente indice/materia, compreso un fattore PCS/potere calorifico quando appartiene esplicitamente alla formula economica. Per il gas usa inoltre una riga multiplier separata e con etichetta originale quando è esplicito il coefficiente correttivo C che converte mc in Smc: il codice lo userà solo sui consumi, mai sul prezzo;
...
Per una BOLLETTA COMPLETA cerca inoltre, senza inventare e senza dedurre da dati personali non necessari:
- residence_status (solo luce): se il documento dichiara esplicitamente domestico residente/non residente, usa value_text esattamente "residente" oppure "non-residente";
- monthly_consumption (solo luce): consumi dei singoli mesi se il PDF riporta uno storico di 12 mesi. Una riga per mese, value_number in kWh, period month; nel label usa YYYY-MM quando l'anno e leggibile oppure il nome del mese. Non ricavare i mesi dividendo il consumo annuo;
- meter_class (solo gas): classe del misuratore/contatore, per esempio G4, G6, G10;
- calorific_value (solo gas): potere calorifico superiore/convenzionale P o PCS espresso in GJ/Smc. Non confonderlo con il coefficiente correttivo C;
- gas_tariff_area (solo gas): soltanto se il documento indica esplicitamente l'ambito tariffario ARERA; usa uno fra nord-occidentale, nord-orientale, centrale, centro-sud-orientale, centro-sud-occidentale, meridionale, sardegna;
- gas_excise_regime (solo gas): soltanto se il documento indica esplicitamente il regime fiscale/accisa; usa "ordinario" oppure "mezzogiorno-dpr218";
- gas_regional_tax_profile (solo gas): soltanto se il documento rende esplicito il profilo/addizionale regionale applicato e coincide con uno dei profili supportati dal calcolatore.
...
- non usare consumi del solo periodo come consumi annui; estraili separatamente come period_consumption quando il totale del periodo è esplicito; per il gas conserva l'unità originale: Smc resta Smc, mentre mc/m3 richiede il coefficiente correttivo C per la conversione deterministica in Smc;
...
- massimo 24 righe per fornitura; usa le righe tecniche soltanto quando esplicite e prioritarie per il confronto; per lo storico mensile inserisci i 12 mesi solo se il documento li mostra chiaramente;
- restituisci esclusivamente JSON conforme allo schema.`;

const USER_PROMPT = `Compila il modulo libero con importo totale, periodo, date, consumo annuo e, se disponibile, consumo totale del periodo fatturato, prezzo finale se esplicito, fasce, componenti, formula, quota fissa e, quando esplicitamente indicate, decorrenza e scadenza delle condizioni economiche. Se e una bolletta completa cerca anche residenza luce, storico mensile luce e dati tecnici gas P/PCS, classe misuratore, ambito e regimi fiscali solo quando scritti esplicitamente. Mantieni la struttura tariffaria originale senza produrre spiegazioni.`;
...
function normalizeResidenceStatus(value) {
  const text = compact(value, 120).toLowerCase();
  if (/non[\s-]*residente|seconda\s+casa/.test(text)) return "non-residente";
  if (/\bresidente\b/.test(text)) return "residente";
  return null;
}

function normalizeGasMeterClass(value) {
  const text = compact(value, 80).toLowerCase().replace(/\s+/g, "");
  const match = text.match(/g(\d{1,3})/i);
  const size = match ? Number(match[1]) : NaN;
  if (!Number.isFinite(size)) return null;
  if (size <= 6) return "g4-g6";
  if (size <= 40) return "g10-g40";
  return "oltre-g40";
}

function normalizeGasTariffArea(value) {
  const text = compact(value, 160).toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "");
  const map = [
    ["nord-occidentale", /nord[\s-]*occidentale/],
    ["nord-orientale", /nord[\s-]*orientale/],
    ["centro-sud-orientale", /centro[\s-]*sud[\s-]*orientale/],
    ["centro-sud-occidentale", /centro[\s-]*sud[\s-]*occidentale/],
    ["meridionale", /\bmeridionale\b/],
    ["sardegna", /\bsardegna\b/],
    ["centrale", /\bcentrale\b/],
  ];
  return map.find(([, pattern]) => pattern.test(text))?.[0] || null;
}

function normalizeGasExciseRegime(value) {
  const text = compact(value, 180).toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "");
  if (/dpr\s*218|mezzogiorno|agevolat/.test(text)) return "mezzogiorno-dpr218";
  if (/ordinari/.test(text)) return "ordinario";
  return null;
}

function normalizeGasRegionalTaxProfile(value) {
  const text = compact(value, 180).toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "");
  const exact = [
    "abruzzo-ordinario", "abruzzo-clima-ef", "basilicata", "campania", "emilia-romagna",
    "lazio-centro-nord", "lazio-mezzogiorno", "liguria-cd", "liguria-e", "liguria-f",
    "lombardia", "marche", "piemonte-ordinario", "puglia", "toscana", "umbria", "veneto",
  ];
  const canonical = text.replace(/\s+/g, "-");
  if (exact.includes(canonical)) return canonical;
  if (/emilia.*romagna/.test(text)) return "emilia-romagna";
  if (/lombardia/.test(text)) return "lombardia";
  if (/basilicata/.test(text)) return "basilicata";
  if (/campania/.test(text)) return "campania";
  if (/marche/.test(text)) return "marche";
  if (/puglia/.test(text)) return "puglia";
  if (/toscana/.test(text)) return "toscana";
  if (/umbria/.test(text)) return "umbria";
  if (/veneto/.test(text)) return "veneto";
  if (/piemonte/.test(text) && /ordinari/.test(text)) return "piemonte-ordinario";
  if (/abruzzo/.test(text) && /(?:zona\s*)?[ef]\b|clima.*[ef]/.test(text)) return "abruzzo-clima-ef";
  if (/abruzzo/.test(text) && /ordinari/.test(text)) return "abruzzo-ordinario";
  if (/lazio/.test(text) && /mezzogiorno|dpr\s*218/.test(text)) return "lazio-mezzogiorno";
  if (/lazio/.test(text) && /centro.*nord/.test(text)) return "lazio-centro-nord";
  if (/liguria/.test(text) && /zona.*f\b/.test(text)) return "liguria-f";
  if (/liguria/.test(text) && /zona.*e\b/.test(text)) return "liguria-e";
  if (/liguria/.test(text) && /zona.*[cd]\b/.test(text)) return "liguria-cd";
  return null;
}

const MONTH_NAMES_IT = ["gennaio", "febbraio", "marzo", "aprile", "maggio", "giugno", "luglio", "agosto", "settembre", "ottobre", "novembre", "dicembre"];
function monthIndexFromRow(row) {
  const text = compact([row?.label, row?.value_text, row?.evidence].filter(Boolean).join(" "), 600).toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "");
  const iso = text.match(/(?:20\d{2})[-\/.](0?[1-9]|1[0-2])\b/);
  if (iso) return Number(iso[1]) - 1;
  for (let index = 0; index < MONTH_NAMES_IT.length; index += 1) {
    const full = MONTH_NAMES_IT[index];
    const short = full.slice(0, 3);
    if (new RegExp(`\\b(?:${full}|${short})\\b`).test(text)) return index;
  }
  const explicit = text.match(/(?:mese|month)\s*(0?[1-9]|1[0-2])\b/);
  return explicit ? Number(explicit[1]) - 1 : null;
}

function inclusiveIsoDays(startValue, endValue) {
  const start = normalizeIsoDate(startValue);
  const end = normalizeIsoDate(endValue);
...
function sameConsumptionValue(left, right) {
  const a = finite(left);
...
function suppressPeriodConsumptionMisreadAsAnnual(normalized) {
  const coverageDays = inclusiveIsoDays(normalized?.billing_period_start, normalized?.billing_period_end);
  if (!coverageDays || coverageDays >= 330) return [];
  const suppressed = [];
  for (const [annualField, periodField] of [
    ["consumo_luce_kwh", "consumo_periodo_luce_kwh"],
    ["consumo_gas_smc", "consumo_periodo_gas_smc"],
  ]) {
    if (!sameConsumptionValue(normalized?.[annualField], normalized?.[periodField])) continue;
    delete normalized[annualField];
    suppressed.push(annualField);
  }
  return suppressed;
}
...
const ESSENTIAL_RECOVERY_PROFILE = "ia_libera_essential_recovery_v2_price_components";
...
- period_consumption: solo il consumo totale dell’intero periodo fatturato della fornitura; non annualizzarlo. Per luce usa kWh. Per gas conserva l'unità realmente mostrata: Smc se già fatturato in Smc; se il documento mostra solo mc/m3, restituisci il valore in mc/m3 e, quando presente, restituisci anche il coefficiente correttivo C come riga multiplier con la sua etichetta originale;
- unit_price: prezzo unitario della vendita/materia. Se esiste solo un prezzo medio/aggregato della vendita, puoi restituirlo come controllo aritmetico, ma NON fermarti a quel dato: restituisci anche come price_component tutte le componenti unitarie esplicite necessarie a isolare la sola materia confrontabile. Per il gas cerca in particolare prezzo all’ingrosso/PSV o corrispettivo materia gas, spread/corrispettivo commerciale, eventuali sconti e fattore PCS quando appartiene alla formula economica;
- fixed_fee: quota fissa della sola vendita/commercializzazione, con segno e periodicità originali.

Regole vincolanti:
- non usare il prezzo medio totale della quota per consumi come prezzo materia confrontabile; se compare, trattalo soltanto come controllo e recupera le componenti sottostanti;
- quando unit_price è tra i dati richiesti, puoi restituire anche price_component, spread, index, multiplier, formula e price_type della stessa fornitura se servono a ricostruire la sola materia;
- non includere rete, trasporto, distribuzione, oneri, imposte, IVA, accise o quota potenza nella materia confrontabile; se sono esposte come componenti unitarie puoi restituirle come price_component con etichetta originale, così il codice le escluderà deterministicamente;
...
function gasVolumeUnit(row) {
  ...
  if (/\bsmc\b|standard\s*metr[oi]\s*cub/i.test(text)) return "smc";
  if (/\bmc\b|\bm3\b|m³|metr[oi]\s*cub/i.test(text)) return "mc";
  return null;
}

function gasConversionCoefficientCandidate(row) {
  const value = rowNumber(row);
  if (value === null || value <= 0) return false;
  const text = essentialRowText(row, true);
  if (/pcs|potere\s+calorifico/.test(text)) return false;
  return /coeff(?:iciente)?\.?\s*(?:correttivo|di\s+conversione)?\s*c\b|coefficiente\s+c\b|conversione\s+(?:da\s+)?mc\s*(?:a|in|→|->)\s*smc/.test(text);
}

function gasConversionCoefficientRow(rows = []) {
  return [...rows]
    .filter(gasConversionCoefficientCandidate)
    .sort((a, b) => Number(b?.purpose === "multiplier") - Number(a?.purpose === "multiplier") || Number(a?.page || 9999) - Number(b?.page || 9999))[0] || null;
}

function gasPcsPriceMultiplierRow(rows = []) {
  const multipliers = rows.filter((row) => row?.purpose === "multiplier" && rowNumber(row) > 0 && !gasConversionCoefficientCandidate(row));
  const explicit = multipliers.find((row) => /pcs|potere\s+calorifico/.test(essentialRowText(row, true)));
  if (explicit) return explicit;
  const formula = comparisonCanonicalText(bestRow(rows, "formula")?.value_text || "");
  return /pcs|potere\s+calorifico/.test(formula) && multipliers.length === 1 ? multipliers[0] : null;
}
...
function essentialCommodityUnit(row, commodity, kind) {
  ...
  if (kind === "price") return /(?:€|eur)\s*\/\s*smc\b/i.test(text);
  const hasCurrencyPerVolume = /(?:€|eur)\s*\/\s*(?:smc|mc|m3)\b|(?:€|eur)\s*\/\s*m³/i.test(text);
  return !hasCurrencyPerVolume && gasVolumeUnit(row) !== null;
}
...
function comparisonBaseRow(row, commodity) {
  ...
  const pattern = commodity === "electricity"
    ...
    : /corrispettivo(?:\s+consumo)?\s+gas|materia\s+(?:prima\s+)?gas|prezzo\s+(?:della\s+)?materia|componente\s+gas|gas\s+naturale|prezzo\s+all[’\']?ingrosso|\bpsv\b/;
  return pattern.test(text);
}

function comparisonNormalizationFactor(rows, commodity) {
  const multiplierRow = commodity === "gas" ? gasPcsPriceMultiplierRow(rows) : bestRow(rows, "multiplier");
  ...
  const normalizationFactor = commodity === "gas"
    ? /pcs|potere\s+calorifico/.test(`${formula} ${evidence}`)
...
function comparisonPriceForSupply(supply, essentials) {
  ...
  const formulaMentionsFactor = commodity === "gas"
    ? /pcs|potere\s+calorifico/.test(formulaText)
    : /perdit|rete|coefficiente.*perdit/.test(formulaText);
...
export function normalizePureAiOutput(parsed, { model = PDF_PURE_AI_DEFAULT_MODEL, responseId = null, transportMode = "pdf_originale", timings = {} } = {}) {
  ...
  for (const supply of adaptive.supplies) {
    ...
    const gasCoefficientRow = isLight ? null : gasConversionCoefficientRow(rows);
    const gasCoefficient = rowNumber(gasCoefficientRow);
    const mapConsumption = (essential, targetField, rawMcField) => {
      const row = essential?.row || null;
      if (!row) return;
      if (isLight) {
        setMapped(
          normalized,
          ...
          row,
          (item) => rowNumber(item),
          essential?.recovered ? { type: targetField.includes("periodo") ? "period_consumption_semantic_recovery" : "essential_field_semantic_recovery", original_purpose: row?.purpose || null, resolved_as: targetField.includes("periodo") ? "period_consumption" : "annual_consumption", mode: essential.mode } : null,
        );
        return;
      }
      const value = rowNumber(row);
      const unit = gasVolumeUnit(row);
      ...
      normalized[rawMcField] = value;
      filledFields.push(rawMcField);
      normalized.diagnostics.push(diagnostic(rawMcField, row, value, { type: "gas_volume_original_mc", unit: compact(row.unit, 40) || "mc" }));
      if (!(gasCoefficient > 0)) return;
      const smc = Number((value * gasCoefficient).toFixed(6));
      normalized[targetField] = smc;
      filledFields.push(targetField);
      normalized.diagnostics.push(diagnostic(targetField, row, smc, { type: "gas_mc_times_c_to_smc", source_mc: value, coefficient_c: gasCoefficient }));
    };

    mapConsumption(essentials.annual, isLight ? "consumo_luce_kwh" : "consumo_gas_smc", "consumo_gas_mc");
    mapConsumption(essentials.period, isLight ? "consumo_periodo_luce_kwh" : "consumo_periodo_gas_smc", "consumo_periodo_gas_mc");
    if (!isLight && gasCoefficientRow) {
      setMapped(normalized, filledFields, "coefficiente_conversione_gas_c", gasCoefficientRow);
    }
    if (isLight) {
      ...
      setMapped(normalized, filledFields, "residenza_luce", bestRow(rows, "residence_status"), (row) => normalizeResidenceStatus(row.value_text));
      const monthlyRows = rows.filter((row) => row?.purpose === "monthly_consumption");
      const monthly = Array(12).fill(null);
      const monthlySourceRows = Array(12).fill(null);
      for (const row of monthlyRows) {
        const monthIndex = monthIndexFromRow(row);
        const value = rowNumber(row);
        if (!Number.isInteger(monthIndex) || monthIndex < 0 || monthIndex > 11 || !(value >= 0)) continue;
        if (monthly[monthIndex] !== null) continue;
        monthly[monthIndex] = value;
        monthlySourceRows[monthIndex] = row;
      }
      if (monthly.every((value) => Number.isFinite(value))) {
        normalized.consumi_mensili_luce_kwh = monthly.map((value) => Number(Number(value).toFixed(6)));
        filledFields.push("consumi_mensili_luce_kwh");
        const firstMonthlyRow = monthlySourceRows.find(Boolean) || { purpose: "monthly_consumption", label: "Storico mensile luce", confidence: 90 };
        normalized.diagnostics.push(diagnostic("consumi_mensili_luce_kwh", firstMonthlyRow, normalized.consumi_mensili_luce_kwh, { type: "twelve_explicit_monthly_values" }));
      }
      ...
      setMapped(normalized, filledFields, "classe_contatore_gas", bestRow(rows, "meter_class"), (row) => normalizeGasMeterClass(row.value_text || row.label));
      setMapped(normalized, filledFields, "potere_calorifico_gas_gj_smc", bestRow(rows, "calorific_value"), (row) => {
        const value = rowNumber(row);
        return Number.isFinite(value) && value >= 0.02 && value <= 0.06 ? value : null;
      });
      setMapped(normalized, filledFields, "ambito_tariffario_gas", bestRow(rows, "gas_tariff_area"), (row) => normalizeGasTariffArea(row.value_text || row.label));
      setMapped(normalized, filledFields, "regime_accisa_gas", bestRow(rows, "gas_excise_regime"), (row) => normalizeGasExciseRegime(row.value_text || row.label));
      setMapped(normalized, filledFields, "regime_arisgam_gas", bestRow(rows, "gas_regional_tax_profile"), (row) => normalizeGasRegionalTaxProfile(row.value_text || row.label));
    }
    ...
    setMapped(normalized, filledFields, `moltiplicatore_indice_${suffix}`, isLight ? bestRow(rows, "multiplier") : gasPcsPriceMultiplierRow(rows));
  ...
  const suppressedAnnualConsumptionFields = suppressPeriodConsumptionMisreadAsAnnual(normalized);

  const lightSignals = ["consumo_luce_kwh", "consumo_periodo_luce_kwh", "prezzo_luce_eur_kwh", "prezzo_luce_f1_eur_kwh", "quota_fissa_vendita_luce_eur_anno", "indice_riferimento_luce"].some((field) => normalized[field] !== null && normalized[field] !== undefined && normalized[field] !== "");
  const gasSignals = ["consumo_gas_smc", "consumo_periodo_gas_smc", "prezzo_gas_eur_smc", "quota_fissa_vendita_gas_eur_anno", "indice_riferimento_gas"].some((field) => normalized[field] !== null && normalized[field] !== undefined && normalized[field] !== "");
  ...
  normalized.ai = {
    ...
    accepted_count: filledFields.filter((field) => !suppressedAnnualConsumptionFields.includes(field)).length,
    filled_fields: [...new Set(filledFields.filter((field) => !suppressedAnnualConsumptionFields.includes(field)))],
...
function priceRecoveryCommodities(normalized, parsed) {
  const adaptive = canonicalizeAdaptive(coerceAdaptive(parsed));
  const commodities = [];
  for (const supply of adaptive.supplies || []) {
    if (!supply || !["electricity", "gas"].includes(supply.commodity)) continue;
    const suffix = supply.commodity === "electricity" ? "luce" : "gas";
    const detail = normalized?.[`normalizzazione_prezzo_${suffix}`] || {};
    const reasons = Array.isArray(detail?.reasons) ? detail.reasons : [];
    if (detail?.source === "prezzo_medio_fallback" || detail?.source === "unavailable" || reasons.includes("formula_e_componenti_insufficienti")) {
      commodities.push(supply.commodity);
    }
  }
  return [...new Set(commodities)];
}

function buildRecoveryNeeds(normalized, parsed) {
  const needs = missingEssentialFields(normalized, parsed).map((item) => ({ commodity: item.commodity, fields: [...item.fields] }));
  for (const commodity of priceRecoveryCommodities(normalized, parsed)) {
    let item = needs.find((entry) => entry.commodity === commodity);
    if (!item) {
      item = { commodity, fields: [] };
      needs.push(item);
    }
    if (!item.fields.includes("unit_price")) item.fields.push("unit_price");
  }
  return needs;
}

function priceRecoveryRank(normalized, commodity) {
  const suffix = commodity === "electricity" ? "luce" : "gas";
  const source = normalized?.[`normalizzazione_prezzo_${suffix}`]?.source || "unavailable";
  return ({ componenti_deterministiche: 4, prezzo_materia_diretto: 3, prezzo_medio_fallback: 1, unavailable: 0 })[source] ?? 0;
}

function priceRecoveryImproved(before, after, commodities = []) {
  return commodities.some((commodity) => priceRecoveryRank(after, commodity) > priceRecoveryRank(before, commodity));
}

function recoveryRowFingerprint(row) {
  return [
    row?.purpose || "",
    comparisonCanonicalText(row?.label || ""),
    rowNumber(row) ?? "",
    comparisonCanonicalText(row?.value_text || ""),
    comparisonCanonicalText(row?.unit || ""),
  ].join("|");
}

function safePriceRecoveryCompanion(row, commodity) {
  if (!row) return null;
  if (row.purpose === "price_component") {
    return rowNumber(row) !== null && comparisonUnitMatches(row, commodity) ? row : null;
  }
  if (row.purpose === "spread") {
    return rowNumber(row) !== null && comparisonUnitMatches(row, commodity) ? row : null;
  }
  if (row.purpose === "multiplier") {
    const text = essentialRowText(row, true);
    if (commodity === "gas") return /pcs|potere\s+calorifico/.test(text) && rowNumber(row) > 0 ? row : null;
    return /perdit|rete/.test(text) && rowNumber(row) > 0 ? row : null;
  }
  if (["formula", "index", "price_type"].includes(row.purpose)) {
    return compact(row.value_text, 300) ? row : null;
  }
  return null;
}

function directMaterialRecoveryComponent(row, commodity) {
  if (!row || row.purpose !== "unit_price" || rowNumber(row) === null || !comparisonUnitMatches(row, commodity)) return null;
  const text = comparisonRowText(row);
  if (/spesa\s+per\s+(?:la\s+)?vendita|costo\s+per\s+(?:la\s+)?vendita|prezzo\s+medio|quota\s+per\s+consumi|rete|oneri|trasport|distribuz|impost|\biva\b|accis/.test(text)) return null;
  const component = { ...row, purpose: "price_component" };
  return comparisonBaseRow(component, commodity) ? component : null;
}
...
function safeEssentialRecoveryRow(row, commodity, purpose, rows = []) {
  ...
  if (purpose === "multiplier") return commodity === "gas" && gasConversionCoefficientCandidate(row);
...
function mergeEssentialRecoveryOutput(primaryParsed, recoveryParsed, missing) {
  ...
  for (const recoverySupply of recovery.supplies || []) {
    ...
    const existing = new Set((target.fields || []).map(recoveryRowFingerprint));
    const priceRecoveryRequested = requested.has("unit_price");
    const alreadyHasUnitPrice = (target.fields || []).some((item) => item?.purpose === "unit_price");
    for (const row of recoveryRows) {
      ...
      const gasCoefficientFallback = recoverySupply.commodity === "gas" && row?.purpose === "multiplier" && requested.has("annual_consumption");
      const priceCompanion = priceRecoveryRequested ? safePriceRecoveryCompanion(row, recoverySupply.commodity) : null;

      if (requestedDirectly || periodFallbackForAnnual || gasCoefficientFallback) {
        if (row?.purpose === "unit_price" && alreadyHasUnitPrice && priceRecoveryRequested) {
          acceptedRow = directMaterialRecoveryComponent(row, recoverySupply.commodity);
        } else if (safeEssentialRecoveryRow(row, recoverySupply.commodity, row.purpose, [...target.fields, ...recoveryRows])) {
          acceptedRow = row;
        }
      }
      if (!acceptedRow && priceCompanion) acceptedRow = priceCompanion;
      if (!acceptedRow) continue;
      const withConfidence = { ...acceptedRow, confidence: Math.max(95, Number(acceptedRow.confidence ?? 95)) };
      const fingerprint = recoveryRowFingerprint(withConfidence);
      if (existing.has(fingerprint)) continue;
      existing.add(fingerprint);
      target.fields.push(withConfidence);
    }
  }
  return merged;
}
...
const PRIVACY_EXCLUDED_FIELDS = new Set([
  "intestatario",
  "codice_fiscale",
  "codice_cliente",
  "codice_cliente_luce",
...
const ESSENTIAL_FIELD_BY_PURPOSE = Object.freeze({
  electricity: Object.freeze({
    annual_consumption: "consumo_luce_kwh",
    unit_price: "prezzo_luce_eur_kwh",
    fixed_fee: "quota_fissa_vendita_luce_eur_anno",
  }),
  gas: Object.freeze({
    annual_consumption: "consumo_gas_smc",
    unit_price: "prezzo_gas_eur_smc",
    fixed_fee: "quota_fissa_vendita_gas_eur_anno",
  }),
});

function causalMissingReasons(result = {}) {
  const reasons = new Map();
  if (result?.ai?.applied) {
    for (const field of PRIVACY_EXCLUDED_FIELDS) reasons.set(field, "non_estratto_per_policy_privacy_lettore");
  }

  const missingAfter = Array.isArray(result?.ai?.essential_missing_after) ? result.ai.essential_missing_after : [];
  const recoveryAttempted = result?.ai?.recovery_attempted === true;
  const recoveryError = compact(result?.ai?.recovery_error, 220);
  for (const item of missingAfter) {
    const map = ESSENTIAL_FIELD_BY_PURPOSE[item?.commodity] || {};
    for (const purpose of Array.isArray(item?.fields) ? item.fields : []) {
      const field = map[purpose];
      if (!field) continue;
      if (!recoveryAttempted) reasons.set(field, "non_individuato_dal_lettore_ia");
      else if (recoveryError && recoveryError !== "essential_recovery_no_improvement") reasons.set(field, "rilettura_ia_mirata_non_completata");
      else reasons.set(field, "non_individuato_dopo_rilettura_ia_mirata");
    }
  }
  return reasons;
}

function applyCausalMissingReasons(result = {}) {
  const reasons = causalMissingReasons(result);
  const contractFields = result?.data_contract?.fields && typeof result.data_contract.fields === "object"
    ? result.data_contract.fields
    : {};

  for (const [field, entry] of Object.entries(contractFields)) {
    if (!entry || entry.status !== "mancante") continue;
    let reason = reasons.get(field) || "";
    if (!reason && result?.ai?.applied && ["valore_non_estratto", "periodo_non_estratto"].includes(String(entry.status_reason || ""))) {
      reason = "non_restituito_dal_lettore_ia";
    }
    if (!reason) continue;
    entry.status_reason = reason;
    const statusKey = ["fornitore_luce", "fornitore_gas"].includes(field) ? "fornitore"
      : ["codice_cliente_luce", "codice_cliente_gas"].includes(field) ? "codice_cliente"
        : field;
    if (result?.field_status?.[statusKey]?.status === "mancante") result.field_status[statusKey].reason = reason;
  }
  return result;
}
...
export async function extractPdfPureAi({
  ...
  try {
    ...
    try {
      ...
      const priceRecoveryBefore = priceRecoveryCommodities(normalizedResult, parsedOutput);
      const recoveryNeeds = buildRecoveryNeeds(normalizedResult, parsedOutput);
      if (essentialMissingCount(recoveryNeeds) > 0) {
        ...
        if (Number.isFinite(recoveryBudget) && recoveryBudget >= 8_000) {
          ...
          try {
            const recoveryRequest = await buildPdfEssentialRecoveryRequest({ filePath: useOpenAiFileId ? null : filePath, fileId: openAiFileId || null, filename, model, missing: recoveryNeeds });
            ...
            const mergedOutput = mergeEssentialRecoveryOutput(parsedOutput, recoveryOutput, recoveryNeeds);
            now = Date.now();
            ...
            const recoveredPrice = priceRecoveryImproved(normalizedResult, recoveredResult, priceRecoveryBefore);
            if (essentialMissingCount(missingAfter) < essentialMissingCount(missingBefore) || recoveredPeriodConsumption || recoveredPrice) {
      ...
      normalizedResult.ai = {
        ...
        price_recovery_requested: priceRecoveryBefore,
  ...
  if (normalizedResult) {
    ...
    return applyCausalMissingReasons(normalizedResult);
  }
  throw analysisError || new Error("openai_invalid_output");
}

// OFFERTALOGICA_BILL_PHOTO_V1_20260920
// Riutilizza lo stesso contratto dati del lettore PDF: cambia solo il trasporto
// del documento (immagine ad alta definizione), non il motore di confronto.
function billPhotoMimeType(value = "") {
  const mime = String(value || "").trim().toLowerCase();
  if (["image/jpeg", "image/png", "image/webp"].includes(mime)) return mime;
  throw new Error("bill_photo_unsupported_mime");
}

function billPhotoInput(bytes, mimeType) {
  const mime = billPhotoMimeType(mimeType);
  return {
    type: "input_image",
...
async function buildBillPhotoRequest({ filePath, mimeType, model, recoveryMissing = null } = {}) {
  if (!filePath) throw new Error("bill_photo_file_path_required");
  const bytes = await fs.readFile(filePath);
  if (!bytes.length) throw new Error("bill_photo_empty");
  const imageInput = billPhotoInput(bytes, mimeType);
  const recovery = Array.isArray(recoveryMissing) && recoveryMissing.length > 0;
  const requested = recovery
    ? recoveryMissing.map((item) => `${item.commodity}: ${(item.fields || []).join(", ")}`).join("; ")
    : "";
  return {
    model,
    store: false,
    temperature: 0,
    max_output_tokens: recovery ? 1_800 : 4_500,
    input: [
      {
        role: "system",
        content: [{ type: "input_text", text: recovery ? ESSENTIAL_RECOVERY_SYSTEM_PROMPT : SYSTEM_PROMPT }],
      },
      {
        role: "user",
        content: [
          imageInput,
          {
            type: "input_text",
            text: recovery
              ? `Campi indispensabili ancora mancanti: ${requested}. Leggi solo ciò che è chiaramente visibile nella foto. Non inventare né annualizzare dati non espliciti.`
              : `${USER_PROMPT}\nQuesta volta il documento è una fotografia di una bolletta. Leggi solo valori chiaramente visibili; se un dato non è leggibile o non è presente, lascialo mancante.`,
          },
        ],
      },
    ],
    text: {
      format: {
        type: "json_schema",
        name: recovery ? "offertalogica_photo_essential_recovery" : "offertalogica_photo_compact_adaptive_form",
        description: recovery
          ? "Recupero mirato dei dati indispensabili visibili nella foto della bolletta"
          : "Lettura economica adattiva da foto bolletta, senza dati personali",
        strict: true,
        schema: REQUEST_OUTPUT_SCHEMA,
      },
    },
...
export async function extractBillPhotoPureAi({
  filePath,
  mimeType = "image/jpeg",
  deadlineAt = null,
  transport = defaultTransport,
  ...
  const fileStats = await fs.stat(filePath);
  const runRequest = async ({ recoveryMissing = null, attempt = 1 } = {}) => {
    const request = await buildBillPhotoRequest({ filePath, mimeType, model, recoveryMissing });
    const effectiveTimeout = Math.min(configuredTimeout, remainingBudget());
    if (!Number.isFinite(effectiveTimeout) || effectiveTimeout < 8_000) throw new Error("openai_insufficient_time_budget");
    const controller = new AbortController();
    let timeoutId;
    try {
      const timeoutPromise = new Promise((_, reject) => {
        timeoutId = setTimeout(() => {
          controller.abort();
          reject(new Error(attempt === 1 ? "openai_timeout" : "openai_photo_recovery_timeout"));
        }, effectiveTimeout);
      });
      const raw = await Promise.race([
        transport({
          ...
          attempt,
          profile: attempt === 1 ? "bill_photo_primary_v1" : "bill_photo_essential_recovery_v1",
        }),
        timeoutPromise,
      ]);
      const body = await transportBody(raw);
      ...
      const outputText = responseOutputText(body);
      if (!outputText) throw new Error("openai_empty_output");
      return { parsed: JSON.parse(outputText), responseId: compact(body?.id, 160) || null, outputText };
    } finally {
      clearTimeout(timeoutId);
    }
  };

  const primary = await runRequest({ attempt: 1 });
  let parsed = primary.parsed;
  let normalized = normalizePureAiOutput(parsed, {
    model,
    responseId: primary.responseId,
    transportMode: "bill_photo_original",
    timings: {
      openai_ms: Date.now() - startedAt,
  ...
  const missingBefore = missingEssentialFields(normalized, parsed);
  let recoveryResponseId = null;
  let recoveryError = null;
  let recoveryAccepted = false;
  let attempts = 1;
  if (essentialMissingCount(missingBefore) > 0 && remainingBudget() >= 9_000) {
    try {
      const recovery = await runRequest({ recoveryMissing: missingBefore, attempt: 2 });
      attempts = 2;
      recoveryResponseId = recovery.responseId;
      const merged = mergeEssentialRecoveryOutput(parsed, recovery.parsed, missingBefore);
      const recovered = normalizePureAiOutput(merged, {
        model,
        responseId: primary.responseId,
        transportMode: "bill_photo_original",
        timings: {
      ...
      const missingAfterRecovery = missingEssentialFields(recovered, merged);
      if (essentialMissingCount(missingAfterRecovery) < essentialMissingCount(missingBefore)) {
        parsed = merged;
        normalized = recovered;
        recoveryAccepted = true;
      } else {
        recoveryError = "photo_recovery_no_improvement";
      }
    } catch (error) {
      recoveryError = compact(error?.message || error, 220) || "photo_recovery_failed";
    }
  }

  const missingAfter = missingEssentialFields(normalized, parsed);
  normalized.ai = {
    ...(normalized.ai || {}),
    input_kind: "photo",
    request_profile: "bill_photo_primary_v1",
    recovery_attempted: attempts > 1,
    recovered_from: recoveryAccepted ? "bill_photo_essential_recovery_v1" : null,
    recovery_response_id: recoveryResponseId,
    recovery_error: recoveryError,
    essential_missing_before: missingBefore,
    essential_missing_after: missingAfter,
  ...
  normalized._reader_trace = {
    trace_version: "reader-trace-photo-v1",
    captured_at: new Date().toISOString(),
    response_id: primary.responseId,
    recovery_response_id: recoveryResponseId,
    raw_output_chars: primary.outputText.length,
    raw_ai: parsed,
  };
  return normalized;
}
