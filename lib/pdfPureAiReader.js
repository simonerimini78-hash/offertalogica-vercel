...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.19";
...
const REQUEST_PROFILE = "priority_economic_compact_v2";
  ...
  items: {
    ...
    properties: {
      field: {
        type: "string",
        enum: [
          "provider", "offer_name", "offer_code", "pod", "pdr",
          "customer_name", "tax_code", "customer_code", "supply_address",
...
const SYSTEM_PROMPT = `Sei il lettore visuale di OffertaLogica per bollette e condizioni economiche luce/gas italiane.

PRIORITÀ ASSOLUTA
Analizza tutte le pagine e, per ogni fornitura realmente presente, cerca prima:
1. consumo annuo totale;
2. prezzo unitario commerciale confrontabile;
3. quota fissa commerciale;
4. formula e componenti necessarie al calcolo del prezzo.

Tratta luce e gas separatamente anche nei documenti dual. Non considerare completata la lettura di una fornitura finché non hai verificato tutti e quattro i gruppi prioritari.

REGOLE ECONOMICHE
- Consumo annuo: accetta soltanto consumo annuo, ultimi 12 mesi o dodici mesi. Escludi consumi fatturati, del periodo, mensili, parziali, esempi e proiezioni. Riporta anche F1/F2/F3/F23 soltanto quando sono esplicitamente annuali.
- Prezzo luce: usa il prezzo della sola vendita/materia energia in EUR/kWh. Nelle bollette è valido anche il prezzo/costo medio unitario della sola vendita; escludi il prezzo medio complessivo della quota consumi, rete, trasporto, oneri, imposte e IVA.
- Prezzo gas: applica la stessa regola alla sola vendita/materia prima gas in EUR/Smc, escludendo distribuzione, trasporto, oneri, accise, imposte e IVA.
- Prezzi per fasce: copia F0/F1/F2/F3/F23 nei rispettivi campi. Non calcolare medie: la media ponderata sarà calcolata dal sistema soltanto con consumi annui di fascia compatibili.
- Formula: riporta soltanto la formula corrente e i relativi indice, moltiplicatore, spread e periodicità quando sono stampati. Escludi esempi, valori storici e rinnovi futuri.
- Quota fissa: nel gruppo value/value_text riporta la componente commerciale esplicita di vendita/commercializzazione (PCV, CCV, QVD o equivalente), conservando valore, segno e unità stampati. Nel gruppo section_total_* riporta separatamente il totale netto della sezione QUOTA FISSA quando è stampato con unità mensile o annuale. Non sommare righe, non cambiare segno e non usare rete, distribuzione, contatore, quota potenza o oneri. Se la componente commerciale è negativa, restituiscila comunque negativa: il sistema deciderà dopo quale valore usare nel confronto.
- Copia solo dati visibili. Non inventare, stimare, sommare righe o ricavare spread per sottrazione.
- evidence deve contenere la riga completa o poche righe contigue che dimostrano valore, unità e significato; page è la pagina visiva.

DATI AGGIUNTIVI
Venditore, nome/codice offerta, POD, PDR, intestatario, codice fiscale, codice cliente, indirizzo e potenza sono secondari. Inseriscili in additional_data soltanto quando li incontri chiaramente durante la lettura. Non dedicare tempo a cercare separatamente un dato secondario assente.

OUTPUT
Inserisci in supplies una sola voce per ogni fornitura realmente presente: electricity, gas oppure entrambe nel dual. Non creare una voce per la fornitura assente. Restituisci soltanto JSON conforme allo schema.`;

const USER_PROMPT = `Leggi il PDF seguendo la priorità economica indicata. Completa prima consumo annuo, prezzo commerciale, quota fissa e formula per luce e gas; poi conserva gli eventuali dati aggiuntivi già individuati.`;
  ...
  if (["number", "fixed"].includes(spec.kind)) {
    ...
    if (spec.kind === "fixed") {
      ...
      if (number < 0) {
        return {
          accepted: false,
          reason: "negative_fixed_component_preserved_not_compared",
          raw_fixed: {
            value: number,
            value_text: compact(answer.value_text, 120) || String(number),
            unit: compact(answer.unit, 60) || null,
            ...
            evidence: compact(answer.evidence, 650) || null,
...
function compactFixedFeeDetails(fixed = {}) {
  ...
  if (commercial.value !== null && commercial.value >= 0) selectedForComparison = "commercial_component";
  else if (sectionTotal.value !== null && sectionTotal.value >= 0) selectedForComparison = "section_total";
...
export function normalizePureAiOutput(parsed, {
  ...
  if (validatedF0 !== null && validatedF0 > 0) {
  ...
  normalized.ai = {
    ...
    verification_protocol: "comparison_essentials_compact_v2",
  };
