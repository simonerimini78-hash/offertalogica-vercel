...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.10";
...
// d'archivio. La richiesta OpenAI usa invece quattro blocchi compatti per fornitura:
// identità minima, consumo annuo, prezzo commerciale e quota fissa.
...
const COMPACT_OUTPUT_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["document", "electricity", "gas"],
  properties: {
    document: {
      type: "object",
      additionalProperties: false,
...
Estrai soltanto i dati indispensabili al confronto economico: consumo annuo, prezzo commerciale corrente e quota fissa commerciale. Conserva fornitore e nome offerta solo come identificazione breve.

REGOLE
- Analizza tutte le pagine, ma restituisci un output molto compatto.
- Tratta luce e gas separatamente; se una commodity non è presente lascia tutti i suoi valori null.
- Non inventare, non stimare e non usare valori medi.
- Consumo annuo: accetta solo una riga dichiarata “consumo annuo” o “ultimi 12 mesi”. Escludi consumi del periodo, fatturati, da inizio fornitura inferiore a 12 mesi, esempi e proiezioni.
- Per la luce conserva total, F1, F2, F3 e F23 soltanto quando appartengono allo stesso riepilogo annuale.
- Prezzo: usa solo condizioni commerciali correnti del venditore. Escludi costo medio, spesa annua stimata, importi totali, rete, trasporto, oneri, imposte, valori storici e rinnovi futuri.
- Luce: conserva prezzo unico/F0 e gli eventuali F1/F2/F3/F23. Non fare medie.
- Variabile: conserva indice, eventuale moltiplicatore, spread e formula esatta. Non calcolare il prezzo finale.
- Quota fissa: usa soltanto commercializzazione/vendita/corrispettivo annuo definito dal venditore. Escludi rete e oneri. period deve essere month o year solo quando la periodicità è stampata nella stessa evidenza.
- Una lettura parziale è valida. Usa null per ogni dato non dimostrato.
- evidence deve contenere la riga o il piccolo blocco che dimostra i valori, non spiegazioni.
...
const USER_PROMPT = `Leggi il documento e compila esclusivamente questi blocchi per luce e gas:
1. identity: fornitore e nome offerta, se immediatamente visibili;
2. annual_consumption: consumo annuo totale e fasce annuali;
3. price: prezzo commerciale corrente, fasce o formula variabile;
4. fixed_fee: quota fissa commerciale.
Non cercare POD/PDR, intestatario, codice cliente, indirizzo, potenza, codice offerta, dati di attivazione o altre informazioni non necessarie al confronto.`;
...
export async function buildPdfPureAiRequest({
  ...
  return {
    ...
    store: false,
    temperature: 0,
    max_output_tokens: Number(maxOutputTokens || 1_800),
    input: [
    ...
    text: {
      format: {
        type: "json_schema",
        name: "offertalogica_comparison_essentials",
...
function normalizeAnswer(spec, answer, document = {}) {
  ...
  if (["number", "fixed"].includes(spec.kind)) {
    ...
    const nonNegative = LIGHT_BAND_CONSUMPTION_FIELDS.includes(spec.field)
      || ["prezzo_luce_eur_kwh", ...LIGHT_BAND_PRICE_FIELDS, "prezzo_gas_eur_smc"].includes(spec.field);
    ...
    if (nonNegative && number < 0) return { accepted: false, reason: "invalid_number" };
    if (positiveMultiplier && number <= 0) return { accepted: false, reason: "invalid_number" };
    if (!signedSpread && !nonNegative && !positiveMultiplier && number <= 0) return { accepted: false, reason: "invalid_number" };
...
function compactSupplyAnswers(supply, commodity) {
  ...
  for (const [questionId, key] of consumptionFields) {
    if (Number.isFinite(Number(consumption[key]))) {
      answers.push(compactAnswer(questionId, {
        valueNumber: Number(consumption[key]), valueText: String(consumption[key]), unit: compact(consumption.unit, 60) || (commodity === "luce" ? "kWh" : "Smc"),
  ...
  if (commodity === "luce") {
    ...
    for (const [questionId, key] of lightPrices) {
      if (Number.isFinite(Number(price[key]))) {
        answers.push(compactAnswer(questionId, {
          valueNumber: Number(price[key]), valueText: String(price[key]), unit: priceUnit,
          page: pricePage, label: key === "single" ? priceLabel : `Prezzo ${key.toUpperCase()}`,
          evidence: priceEvidence, confidence: priceConfidence,
        }));
      }
    }
    const bands = ["f1", "f2", "f3", "f23"].filter((key) => Number.isFinite(Number(price[key])));
    if (bands.length) {
    ...
    } else if (Number.isFinite(Number(price.single)) || Number.isFinite(Number(price.f0))) {
      answers.push(compactAnswer("struttura_prezzo_luce", {
        valueText: "monoraria", page: pricePage, label: priceLabel, evidence: priceEvidence, confidence: priceConfidence,
      }));
    }
  } else if (Number.isFinite(Number(price.single))) {
    answers.push(compactAnswer("prezzo_gas_eur_smc", {
      valueNumber: Number(price.single), valueText: String(price.single), unit: priceUnit,
      page: pricePage, label: priceLabel, evidence: priceEvidence, confidence: priceConfidence,
    }));
  }
  ...
  if (Number.isFinite(Number(price.multiplier))) answers.push(compactAnswer(`moltiplicatore_indice_${prefix}`, { valueNumber: Number(price.multiplier), valueText: String(price.multiplier), unit: "coefficiente", page: pricePage, label: priceLabel, evidence: priceEvidence, confidence: priceConfidence }));
  if (Number.isFinite(Number(price.spread))) answers.push(compactAnswer(`spread_${prefix}_${commodity === "luce" ? "eur_kwh" : "eur_smc"}`, { valueNumber: Number(price.spread), valueText: String(price.spread), unit: priceUnit, page: pricePage, label: priceLabel, evidence: priceEvidence, confidence: priceConfidence }));
  ...
  if (Number.isFinite(Number(fixed.value)) && ["month", "year"].includes(fixed.period)) {
    answers.push(compactAnswer(`quota_fissa_vendita_${prefix}`, {
      valueNumber: Number(fixed.value), valueText: String(fixed.value), unit: compact(fixed.unit, 60) || null,
      period: fixed.period, page: fixed.page || null, label: compact(fixed.label, 180) || "Quota fissa commerciale",
      evidence: compact(fixed.evidence, 450) || null, confidence: Number(fixed.confidence || 0),
    }));
  }
  return answers;
}
...
export function normalizePureAiOutput(parsed, {
  ...
  const f1Consumption = Number(normalized.consumo_luce_f1_kwh);
  const f2Consumption = Number(normalized.consumo_luce_f2_kwh);
  const f3Consumption = Number(normalized.consumo_luce_f3_kwh);
  const f23Consumption = Number(normalized.consumo_luce_f23_kwh);
  if (!Number.isFinite(f23Consumption) && Number.isFinite(f2Consumption) && Number.isFinite(f3Consumption)) {
  ...
  if (!Number.isFinite(Number(normalized.consumo_luce_kwh))) {
    const derivedTotal = Number.isFinite(f1Consumption) && Number.isFinite(Number(normalized.consumo_luce_f23_kwh))
      ? f1Consumption + Number(normalized.consumo_luce_f23_kwh)
      : Number.isFinite(f1Consumption) && Number.isFinite(f2Consumption) && Number.isFinite(f3Consumption)
    ...
    if (Number.isFinite(derivedTotal) && derivedTotal > 0) {
      ...
      derivedBandDiagnostics.push({
        ...
        source_match: null, source_role: "annual_total", certainty: "certain", usable_for_comparison: true,
        verification_reason: null, coverage_months: 12, method: "deterministic_sum", source_version: PDF_PURE_AI_READER_VERSION,
        derivation: { type: "annual_bands_to_total", source_fields: Number.isFinite(Number(normalized.consumo_luce_f23_kwh)) ? ["consumo_luce_f1_kwh", "consumo_luce_f23_kwh"] : ["consumo_luce_f1_kwh", "consumo_luce_f2_kwh", "consumo_luce_f3_kwh"], derived_value: normalized.consumo_luce_kwh },
      });
  ...
  if (normalized.prezzo_luce_eur_kwh === undefined && Number.isFinite(Number(normalized.prezzo_luce_f0_eur_kwh))) {
    normalized.prezzo_luce_eur_kwh = Number(normalized.prezzo_luce_f0_eur_kwh);
  ...
  try {
    ...
    try {
      ...
      const raw = await Promise.race([
        transport({ request, apiKey, signal: controller.signal, attempt: 1, profile: "comparison_essentials" }),
        timeoutPromise,
      ]);
      ...
      normalizedResult.ai = {
        ...(normalizedResult.ai || {}),
        request_profile: "comparison_essentials",
