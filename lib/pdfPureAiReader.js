...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.13";
...
// Il contratto storico a domande resta disponibile nel normalizzatore e nei replay
// d'archivio. La richiesta OpenAI separa ora:
// 1) fatti osservati nel documento, copiati senza reinterpretarli;
// 2) dati contrattuali utilizzabili per il confronto;
// 3) una verifica mirata automatica dei soli campi mancanti, quando resta tempo.
const COMPACT_IDENTITY_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["provider", "offer_name", "page", "evidence", "confidence"],
  properties: {
...
const COMPACT_CONSUMPTION_SCHEMA = {
  type: "object",
  additionalProperties: false,
...
const COMPACT_PRICE_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: [
    "type", "single", "f0", "f1", "f2", "f3", "f23", "index", "multiplier",
    "spread", "formula", "periodicity", "unit", "page", "label", "evidence", "confidence",
  ],
  properties: {
...
const COMPACT_FIXED_FEE_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["value", "period", "unit", "page", "label", "evidence", "confidence"],
  properties: {
...
const COMPACT_SUPPLY_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["identity", "annual_consumption", "price", "fixed_fee"],
  properties: {
    identity: COMPACT_IDENTITY_SCHEMA,
    annual_consumption: COMPACT_CONSUMPTION_SCHEMA,
    price: COMPACT_PRICE_SCHEMA,
    fixed_fee: COMPACT_FIXED_FEE_SCHEMA,
  },
};

const OBSERVED_FACT_FIELDS = Object.freeze([
  "customer_type", "intestatario", "codice_fiscale", "codice_cliente", "codice_cliente_luce", "codice_cliente_gas",
  "fornitore_luce", "fornitore_gas", "nome_offerta_luce", "nome_offerta_gas", "codice_offerta_luce", "codice_offerta_gas",
  "pod", "pdr", "indirizzo_fornitura_luce", "indirizzo_fornitura_gas", "potenza_impegnata_kw", "potenza_disponibile_kw",
  "numero_fattura", "data_emissione", "scadenza_pagamento", "data_attivazione_luce", "data_attivazione_gas",
  "consumo_fatturato_luce_kwh", "consumo_fatturato_luce_f1_kwh", "consumo_fatturato_luce_f2_kwh", "consumo_fatturato_luce_f3_kwh", "consumo_fatturato_luce_f23_kwh",
  "consumo_fatturato_gas_smc", "spesa_materia_energia_luce_eur", "spesa_materia_gas_eur",
  "costo_medio_materia_energia_luce_eur_kwh", "costo_medio_materia_gas_eur_smc",
  "totale_bolletta_eur", "costo_medio_bolletta_luce_eur_kwh", "costo_medio_bolletta_gas_eur_smc",
]);

const OBSERVED_FACT_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["field", "commodity", "value_text", "value_number", "unit", "period", "role", "page", "label", "evidence", "confidence"],
  properties: {
    field: { type: "string", enum: OBSERVED_FACT_FIELDS },
    commodity: { type: "string", enum: ["common", "electricity", "gas"] },
    value_text: { type: ["string", "null"], maxLength: 350 },
    value_number: { type: ["number", "null"] },
    unit: { type: ["string", "null"], maxLength: 80 },
    period: { type: "string", enum: ["none", "billing_period", "month", "year", "annual"] },
    role: { type: "string", enum: ["identity", "customer_data", "technical", "billing", "period_consumption", "annual_total", "period_cost", "average_cost", "contract_term", "unknown"] },
    page: { type: ["integer", "null"], minimum: 1 },
...
const COMPACT_OUTPUT_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["document", "facts", "electricity", "gas"],
  properties: {
    document: {
      type: "object",
      additionalProperties: false,
      required: [
        "kind", "commodity", "customer_type", "page_count", "classification_evidence",
        "billing_period_start", "billing_period_end", "supply_start_date",
      ],
      properties: {
        ...
        classification_evidence: { type: ["string", "null"], maxLength: 500 },
        billing_period_start: { type: ["string", "null"], maxLength: 40 },
        billing_period_end: { type: ["string", "null"], maxLength: 40 },
        supply_start_date: { type: ["string", "null"], maxLength: 40 },
      },
    },
    facts: { type: "array", maxItems: 80, items: OBSERVED_FACT_SCHEMA },
    electricity: COMPACT_SUPPLY_SCHEMA,
    gas: COMPACT_SUPPLY_SCHEMA,
  },
};

const SYSTEM_PROMPT = `Sei il lettore visuale di OffertaLogica per bollette e schede luce/gas italiane.

OBIETTIVO
Leggi integralmente tutte le pagine. Separa sempre ciò che è scritto nel documento da ciò che è realmente utilizzabile nel confronto economico.

1. FACTS — DATI OSSERVATI
- Inserisci in facts ogni dato utile presente nel documento: intestatario, CF/P.IVA, codici cliente, POD/PDR, indirizzo, potenza, fornitore, offerta e codice offerta, date, consumi fatturati, spesa materia energia/gas, costi medi e totale bolletta.
- Copia il valore esatto con pagina, etichetta ed evidence letterale.
- Non fermarti dopo aver trovato fornitore e offerta.
- I consumi fatturati o del periodo vanno in facts come period_consumption. Non trasformarli in consumo annuo.
- I costi medi vanno in facts come average_cost. Non trasformarli in prezzo contrattuale.

2. BLOCCHI DI CONFRONTO
- annual_consumption: solo una riga dichiarata consumo annuo o ultimi 12 mesi. Escludi consumi fatturati, mensili, periodici, esempi e proiezioni.
- price: solo condizioni commerciali correnti del venditore. Escludi costo medio, spesa totale, rete, trasporto, oneri, imposte, valori storici e rinnovi futuri.
- fixed_fee: solo quota fissa commerciale di vendita/commercializzazione. Escludi rete e oneri.
- Per prezzi variabili conserva indice, moltiplicatore, spread e formula esatta; non calcolare il prezzo finale.

REGOLE DI CERTEZZA
- Tratta luce e gas separatamente.
- Non inventare, non stimare, non annualizzare consumi del periodo.
- Una lettura parziale è valida: usa null quando il dato non è dimostrato.
- evidence deve essere la riga o il piccolo blocco del documento, non una spiegazione.
- customer_type è utilizzabile soltanto se anche facts contiene customer_type con evidenza letterale.

CLASSIFICAZIONE
bill = bolletta cliente; offer_sheet = scheda/condizioni economiche; unknown = non pertinente.

Restituisci soltanto JSON conforme allo schema.`;

const USER_PROMPT = `Analizza tutte le pagine e compila:
1. document: classificazione e date visibili;
2. facts: tutti i dati utili realmente presenti, anche quando non sono utilizzabili nel confronto;
3. electricity e gas: identità, consumo annuo autentico, prezzo contrattuale corrente e quota fissa commerciale.
Mantieni separati consumi fatturati, costi medi e condizioni contrattuali.`;

const RECOVERY_PROMPT = `Esegui una seconda verifica visuale mirata. Cerca soltanto i campi indicati come mancanti, controllando tutte le pagine, intestazioni, riquadri e tabelle. Riporta facts con valore, pagina ed evidence letterale. Nei blocchi annual_consumption, price e fixed_fee inserisci un valore solo se rispetta le regole di certezza; altrimenti lascialo null. Non sostituire il prezzo contrattuale con un costo medio e non sostituire il consumo annuo con consumi fatturati.`;

const FAST_FALLBACK_PROMPT = `Esegui una lettura rapida ma rigorosa di tutte le pagine.
Priorità assoluta:
1. fornitore, commodity, intestatario e tipologia cliente con evidenza;
2. POD/PDR, indirizzo, potenza, codice cliente e nome/codice offerta;
3. consumo annuo autentico, prezzo contrattuale corrente e quota fissa commerciale;
4. consumi fatturati, spesa materia, costi medi e totale bolletta come facts non utilizzabili automaticamente nel confronto.
Mantieni facts conciso: massimo 30 elementi, senza duplicati. Non usare consumi fatturati come consumo annuo e non usare costi medi come prezzo contrattuale.`;
...
export async function buildPdfPureAiRequest({
  ...
  const recovery = profile === "missing_fields_recovery";
  const fastFallback = profile === "fast_fallback";
  const focus = [...new Set((Array.isArray(focusFields) ? focusFields : []).map((value) => compact(value, 120)).filter(Boolean))];
  const prompt = recovery
    ? `${RECOVERY_PROMPT}\n\nCampi da verificare: ${focus.length ? focus.join(", ") : "dati utili mancanti"}.`
    : fastFallback
      ? FAST_FALLBACK_PROMPT
      : USER_PROMPT;
  const defaultMaxOutputTokens = recovery ? 1_400 : fastFallback ? 1_800 : 2_800;
  return {
    model,
    store: false,
    temperature: 0,
    max_output_tokens: Number(maxOutputTokens || defaultMaxOutputTokens),
    input: [
    ...
    text: {
      format: {
        type: "json_schema",
        name: recovery
          ? "offertalogica_missing_fields_recovery"
          : fastFallback
            ? "offertalogica_fast_fallback"
            : "offertalogica_complete_document",
        description: recovery
          ? "Verifica mirata dei dati mancanti in una bolletta o scheda luce/gas"
          : fastFallback
            ? "Lettura rapida di sicurezza dei dati essenziali di bollette e offerte luce/gas"
            : "Dati osservati e condizioni economiche verificabili di bollette e offerte luce/gas",
        strict: true,
...
const OBSERVED_FACT_FIELD_SET = new Set(OBSERVED_FACT_FIELDS);

function factSourceRole(role) {
  if (role === "identity") return "identity";
  if (role === "customer_data" || role === "technical" || role === "billing") return "customer_data";
  if (role === "annual_total") return "annual_total";
  if (role === "period_consumption") return "period_total";
  if (role === "contract_term") return "contract_term";
  if (role === "average_cost") return "average_cost";
  return "unknown";
}

function observedFactsToAnswers(facts) {
  if (!Array.isArray(facts)) return [];
  const answers = [];
  for (const fact of facts) {
    const questionId = compact(fact?.field, 120);
    if (!QUESTION_BY_ID.has(questionId)) continue;
    const valueText = fact?.value_text === null || fact?.value_text === undefined ? null : compact(fact.value_text, 350);
    const valueNumber = finiteNumberOrNull(fact?.value_number);
    if (valueText === null && valueNumber === null) continue;
    const role = factSourceRole(fact?.role);
    answers.push(compactAnswer(questionId, {
      valueText,
      valueNumber,
      unit: compact(fact?.unit, 80) || null,
      period: ["month", "year"].includes(fact?.period) ? fact.period : "none",
      page: Number.isInteger(Number(fact?.page)) && Number(fact.page) > 0 ? Number(fact.page) : null,
      label: compact(fact?.label, 180) || questionId,
      evidence: compact(fact?.evidence, 650) || null,
      confidence: Number(fact?.confidence || 0),
      sourceRole: role === "unknown" ? null : role,
    }));
  }
  return answers;
}

function normalizeObservedFacts(facts) {
  if (!Array.isArray(facts)) return [];
  return facts
    .filter((fact) => OBSERVED_FACT_FIELD_SET.has(compact(fact?.field, 120)))
    .map((fact) => {
      const valueNumber = finiteNumberOrNull(fact?.value_number);
      const valueText = fact?.value_text === null || fact?.value_text === undefined ? null : compact(fact.value_text, 350);
      return {
        field: compact(fact.field, 120),
        commodity: ["common", "electricity", "gas"].includes(fact?.commodity) ? fact.commodity : "common",
        value: valueNumber !== null ? valueNumber : valueText,
        value_text: valueText,
        value_number: valueNumber,
        unit: compact(fact?.unit, 80) || null,
        period: ["none", "billing_period", "month", "year", "annual"].includes(fact?.period) ? fact.period : "none",
        role: compact(fact?.role, 80) || "unknown",
        page: Number.isInteger(Number(fact?.page)) && Number(fact.page) > 0 ? Number(fact.page) : null,
        label: compact(fact?.label, 180) || null,
        evidence: compact(fact?.evidence, 650) || null,
        confidence: Math.max(0, Math.min(100, Number(fact?.confidence || 0))),
        usable_for_comparison: ["annual_total", "contract_term"].includes(fact?.role),
        source: "openai_visual_ai_only",
      };
    })
    .filter((fact) => fact.value !== null && fact.value !== undefined && fact.value !== "");
}

function observedFactByField(facts, field) {
  return facts.find((fact) => fact.field === field && fact.value_number !== null) || null;
}

function deriveObservedData(facts) {
  const derived = [];
  const definitions = [
    {
      commodity: "electricity",
      consumption: "consumo_fatturato_luce_kwh",
      expense: "spesa_materia_energia_luce_eur",
      observedAverage: "costo_medio_materia_energia_luce_eur_kwh",
      field: "costo_effettivo_materia_energia_luce_eur_kwh",
      unit: "EUR/kWh",
    },
    {
      commodity: "gas",
      consumption: "consumo_fatturato_gas_smc",
      expense: "spesa_materia_gas_eur",
      observedAverage: "costo_medio_materia_gas_eur_smc",
      field: "costo_effettivo_materia_gas_eur_smc",
      unit: "EUR/Smc",
    },
  ];
  for (const definition of definitions) {
    const consumption = observedFactByField(facts, definition.consumption);
    const expense = observedFactByField(facts, definition.expense);
    if (!consumption || !expense || Number(consumption.value_number) <= 0) continue;
    const value = Number((Number(expense.value_number) / Number(consumption.value_number)).toFixed(8));
    const printedAverage = observedFactByField(facts, definition.observedAverage);
    derived.push({
      field: definition.field,
      commodity: definition.commodity,
      value,
      unit: definition.unit,
      formula: `${expense.value_number} / ${consumption.value_number}`,
      inputs: [definition.expense, definition.consumption],
      printed_value: printedAverage?.value_number ?? null,
      printed_difference: printedAverage ? Number((value - Number(printedAverage.value_number)).toFixed(8)) : null,
      classification: "derived_period_cost",
      usable_for_comparison: false,
      reason: "costo_effettivo_del_periodo_non_prezzo_contrattuale",
    });
  }
  return derived;
}
...
function factQuality(fact) {
  return Number(fact?.confidence || 0)
    + (compact(fact?.evidence, 650) ? 25 : 0)
    + (Number.isInteger(Number(fact?.page)) && Number(fact.page) > 0 ? 10 : 0)
    + (fact?.value_number !== null && fact?.value_number !== undefined ? 5 : 0);
}

function mergeFacts(primaryFacts, recoveryFacts) {
  const byKey = new Map();
  for (const fact of [...(Array.isArray(primaryFacts) ? primaryFacts : []), ...(Array.isArray(recoveryFacts) ? recoveryFacts : [])]) {
    if (!fact || !OBSERVED_FACT_FIELD_SET.has(compact(fact.field, 120))) continue;
    const key = `${compact(fact.field, 120)}|${compact(fact.commodity, 40) || "common"}`;
    const existing = byKey.get(key);
    if (!existing || factQuality(fact) > factQuality(existing)) byKey.set(key, fact);
  }
  return [...byKey.values()];
}

function structuredMissing(value, key = "") {
  if (value === null || value === undefined || value === "") return true;
  if (value === "unknown" || value === "none") return true;
  if (key === "confidence" && Number(value) <= 0) return true;
  return false;
}

function fillStructuredMissing(primary, recovery, key = "") {
  if (structuredMissing(primary, key)) return recovery;
  if (Array.isArray(primary)) return primary;
  if (!primary || typeof primary !== "object" || Array.isArray(primary)) return primary;
  if (!recovery || typeof recovery !== "object" || Array.isArray(recovery)) return primary;
  const output = { ...primary };
  for (const [childKey, recoveryValue] of Object.entries(recovery)) {
    output[childKey] = Object.prototype.hasOwnProperty.call(output, childKey)
      ? fillStructuredMissing(output[childKey], recoveryValue, childKey)
      : recoveryValue;
  }
  return output;
}

function mergeStructuredOutputs(primary, recovery) {
  if (!recovery || typeof recovery !== "object") return primary;
  const merged = fillStructuredMissing(primary, recovery);
  merged.facts = mergeFacts(primary?.facts, recovery?.facts);
  return merged;
}

function hasObservedField(normalized, field) {
  return Array.isArray(normalized?.observed_data) && normalized.observed_data.some((item) => item.field === field);
}

function recoveryFocusFields(normalized) {
  const fields = [];
  const addMissing = (field) => {
    if (normalized?.[field] === null || normalized?.[field] === undefined || normalized?.[field] === "") fields.push(field);
  };
  const addObserved = (field) => {
    if (!hasObservedField(normalized, field)) fields.push(field);
  };
  const isBill = normalized?.kind === "bolletta";
  if (isBill) {
    for (const field of ["customer_type", "intestatario", "codice_fiscale", "codice_cliente"]) addMissing(field);
  }
  if (["luce", "dual"].includes(normalized?.commodity)) {
    if (isBill) {
      for (const field of ["pod", "indirizzo_fornitura_luce", "potenza_impegnata_kw", "potenza_disponibile_kw"]) addMissing(field);
      for (const field of ["consumo_fatturato_luce_kwh", "spesa_materia_energia_luce_eur", "costo_medio_materia_energia_luce_eur_kwh", "totale_bolletta_eur"]) addObserved(field);
    }
    for (const field of ["codice_offerta_luce", "consumo_luce_kwh", "prezzo_luce_eur_kwh", "quota_fissa_vendita_luce_eur_anno", "tipo_prezzo_luce"]) addMissing(field);
  }
  if (["gas", "dual"].includes(normalized?.commodity)) {
    if (isBill) {
      for (const field of ["pdr", "indirizzo_fornitura_gas"]) addMissing(field);
      for (const field of ["consumo_fatturato_gas_smc", "spesa_materia_gas_eur", "costo_medio_materia_gas_eur_smc", "totale_bolletta_eur"]) addObserved(field);
    }
    for (const field of ["codice_offerta_gas", "consumo_gas_smc", "prezzo_gas_eur_smc", "quota_fissa_vendita_gas_eur_anno", "tipo_prezzo_gas"]) addMissing(field);
  }
  return [...new Set(fields)].slice(0, 24);
}

function boundedRecoveryBudget(value) {
  const parsed = Number(value ?? 14_000);
  return Number.isFinite(parsed) ? Math.max(12_000, Math.min(20_000, parsed)) : 14_000;
}

function boundedPrimaryPassTimeout(value) {
  const parsed = Number(value ?? 24_000);
  return Number.isFinite(parsed) ? Math.max(18_000, Math.min(28_000, parsed)) : 24_000;
}

function boundedFallbackMinBudget(value) {
  const parsed = Number(value ?? 22_000);
  return Number.isFinite(parsed) ? Math.max(18_000, Math.min(26_000, parsed)) : 22_000;
}

function boundedFallbackPassTimeout(value) {
  const parsed = Number(value ?? 21_000);
  return Number.isFinite(parsed) ? Math.max(15_000, Math.min(22_000, parsed)) : 21_000;
}

function boundedRecoveryPassTimeout(value) {
  const parsed = Number(value ?? 9_000);
  return Number.isFinite(parsed) ? Math.max(8_000, Math.min(12_000, parsed)) : 9_000;
}

function recoverableInitialPassError(error) {
  return /openai_timeout|openai_incomplete|openai_empty_output|AbortError|fetch failed|socket/i
...
function compactOutputToLegacy(parsed) {
  ...
  const factAnswers = observedFactsToAnswers(parsed.facts);
  ...
  const answers = [...factAnswers, ...electricityAnswers, ...gasAnswers];
  if (providers.length) {
...
export function normalizePureAiOutput(parsed, {
  ...
  const structuredOutput = parsed;
  const observedData = normalizeObservedFacts(structuredOutput?.facts);
  ...
  const normalized = {
    ...
    // La classificazione sintetica del documento non basta per compilare il profilo cliente:
    // customer_type viene accettato solo se compare anche come fatto con evidenza letterale.
    customer_type: null,
    ...
    derived_data: deriveObservedData(observedData),
  };
  const filledFields = [];
  const rejected = [];
  ...
  normalized.ai = {
    ...
    verification_protocol: "complete_document_plus_targeted_recovery_v2",
  };
  return applyPdfDataContract(applyPdfFieldValidation(normalized));
}

async function runOpenAiPass({ request, transport, apiKey, timeoutMs, attempt, profile }) {
  const controller = new AbortController();
  let timeoutId;
  try {
    const timeoutPromise = new Promise((_, reject) => {
      timeoutId = setTimeout(() => { controller.abort(); reject(new Error("openai_timeout")); }, timeoutMs);
    });
    const raw = await Promise.race([
      transport({ request, apiKey, signal: controller.signal, attempt, profile }),
      timeoutPromise,
    ]);
...
export async function extractPdfPureAi({
  ...
  let initialOpenAiMs = 0;
  let primaryOpenAiMs = 0;
  let fallbackOpenAiMs = 0;
  let recoveryOpenAiMs = 0;
  ...
  try {
    ...
    const fallbackMinBudget = boundedFallbackMinBudget(env.PDF_AI_FALLBACK_MIN_BUDGET_MS);
    const primaryTimeoutCap = boundedPrimaryPassTimeout(env.PDF_AI_PRIMARY_PASS_TIMEOUT_MS);
    const initialBuildStartedAt = Date.now();
    const initialRequest = await buildPdfPureAiRequest({
    ...
    requestBuildMs += Date.now() - initialBuildStartedAt;
    const initialTimeout = Math.min(
      configuredTimeout,
      primaryTimeoutCap,
      Math.max(8_000, remainingBudget() - fallbackMinBudget),
    );
    if (!Number.isFinite(initialTimeout) || initialTimeout < 8_000) throw new Error("openai_insufficient_time_budget");

    const primaryStartedAt = Date.now();
    passCount = 1;
    try {
      const primaryPass = await runOpenAiPass({
      ...
      primaryOpenAiMs = Date.now() - primaryStartedAt;
      primaryParsed = primaryPass.parsed;
      primaryOutputText = primaryPass.outputText;
      primaryResponseId = compact(primaryPass.body?.id, 160) || null;
      initialParsed = primaryParsed;
      initialOutputText = primaryOutputText;
      initialResponseId = primaryResponseId;
    } catch (error) {
      primaryOpenAiMs = Date.now() - primaryStartedAt;
      primaryError = compact(error?.message || error, 300) || "primary_failed";
      if (!recoverableInitialPassError(error) || remainingBudget() < fallbackMinBudget) throw error;

      const fallbackBuildStartedAt = Date.now();
      const fallbackRequest = await buildPdfPureAiRequest({
        filePath: useOpenAiFileId ? null : filePath,
        fileId: openAiFileId || null,
        filename,
        model,
      ...
      requestBuildMs += Date.now() - fallbackBuildStartedAt;
      const fallbackTimeout = Math.min(
        configuredTimeout,
        boundedFallbackPassTimeout(env.PDF_AI_FALLBACK_TIMEOUT_MS),
        remainingBudget() - 4_000,
      );
      if (!Number.isFinite(fallbackTimeout) || fallbackTimeout < 8_000) throw error;

      const fallbackStartedAt = Date.now();
      passCount += 1;
      try {
        const fallbackPass = await runOpenAiPass({
        ...
        fallbackOpenAiMs = Date.now() - fallbackStartedAt;
        fallbackParsed = fallbackPass.parsed;
        fallbackOutputText = fallbackPass.outputText;
        fallbackResponseId = compact(fallbackPass.body?.id, 160) || null;
        initialParsed = fallbackParsed;
        initialOutputText = fallbackOutputText;
        initialResponseId = fallbackResponseId;
        baselineProfile = "fast_fallback";
        fallbackUsed = true;
      } catch (secondError) {
        fallbackOpenAiMs = Date.now() - fallbackStartedAt;
        fallbackError = compact(secondError?.message || secondError, 300) || "fallback_failed";
        throw secondError;
      }
    }

    initialOpenAiMs = primaryOpenAiMs + fallbackOpenAiMs;
    mergedParsed = initialParsed;

    const initialNormalized = normalizePureAiOutput(initialParsed, {
      model,
      ...
      timings: {
        ...
        openai_ms: initialOpenAiMs,
        total_ms: Date.now() - startedAt,
        openai_attempts: passCount,
        input_file_bytes: Number(fileStats.size || 0),
        file_id_threshold_bytes: fileIdThreshold,
      },
    });

    recoveryFocus = recoveryFocusFields(initialNormalized);
    const recoveryMinBudget = boundedRecoveryBudget(env.PDF_AI_RECOVERY_MIN_BUDGET_MS);
    const supportsStructuredRecovery = Array.isArray(initialParsed?.facts);
    if (!fallbackUsed && supportsStructuredRecovery && recoveryFocus.length && remainingBudget() >= recoveryMinBudget) {
      const recoveryBuildStartedAt = Date.now();
      const recoveryRequest = await buildPdfPureAiRequest({
        filePath: useOpenAiFileId ? null : filePath,
        fileId: openAiFileId || null,
        filename,
        model,
        profile: "missing_fields_recovery",
        focusFields: recoveryFocus,
      });
      requestBuildMs += Date.now() - recoveryBuildStartedAt;
      const recoveryTimeout = Math.min(
        configuredTimeout,
        boundedRecoveryPassTimeout(env.PDF_AI_RECOVERY_TIMEOUT_MS),
        remainingBudget() - 6_000,
      );
      if (Number.isFinite(recoveryTimeout) && recoveryTimeout >= 8_000) {
        const recoveryStartedAt = Date.now();
        passCount += 1;
        try {
          const recoveryPass = await runOpenAiPass({
          ...
          recoveryOpenAiMs = Date.now() - recoveryStartedAt;
          recoveryParsed = recoveryPass.parsed;
          recoveryOutputText = recoveryPass.outputText;
          recoveryResponseId = compact(recoveryPass.body?.id, 160) || null;
          mergedParsed = mergeStructuredOutputs(initialParsed, recoveryParsed);
        } catch (error) {
          recoveryOpenAiMs = Date.now() - recoveryStartedAt;
          recoveryError = compact(error?.message || error, 300) || "recovery_failed";
        }
      }
    }

    const now = Date.now();
    normalizedResult = normalizePureAiOutput(mergedParsed, {
      model,
      responseId: initialResponseId,
      transportMode: useOpenAiFileId ? "openai_file_id" : "pdf_originale",
      timings: {
    ...
    normalizedResult.ai = {
      ...(normalizedResult.ai || {}),
      request_profile: baselineProfile,
      primary_profile: "complete_document",
      primary_error: primaryError,
      primary_response_id: primaryResponseId,
      primary_openai_ms: primaryOpenAiMs,
      fallback_used: fallbackUsed,
      fallback_profile: fallbackUsed ? "fast_fallback" : null,
      fallback_error: fallbackError,
      fallback_response_id: fallbackResponseId,
      fallback_openai_ms: fallbackOpenAiMs,
      recovery_attempted: Boolean(recoveryParsed || recoveryError),
      recovery_completed: Boolean(recoveryParsed),
      recovery_skipped_reason: fallbackUsed ? "fallback_used" : null,
      recovery_focus_fields: recoveryFocus,
      recovery_error: recoveryError,
      recovered_from: recoveryParsed ? "missing_fields_recovery" : fallbackUsed ? "fast_fallback" : null,
      initial_response_id: initialResponseId,
      recovery_response_id: recoveryResponseId,
      initial_openai_ms: initialOpenAiMs,
      recovery_openai_ms: recoveryOpenAiMs,
      analysis_pass_count: passCount,
      openai_attempts: passCount,
      retry_count: fallbackUsed ? 1 : 0,
    };
    // Traccia interna: permette allo staff di confrontare il tentativo completo,
    // l'eventuale fallback rapido, la verifica mirata e la normalizzazione finale.
    normalizedResult._reader_trace = {
      trace_version: "reader-trace-v3",
      captured_at: new Date(now).toISOString(),
      response_id: initialResponseId,
      request_profile: baselineProfile,
      raw_output_chars: primaryOutputText.length + fallbackOutputText.length + recoveryOutputText.length,
      raw_ai: mergedParsed,
      raw_ai_primary: primaryParsed,
      raw_ai_fallback: fallbackParsed,
      raw_ai_initial: initialParsed,
      raw_ai_recovery: recoveryParsed,
      primary_error: primaryError,
      fallback_error: fallbackError,
      recovery_focus_fields: recoveryFocus,
      recovery_error: recoveryError,
