...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.16";
...
const QUESTION_SPECS = Object.freeze([
  ...
  { id: "quota_fissa_vendita_luce", field: "quota_fissa_vendita_luce_eur_anno", kind: "fixed", question: "Qual è la quota fissa confrontabile della luce? Nelle schede sintetiche usa il corrispettivo fisso commerciale definito dal venditore. Nelle bollette: se la componente di vendita è positiva, riporta quella; se la componente di vendita è negativa, non usare il valore negativo e non cambiarne il segno: riporta invece il totale netto positivo della quota fissa nella stessa sezione, escludendo una riga separata espressa in €/kW/mese o riferita soltanto alla potenza. Non sommare righe e non usare il valore assoluto. Indica period=month o year soltanto quando l’unità è stampata nella stessa riga selezionata; altrimenti usa found=false." },
  ...
  { id: "quota_fissa_vendita_gas", field: "quota_fissa_vendita_gas_eur_anno", kind: "fixed", question: "Qual è la quota fissa confrontabile del gas? Nelle schede sintetiche usa il corrispettivo fisso commerciale definito dal venditore. Nelle bollette: se la componente di vendita è positiva, riporta quella; se la componente di vendita è negativa, non usare il valore negativo e non cambiarne il segno: riporta invece il totale netto positivo della quota fissa nella stessa sezione, escludendo righe di trasporto, distribuzione, contatore o altri importi separati. Non sommare righe e non usare il valore assoluto. Indica period=month o year soltanto quando l’unità è stampata nella stessa riga selezionata; altrimenti usa found=false." },
...
- Quota fissa: nelle schede usa il corrispettivo commerciale contrattuale. Nelle bollette conserva segno e unità originali. Se la componente di vendita è negativa, non restituirla, non trasformarla in positiva e non annualizzarla: scegli soltanto il totale netto positivo della quota fissa stampato nella stessa sezione. Non usare una riga separata in €/kW/mese o riferita soltanto alla potenza, non sommare righe e non usare valori assoluti. Riporta in evidence solo la riga scelta, così rete/oneri presenti in righe vicine non contaminano il dato. period=month o year solo quando l’unità compare nella stessa riga.
...
function semanticValidation(spec, answer, document = {}) {
  ...
  if (["quota_fissa_vendita_luce_eur_anno", "quota_fissa_vendita_gas_eur_anno"].includes(spec.field)) {
    ...
    const powerOnly = (/€\s*\/?\s*kw\b|\beur\s*\/?\s*kw\b|\bkw\s*\/?\s*mese\b|\bquota\s+potenza\b/.test(text)) && !/\bquota\s+fiss[ao]\b/.test(text);
    if (futureTerm) return { accepted: false, reason: "semantic_fixed_fee_future_term" };
    if (powerOnly) return { accepted: false, reason: "semantic_fixed_fee_power_only" };
