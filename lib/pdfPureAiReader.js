...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.1";
...
export async function extractPdfPureAi({
  ...
  const initialRemaining = deadlineAt ? Number(deadlineAt) - Date.now() - 2_000 : configuredTimeout;
  ...
  const remaining = deadlineAt ? Number(deadlineAt) - Date.now() - 2_000 : configuredTimeout;
