...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.17";
...
const QUESTION_SPECS = Object.freeze([
  ...
  { id: "quota_fissa_vendita_luce", field: "quota_fissa_vendita_luce_eur_anno", kind: "fixed", question: "Qual è la quota fissa confrontabile della luce? Nelle schede sintetiche usa il corrispettivo fisso commerciale definito dal venditore e imposta fixed_fee_role=commercial_component. Nelle bollette: se la riga 'di cui spesa per la vendita di energia elettrica' o l'equivalente riga di vendita/commercializzazione è positiva, riporta quella e imposta fixed_fee_role=commercial_component. Se quella componente è negativa, non restituirla e non cambiarne il segno: riporta il totale netto positivo della sezione QUOTA FISSA e imposta fixed_fee_role=section_total. È ammessa anche una riga chiamata 'Quota fissa e quota potenza' quando il valore scelto è espresso in €/mese; una eventuale riga separata in €/kW/mese è power_component e non va sommata né scelta. Le righe di rete/oneri sono regulated_component e non vanno scelte. Indica period=month o year soltanto quando l’unità è stampata nella riga selezionata; altrimenti usa found=false." },
  ...
  { id: "quota_fissa_vendita_gas", field: "quota_fissa_vendita_gas_eur_anno", kind: "fixed", question: "Qual è la quota fissa confrontabile del gas? Nelle schede sintetiche usa il corrispettivo fisso commerciale definito dal venditore e imposta fixed_fee_role=commercial_component. Nelle bollette: se la riga 'di cui spesa per la vendita/commercializzazione' è positiva, riporta quella e imposta fixed_fee_role=commercial_component. Se quella componente è negativa, non restituirla e non cambiarne il segno: riporta il totale netto positivo della sezione QUOTA FISSA e imposta fixed_fee_role=section_total. Le righe di trasporto, distribuzione, contatore, rete o oneri sono regulated_component e non vanno scelte. Non sommare righe e non usare valori assoluti. Indica period=month o year soltanto quando l’unità è stampata nella riga selezionata; altrimenti usa found=false." },
...
const REQUEST_ANSWER_SCHEMA = {
  ...
  required: [
    ...
    "page", "label", "evidence", "confidence", "fixed_fee_role",
  ],
  properties: {
    ...
    fixed_fee_role: {
      type: "string",
      enum: ["none", "commercial_component", "section_total", "regulated_component", "power_component"],
    },
  },
};
...
- Prezzo luce: cerca un solo prezzo unitario confrontabile della componente commerciale di vendita/materia energia in EUR/kWh. Nelle condizioni economiche usa il prezzo unico o monorario corrente. Nelle bollette accetta anche un prezzo/costo medio unitario quando la stessa riga o voce lo riferisce esplicitamente soltanto a vendita energia, materia energia, componente energia o equivalente commerciale. Deve essere separato da rete, trasporto, oneri, imposte e IVA. Non usare il prezzo medio complessivo della quota consumi o della bolletta.
- Prezzo gas: applica la stessa regola alla sola vendita/materia prima gas in EUR/Smc. Accetta il prezzo medio commerciale esplicito della bolletta, ma escludi rete, trasporto, distribuzione, oneri, accise, imposte e IVA.
- Gerarchia prezzo luce: il sistema darà priorità a F0; in assenza di F0 potrà calcolare una media ponderata solo se prezzi e consumi annui per fascia sono completi e compatibili; altrimenti userà il prezzo medio commerciale esplicito estratto da questa risposta. Tu non calcolare medie.
...
- Quota fissa: per ogni risposta imposta sempre fixed_fee_role. Usa none per tutte le domande che non riguardano la quota fissa. Per la quota fissa usa commercial_component quando scegli la riga positiva della vendita/commercializzazione; usa section_total quando la componente commerciale è negativa e scegli il totale netto positivo della sezione QUOTA FISSA; usa regulated_component per rete, trasporto, distribuzione, contatore o oneri; usa power_component per una riga separata in €/kW/mese. Nelle bollette conserva segno e unità originali. Non trasformare un valore negativo in positivo, non sommare righe e non usare valori assoluti. Una riga totale denominata "Quota fissa e quota potenza" è ammessa come section_total se il valore selezionato è in €/mese; la riga separata in €/kW/mese resta power_component e non va aggiunta. Riporta in evidence la riga scelta; period=month o year solo quando l’unità compare nella stessa riga.
...
const USER_PROMPT = `Rispondi alle seguenti domande nello stesso ordine. Inserisci sempre una voce answers per ciascun question_id; usa found=false e valori null quando il dato non è dimostrato. Imposta fixed_fee_role=none per tutte le domande non relative alla quota fissa.\n\n${REQUEST_QUESTION_SPECS
...
const FIXED_FEE_ROLES = Object.freeze([
  "none", "commercial_component", "section_total", "regulated_component", "power_component",
]);

function answerFixedFeeRole(answer) {
  if (FIXED_FEE_ROLES.includes(answer?.fixed_fee_role)) return answer.fixed_fee_role;
  const text = answerSemanticText(answer);
  const selectedText = semanticText([answer?.label, answer?.value_text, answer?.unit].filter(Boolean).join(" "), 500);
  const unit = semanticText(answer?.unit, 120);
  if (/\bquota\s+potenza\b/.test(selectedText) && (/\bkw\b/.test(unit) || /€\s*\/?\s*kw|eur\s*\/?\s*kw/.test(selectedText))) return "power_component";
  if (/\b(vendita|commercializzazione|pcv|ccv|qvd)\b/.test(selectedText)) return "commercial_component";
  if (/\bquota\s+fiss[ao]\b/.test(selectedText)) return "section_total";
  if (/\b(trasporto|distribuzione|gestione\s+contatore|oneri|rete)\b/.test(selectedText)) return "regulated_component";
  if (/\b(vendita|commercializzazione|pcv|ccv|qvd)\b/.test(text)) return "commercial_component";
  if (/\bquota\s+fiss[ao]\b/.test(text)) return "section_total";
  if (/\b(trasporto|distribuzione|gestione\s+contatore|oneri|rete)\b/.test(text)) return "regulated_component";
  return "none";
}
...
function semanticValidation(spec, answer, document = {}) {
  ...
  let usable = answerUsable(answer, role);
  ...
  if (["quota_fissa_vendita_luce_eur_anno", "quota_fissa_vendita_gas_eur_anno"].includes(spec.field)) {
    const fixedFeeRole = answerFixedFeeRole(answer);
    const selectedText = semanticText([answer?.label, answer?.value_text, answer?.unit].filter(Boolean).join(" "), 500);
    ...
    const selectedCommercial = /\b(commercializzazione|vendita|pcv|ccv|qvd)\b/.test(selectedText)
      || (isOfferSheet && /\b(?:quota|corrispettivo|componente|costo)\s+fiss[oa]\b|\bcorrispettivo\s+annuo\b|\bcosto\s+fisso\s+ann(?:o|uo)\b/.test(selectedText));
    const sectionTotal = /\bquota\s+fiss[ao]\b/.test(text);
    const selectedSectionTotal = /\bquota\s+fiss[ao]\b/.test(selectedText);
    ...
    const selectedUnit = semanticText(answer?.unit, 120);
    const selectedPowerUnit = /\bkw\b/.test(selectedUnit) || /€\s*\/?\s*kw\b|\beur\s*\/?\s*kw\b/.test(semanticText(answer?.value_text, 160));
    if (futureTerm) return { accepted: false, reason: "semantic_fixed_fee_future_term" };
    if (fixedFeeRole === "power_component" || selectedPowerUnit) return { accepted: false, reason: "semantic_fixed_fee_power_only" };
    if (fixedFeeRole === "regulated_component") return { accepted: false, reason: "semantic_fixed_fee_regulated_component" };
    const explicitFixedFeeRole = ["commercial_component", "section_total", "regulated_component", "power_component"].includes(answer?.fixed_fee_role);
    if (fixedFeeRole === "commercial_component" && (!commercialFixed || (explicitFixedFeeRole && !selectedCommercial))) return { accepted: false, reason: "semantic_fixed_fee_not_commercial" };
    if (fixedFeeRole === "section_total" && (!sectionTotal || (explicitFixedFeeRole && !selectedSectionTotal))) return { accepted: false, reason: "semantic_fixed_fee_not_section_total" };
    if (!["commercial_component", "section_total"].includes(fixedFeeRole)) {
      const regulatedOnly = /\b(trasporto|distribuzione|gestione\s+contatore|oneri|impost[ae]|accis[ae]|iva|rete)\b/.test(text) && !commercialFixed && !sectionTotal;
      if (regulatedOnly || role === "regulated") return { accepted: false, reason: "semantic_fixed_fee_regulated_component" };
      if (!commercialFixed && !sectionTotal) return { accepted: false, reason: "semantic_fixed_fee_not_commercial" };
    }
    if (!hasCurrency) return { accepted: false, reason: "semantic_fixed_fee_currency_missing" };
...
function normalizeAnswer(spec, answer, document = {}) {
  ...
  if (["number", "fixed"].includes(spec.kind)) {
    ...
    if (spec.kind === "fixed") {
      ...
      if (number < 0) return { accepted: false, reason: "negative_fixed_component_not_comparable" };
      const isBillMonthly = document?.kind === "bill" && answer.period === "month";
      const targetField = isBillMonthly
        ? spec.field.replace(/_eur_anno$/, "_eur_mese").replace("quota_fissa_vendita_", "quota_fissa_confrontabile_")
        : spec.field;
      const value = isBillMonthly ? number : answer.period === "month" ? Number((number * 12).toFixed(6)) : number;
      return {
        accepted: true,
        value,
        ...
        derivation: {
          type: isBillMonthly ? "bill_monthly_fixed_observed" : answer.period === "month" ? "contract_monthly_to_annual" : "annual_literal",
          original_value: number,
          original_unit: compact(answer.unit, 60) || null,
          original_period: answer.period,
          factor: isBillMonthly ? 1 : answer.period === "month" ? 12 : 1,
          derived_value: value,
        },
      };
    }
...
function diagnosticFor(spec, answer, normalized) {
  return {
    field: normalized.target_field || spec.field,
...
function compactAnswer(question_id, { valueText = null, valueNumber = null, unit = null, period = "none", page = null, label = null, evidence = null, confidence = 0, sourceRole = null, fixedFeeRole = "none" } = {}) {
  return {
    question_id,
    ...
    evidence,
    confidence,
    fixed_fee_role: FIXED_FEE_ROLES.includes(fixedFeeRole) ? fixedFeeRole : "none",
...
function compactSupplyAnswers(supply, commodity) {
  ...
  if (fixedValue !== null && ["month", "year"].includes(fixed.period)) {
    answers.push(compactAnswer(`quota_fissa_vendita_${prefix}`, {
      ...
      fixedFeeRole: FIXED_FEE_ROLES.includes(fixed.role) ? fixed.role : "commercial_component",
    }));
  }
  return answers;
}
...
function resolveDocumentKind(declaredKind, input) {
  ...
  const offerEconomicSignals = presentFieldCount(input, [
    ...
    "quota_fissa_vendita_luce_eur_anno", "quota_fissa_vendita_gas_eur_anno",
    "quota_fissa_confrontabile_luce_eur_mese", "quota_fissa_confrontabile_gas_eur_mese",
  ]);
...
function commoditySignalCount(input, commodity) {
  const fields = commodity === "luce"
    ? [
      "pod", "consumo_luce_kwh", ...LIGHT_BAND_CONSUMPTION_FIELDS, "prezzo_luce_eur_kwh", ...LIGHT_BAND_PRICE_FIELDS, "quota_fissa_vendita_luce_eur_anno", "quota_fissa_confrontabile_luce_eur_mese",
    ...
    : [
      "pdr", "consumo_gas_smc", "prezzo_gas_eur_smc", "quota_fissa_vendita_gas_eur_anno", "quota_fissa_confrontabile_gas_eur_mese",
...
export function normalizePureAiOutput(parsed, {
  ...
  for (const spec of QUESTION_SPECS) {
    ...
    const targetField = result.target_field || spec.field;
    normalized[targetField] = result.value;
