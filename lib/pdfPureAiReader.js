...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v2.0.2-ia-libera-compact-form";
...
const REQUEST_PROFILE = "ia_libera_compact_form_v3_1";
...
PRIORITÀ ASSOLUTA: prima di completare una fornitura, cerca in tutto il PDF i tre dati minimi del confronto: (1) consumo annuo/ultimi 12 mesi, (2) prezzo unitario della sola vendita o materia energia/gas, (3) quota fissa della sola vendita/commercializzazione. Se uno di questi dati è presente nel documento non ometterlo, anche quando compare nello Scontrino dell'energia, nel Box dell'offerta o negli Elementi di dettaglio. Non sostituire mai il consumo annuo con il consumo del periodo e non sostituire mai il prezzo vendita con quota consumi totale, rete, oneri, imposte o IVA.

- annual_consumption: consumo annuo o ultimi 12 mesi;
- band_consumption: consumo annuo F1/F2/F3/F23;
- unit_price: il prezzo unitario finale che inseriresti nel campo “Quanto paghi la luce/gas?”, riferito alla sola vendita o materia energia/gas. Nelle bollette cerca con priorità nello “Scontrino dell’energia” la riga “di cui spesa per la vendita di energia elettrica/gas naturale” sotto la quota per consumi: il suo prezzo medio unitario è il dato obbligatorio da restituire come unit_price, anche per offerte variabili o indicizzate. Il Box dell’offerta, le formule e le tabelle con più mesi non sostituiscono questo valore medio già applicato. Se il documento mostra che un corrispettivo base deve essere corretto con sconti, perdite o altre componenti, non usare quel valore base come unit_price: riportalo come price_component. Usa unit_price soltanto quando il prezzo finale è esplicito nel documento; non calcolarlo e non inventarlo;
- band_price: prezzo finale F0/F1/F2/F3/F23;
- price_component: componente corrente che partecipa al prezzo, con l'etichetta originale;
...
- formula: espressione o testo che descrive come si forma il prezzo;
- index, spread, multiplier, periodicity;
...
- non duplicare la stessa voce come unit_price e price_component;
- non creare righe con valore null, “null”, “undefined” o prive di contenuto;
...
const USER_PROMPT = `Compila il modulo libero con consumi, prezzo finale se esplicito, fasce, componenti, formula e quota fissa presenti nel documento. Mantieni la struttura tariffaria originale senza produrre spiegazioni.`;
...
function essentialCommodityUnit(row, commodity, kind) {
  ...
  const commodityUnit = commodity === "electricity" ? "kwh" : "smc";
  if (!new RegExp(`\\b${commodityUnit}\\b`, "i").test(text)) return false;
  const hasCurrencyPerUnit = new RegExp(`(?:€|eur)\\s*\\/\\s*${commodityUnit}\\b`, "i").test(text);
...
function annualConsumptionCandidate(row, commodity) {
  ...
  else if (row?.purpose === "annual_consumption" && !periodOnly.test(evidence)) score = 170;
...
export function normalizePureAiOutput(parsed, { model = PDF_PURE_AI_DEFAULT_MODEL, responseId = null, transportMode = "pdf_originale", timings = {} } = {}) {
  ...
  const normalized = {
    ...
    textExtracted: 0,
    needsReview: true,
    adaptive_form: { version: "adaptive-form-v3.1-compact", supplies: [] },
...
function mergeEssentialRecoveryOutput(primaryParsed, recoveryParsed, missing) {
  ...
  for (const recoverySupply of recovery.supplies || []) {
    ...
    for (const row of recoveryRows) {
      if (!requested.has(row?.purpose)) continue;
      ...
      if (essentialMissingCount(missingBefore) > 0) {
        ...
        if (Number.isFinite(recoveryBudget) && recoveryBudget >= 8_000) {
          ...
          try {
            ...
            if (essentialMissingCount(missingAfter) < essentialMissingCount(missingBefore)) {
