...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.12";
...
export async function buildPdfPureAiRequest({
  ...
  return {
    ...
    store: false,
    temperature: 0,
    max_output_tokens: Number(maxOutputTokens || (recovery ? 3_200 : 4_500)),
    input: [
    ...
    text: {
      format: {
        type: "json_schema",
        name: recovery ? "offertalogica_missing_fields_recovery" : "offertalogica_complete_document",
...
function boundedRecoveryBudget(value) {
  const parsed = Number(value ?? 18_000);
  return Number.isFinite(parsed) ? Math.max(12_000, Math.min(30_000, parsed)) : 18_000;
}
...
export async function extractPdfPureAi({
  ...
  try {
    ...
    initialOpenAiMs = Date.now() - initialStartedAt;
    passCount = 1;
    initialParsed = initialPass.parsed;
    initialOutputText = initialPass.outputText;
    initialResponseId = compact(initialPass.body?.id, 160) || null;
    ...
    if (supportsStructuredRecovery && recoveryFocus.length && remainingBudget() >= recoveryMinBudget) {
      ...
      const recoveryTimeout = Math.min(configuredTimeout, remainingBudget());
    ...
    normalizedResult.ai = {
      ...(normalizedResult.ai || {}),
      request_profile: "complete_document",
      ...
      recovered_from: recoveryParsed ? "missing_fields_recovery" : null,
    ...
    // Traccia interna: permette allo staff di confrontare prima lettura,
    // verifica mirata, risultato unificato e normalizzazione finale.
    normalizedResult._reader_trace = {
      trace_version: "reader-trace-v2",
      captured_at: new Date(now).toISOString(),
      response_id: initialResponseId,
      request_profile: "complete_document",
      raw_output_chars: initialOutputText.length + recoveryOutputText.length,
      raw_ai: mergedParsed,
