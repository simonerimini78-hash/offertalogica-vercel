...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.21-ia-libera-slim";
...
const REQUEST_PROFILE = "ia_libera_comparison_form_slim_v1";
    ...
    properties: {
      field: {
        ...
        enum: [
          ...
          "committed_power_kw", "available_power_kw", "contract_code",
          "meter_serial", "billing_frequency",
        ],
      },
...
const SYSTEM_PROMPT = `Sei il lettore visuale di OffertaLogica.

COMPITO
Leggi il PDF e compila il blocco supplies per confrontare la fornitura attuale. Tratta luce e gas separatamente.

PER OGNI FORNITURA PRESENTE
- identity.provider: venditore.
- annual_consumption.total: consumo annuo, ultimi 12 mesi o “in un anno”. Non usare il consumo del periodo.
- price.single: prezzo commerciale unico della vendita in EUR/kWh o EUR/Smc, quando è stampato senza etichetta di fascia.
- price.f0: valore esplicitamente etichettato F0.
- price.f1/f2/f3/f23: soltanto valori direttamente etichettati con la relativa fascia.
- price.type, index, multiplier, spread, formula e periodicity: copiali quando presenti.
- fixed_fee: quota fissa commerciale come stampata, mantenendo valore, segno, unità e periodo. Non convertirla.

REGOLE
- Cerca i dati in qualunque pagina o sezione del PDF.
- Copia i valori visibili senza arrotondare, cambiare segno o ricavarli per sottrazione.
- Un prezzo generale non diventa F1 solo perché nella formula vicina compare F1.
- Se manca un dato, usa null. Non riempire la casella con un dato diverso.
- Per consumo, prezzo e quota fissa indica pagina, etichetta ed evidence sufficienti al controllo.

DATI PER DOPO
In additional_data copia soltanto gli altri dati già incontrati chiaramente durante la lettura, usando i nomi ammessi dallo schema. Non cercare separatamente dati secondari assenti e non rallentare la compilazione del confronto.

Restituisci esclusivamente JSON conforme allo schema.`;

const USER_PROMPT = `Compila prima il modulo di confronto supplies. Poi conserva in additional_data soltanto gli altri dati già visibili e già individuati.`;
...
export function normalizePureAiOutput(parsed, {
  ...
  normalized.ai = {
    ...
    verification_protocol: "ia_libera_comparison_form_slim_v1",
