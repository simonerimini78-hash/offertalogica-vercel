...
export const PDF_PURE_AI_READER_VERSION = "pure-ai-native-pdf-v1.0.6";
...
const ANSWER_SCHEMA = {
  ...
  required: [
    ...
    "page", "label", "evidence", "confidence", "source_role",
    "usable_for_comparison", "certainty", "reason", "coverage_months",
  ],
  properties: {
    ...
    source_role: { type: "string", enum: ANSWER_SOURCE_ROLES },
    usable_for_comparison: { type: "boolean" },
    certainty: { type: "string", enum: ANSWER_CERTAINTY },
    reason: { type: ["string", "null"], maxLength: 300 },
    coverage_months: { type: ["integer", "null"], minimum: 0, maximum: 1200 },
  },
};
  ...
  properties: {
    document: {
      ...
      properties: {
        ...
        supply_start_date: { type: ["string", "null"] },
      },
    },
    answers: {
      type: "array",
...
const SYSTEM_PROMPT = `Sei il lettore visuale di OffertaLogica. Devi comportarti come un consulente che interroga il documento con domande puntuali, non come un estrattore generico.

SEQUENZA OBBLIGATORIA
1. Classifica il documento: bill se contiene dati specifici di cliente/POD/PDR; offer_sheet se contiene scheda sintetica o condizioni economiche senza dati specifici del cliente; unknown negli altri casi.
2. Identifica commodity, profilo cliente, periodo fatturato e data di inizio fornitura.
3. Rispondi a ogni domanda copiando solo dati visibili.
4. Per ogni risposta assegna source_role, usable_for_comparison, certainty e reason.
5. Esegui un controllo finale: se il valore è medio, di esempio, riferito al periodo o non dimostrato, usable_for_comparison deve essere false.

REGOLE GENERALI
- Analizza tutte le pagine come un unico documento multipagina e tratta luce e gas separatamente.
- Non inventare, non stimare, non annualizzare e non completare dati mancanti.
- evidence deve riportare la riga o le righe contigue che provano il valore. Non inserire nel label il nome tecnico della domanda: usa l'etichetta realmente stampata.
- certainty=certain solo per un valore letterale dimostrato; derived solo per un calcolo esplicito e verificabile; review per un dato plausibile ma non sufficiente; not_available quando found=false.
- source_role: identity per nomi/codici; customer_data per dati del punto di fornitura; annual_total solo per un vero totale di 12 mesi; period_total per mese/bimestre/periodo/da inizio fornitura; contract_term per condizioni economiche contrattuali; billed_rate per prezzi applicati nel periodo; average_cost per costi medi; regulated per rete/oneri/imposte; example per esempi, simulazioni o stime; unknown negli altri casi.

CONSUMI
- Un consumo è annuale solo se copre realmente 12 mesi completi. La sola intestazione “ultimi 12 mesi” non basta se la riga dice “da inizio fornitura” o se sono elencati meno di 12 mesi.
- Non usare consumi fatturati, del periodo, del mese, letture, stime, proiezioni o singole fasce come consumo annuo.
- coverage_months deve indicare i mesi effettivamente dimostrati; usa null se non determinabili.

PREZZI E COSTI
- Per prezzo luce/gas usa soltanto una condizione commerciale contrattuale. Rifiuta costo medio, costo unitario della materia, costo dell'intera bolletta, spesa annua stimata, esempi di consumo, importi totali, rete, oneri e imposte.
- Se una bolletta mostra prezzi F1/F2/F3 del solo periodo, classificali billed_rate e non trasformarli in un singolo prezzo contrattuale.
...
- Per la quota fissa accetta period=month o year soltanto quando la stessa evidenza contiene chiaramente €/mese, mensile, €/anno o annuale. Non dedurre la periodicità dalla quantità 1.

SCHEDE SINTETICHE E CONDIZIONI ECONOMICHE
- Riporta sempre, quando presenti: venditore, nome esatto offerta, codice offerta, commodity, destinatari domestico/business, tipo prezzo, prezzo commerciale, quota fissa commerciale, indice, spread, formula, periodicità e validità.
- Non usare le tabelle di “spesa annua stimata”, gli esempi per profili di consumo, i valori storici dell'indice, i costi di rinnovo futuro o i servizi regolati come condizioni dell'offerta corrente.
- Il prezzo e la quota fissa devono provenire dalla sezione delle condizioni economiche/corrispettivi definiti dal venditore. Per una scheda, source_role deve essere contract_term e usable_for_comparison=true soltanto se la riga è inequivocabile.
- I dati della scheda descrivono esclusivamente una nuova offerta: non cercare POD, PDR, codice cliente o consumo dell'utente.

CODICI E PROFILO
- POD, PDR, codice fiscale, codice cliente e codice offerta devono essere copiati carattere per carattere.
- La partita IVA dell'intestatario non va confusa con quella del venditore.
- Non classificare business solo perché compare la partita IVA del venditore.

Restituisci soltanto JSON conforme allo schema.`;

const USER_PROMPT = `Esamina il documento seguendo la sequenza e rispondi alle domande seguenti. Per ogni valore spiega nel campo reason perché è o non è utilizzabile nel confronto. Se manca una prova sufficiente usa found=false, usable_for_comparison=false e certainty=not_available.
...
export async function buildPdfPureAiRequest({
  ...
  return {
    ...
    text: {
      format: {
        type: "json_schema",
        name: "offertalogica_pure_ai_document",
        description: "Risposte verificate a domande mirate su bollette e schede sintetiche italiane",
        strict: true,
...
function answerSemanticText(answer) {
  // Non includere il label tecnico nella prova semantica: in passato il nome
  // della domanda "prezzo_*" creava un falso riscontro positivo.
  return semanticText([answer?.evidence, answer?.value_text, answer?.unit].filter(Boolean).join(" | "));
}
...
function semanticValidation(spec, answer, document = {}) {
  ...
  if (comparisonCriticalField(spec.field) && !usable) return { accepted: false, reason: "semantic_not_usable_for_comparison" };
...
function diagnosticFor(spec, answer, normalized) {
  return {
    ...
    verification_reason: compact(answer.reason, 300) || null,
    coverage_months: Number.isInteger(Number(answer.coverage_months)) ? Number(answer.coverage_months) : null,
...
export function normalizePureAiOutput(parsed, {
  ...
  normalized.ai = {
    ...
    verification_protocol: "targeted_questions_v1",
  };
