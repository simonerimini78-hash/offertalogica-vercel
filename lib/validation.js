...
export function sanitizeLead(input) {
  ...
  const requestType = String(input?.calculation?.requestType || "").trim().toLowerCase();
  const emailOptional = requestType === "photovoltaic_consulting";

  if (!name) throw new Error("Nome obbligatorio");
  if ((!emailOptional || email) && !validEmail(email)) throw new Error("Email non valida");
...
function sanitizePhotovoltaicProfile(input = {}) {
  if (!input || typeof input !== "object" || Array.isArray(input)) return null;
  const ownership = limitedString(input.ownership, 30).toLowerCase();
  const timeframe = limitedString(input.timeframe, 30).toLowerCase();
  const source = limitedString(input.source, 40).toLowerCase();
  const usageProfile = limitedString(input.usageProfile, 40).toLowerCase();
  const confidence = limitedString(input.confidence, 30).toLowerCase();
  return {
    source: ["quick", "detailed"].includes(source) ? source : "unknown",
    ownership: ["owner", "buying", "not_owner", "unknown"].includes(ownership) ? ownership : "unknown",
    timeframe: ["0_3", "3_6", "6_12", "exploring"].includes(timeframe) ? timeframe : "",
    place: limitedString(input.place, 160),
    lat: finiteOrNull(input.lat),
    lon: finiteOrNull(input.lon),
    annualConsumptionKwh: finiteOrNull(input.annualConsumptionKwh),
    usageProfile: ["low", "medium", "high", "continuous", "unknown", "bands", "declared"].includes(usageProfile) ? usageProfile : "unknown",
    scenarioPowerKw: finiteOrNull(input.scenarioPowerKw),
    annualProductionKwh: finiteOrNull(input.annualProductionKwh),
    selfConsumptionKwh: finiteOrNull(input.selfConsumptionKwh),
    residualGridKwh: finiteOrNull(input.residualGridKwh),
    excessKwh: finiteOrNull(input.excessKwh),
    coveragePct: finiteOrNull(input.coveragePct),
    assessment: limitedString(input.assessment, 80),
    assessmentLabel: limitedString(input.assessmentLabel, 220),
    confidence: ["low", "medium", "good"].includes(confidence) ? confidence : "",
    evaluatedAt: limitedString(input.evaluatedAt, 40),
  };
}
...
export function sanitizeLeadCalculation(input) {
  ...
  return {
    ...
    photovoltaicProfile: sanitizePhotovoltaicProfile(input.photovoltaicProfile),
    requestType: ["assistance_callback", "photovoltaic_consulting"].includes(requestType) ? requestType : "comparison",
