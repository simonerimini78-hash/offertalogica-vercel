...
import { pdfAiMode, runPdfAiFallback } from "./pdfAiReader.js";
import {
  canonicalPdfField,
...
export const PDF_AI_FALLBACK_PIPELINE_VERSION = "v106.8.8.9-consolidated-complete-1";
...
const DEDICATED_ECONOMIC_SOURCE_PATTERN = /economic-recovery(?:-inventory)?/i;
const FIXED_SALES_LABEL_PATTERN = /\b(?:quota\s+fissa|commercializzazione|ccv|pcv|pfix)\b/i;
const TOTAL_OR_AVERAGE_CONSUMPTION_PATTERN = /\b(?:prezzo\s+medio|costo\s+medio|spesa\s+media|quota\s+(?:per\s+)?consumi(?:\s+totale)?|totale\s+(?:quota\s+)?consumi|costo\s+unitario\s+totale)\b/i;
...
const SALES_VARIABLE_COMPONENT_PATTERN = /\b(?:di\s+cui\s+)?(?:spesa|quota|componente|corrispettivo)\s+(?:variabile\s+)?(?:per\s+)?(?:la\s+)?vendita\s+(?:di\s+)?(?:energia\s+elettrica|gas\s+naturale)|\b(?:spesa\s+per\s+la\s+vendita|materia\s+energia|materia\s+prima\s+gas|corrispettivo\s+energia|componente\s+energia)\b/i;
...
const EXCLUDED_ECONOMIC_COMPONENT_PATTERN = /\b(?:rete|oneri|trasporto|distribuzione|contatore|potenza|imposte?|accise|iva|canone|acqua|servizi?)\b/i;
...
function isDedicatedEconomicCandidate(candidate) {
  return DEDICATED_ECONOMIC_SOURCE_PATTERN.test(`${candidate?.source_version || ""} ${candidate?.method || ""}`);
}

function fixedSalesEvidenceAllowed(candidate) {
  const evidence = literalEvidence(candidate);
  const context = `${candidate?.label || ""} ${evidence}`;
  return FIXED_SALES_LABEL_PATTERN.test(context)
    && !EXCLUDED_ECONOMIC_COMPONENT_PATTERN.test(context.replace(/quota\s+fissa/ig, ""))
    && (SALES_FIXED_COMPONENT_PATTERN.test(context)
      || (isDedicatedEconomicCandidate(candidate) && candidate?.semantic_role === "sales_component"));
}
...
function candidatePriority(entry) {
  ...
  if (isDedicatedEconomicCandidate(candidate)) score += 16;
...
function normalizeNumericCandidate(field, candidate) {
  ...
  if (["quota_fissa_vendita_luce_eur_anno", "quota_fissa_vendita_gas_eur_anno"].includes(field)) {
    if (!fixedSalesEvidenceAllowed(candidate)) return null;
...
function canonicalizeCandidate(input) {
  ...
  if (["quota_fissa_vendita_luce_eur_anno", "quota_fissa_vendita_gas_eur_anno"].includes(candidate.field)
    && fixedSalesEvidenceAllowed(candidate)) {
...
function effectiveConfidenceThreshold(candidate) {
  ...
  if (["prezzo_luce_eur_kwh", "prezzo_gas_eur_smc"].includes(field)
    && SALES_VARIABLE_COMPONENT_PATTERN.test(evidence)
    && !EXCLUDED_ECONOMIC_COMPONENT_PATTERN.test(evidence)) {
    return isDedicatedEconomicCandidate(candidate) ? 85 : 90;
  }
  if (["quota_fissa_vendita_luce_eur_anno", "quota_fissa_vendita_gas_eur_anno"].includes(field)
    && fixedSalesEvidenceAllowed(candidate)
    && (MONTHLY_MONEY_UNIT_PATTERN.test(unit) || ANNUAL_MONEY_UNIT_PATTERN.test(unit))) {
    return isDedicatedEconomicCandidate(candidate) ? 85 : 90;
  }
...
function candidateRejection(candidate, aiConflictFields) {
  ...
  if (["prezzo_luce_eur_kwh", "prezzo_gas_eur_smc"].includes(candidate.field)) {
    const explicitSalesRow = SALES_VARIABLE_COMPONENT_PATTERN.test(visualContext);
    if (AVERAGE_UNIT_COST_LABEL_PATTERN.test(visualContext)) return "average_unit_cost_not_contract_price";
    if (TOTAL_OR_AVERAGE_CONSUMPTION_PATTERN.test(visualContext)
      && !(isDedicatedEconomicCandidate(candidate) && explicitSalesRow)) {
...
function candidateDiagnostic(candidate, value) {
  return {
    ...
    method: isDedicatedEconomicCandidate(candidate)
      ? "openai_visual_economic_recovery"
      : /critical_recovery/i.test(String(candidate.method || ""))
        ? "openai_visual_critical_recovery"
        : "openai_visual_consolidated",
  };
}
...
export function mergeCompletedPdfAiForReview(base, ai, policy = { reason: "unrecognized_after_parser_and_ocr" }) {
  ...
  const material = selected.length >= 2
    && selected.some((item) => HIGH_VALUE_FIELDS.has(item.field) || ECONOMIC_FIELDS.has(item.field))
  ...
  merged.ai = {
    ...
    economic_recovery_sales_rows: Number(ai.economic_recovery_sales_rows || 0),
    economic_recovery_candidates: Number(ai.economic_recovery_candidates || 0),
    economic_recovery_failures: Array.isArray(ai.economic_recovery_failures) ? ai.economic_recovery_failures : [],
