...
import { runPdfAiFallbackImages } from "./pdfAiRasterBatchedReader.js";
import {
  canonicalPdfField,
...
export const PDF_AI_FALLBACK_PIPELINE_VERSION = "v106.8.8.5-batched-raster-1";
...
const AVERAGE_UNIT_COST_LABEL_PATTERN = /\b(?:prezzo\s+medio|costo\s+medio|spesa\s+media|costo|spesa)\s+medio(?:a)?\s+unitari[oa]?|\baverage\s+(?:unit\s+)?(?:price|cost)\b/i;
...
function evidenceSignalsAnnual(candidate) {
  ...
  return ANNUAL_CONSUMPTION_LABEL_PATTERN.test(context)
    || /\b(?:anno|annuo|annua|annuale|12\s*mesi|year|annual)\b/i.test(context);
}
...
function normalizeNumericCandidate(field, candidate) {
  ...
  const context = `${explicitUnit} ${candidate.label || ""} ${candidate.evidence || ""}`;
...
function canonicalizeCandidate(input) {
  ...
  if (["codice_offerta", "codice_offerta_luce", "codice_offerta_gas"].includes(candidate.field)) {
    if (SUPPLIER_PRODUCT_CODE_LABEL_PATTERN.test(context) && !OFFICIAL_OFFER_CODE_LABEL_PATTERN.test(context)) {
      const commodity = candidate.field.endsWith("_luce") || candidate.commodity === "luce"
        ? "luce"
        : candidate.field.endsWith("_gas") || candidate.commodity === "gas"
          ? "gas"
          : null;
    ...
    } else if (OFFICIAL_OFFER_CODE_LABEL_PATTERN.test(context)) {
      candidate.semantic_role = "identifier";
    }
  }

  if (DATE_FIELDS.has(candidate.field) && ECONOMIC_DATE_LABEL_PATTERN.test(context)) {
    candidate.semantic_role = "contract_period";
  }
  if (candidate.field === "fornitore" && /\b(?:fornitore|supplier|logo|marchio)\b/i.test(context)) {
    candidate.semantic_role = "classification";
  }
  if (["kind", "commodity", "customer_type"].includes(candidate.field)) {
    candidate.semantic_role = "classification";
  }
  if (candidate.field === "pod" && EXPLICIT_POD_LABEL_PATTERN.test(context)) candidate.semantic_role = "identifier";
  if (candidate.field === "pdr" && EXPLICIT_PDR_LABEL_PATTERN.test(context)) candidate.semantic_role = "identifier";
  if (candidate.field === "codice_fiscale" && EXPLICIT_TAX_LABEL_PATTERN.test(context)) candidate.semantic_role = "identifier";
  if (candidate.field === "codice_cliente" && EXPLICIT_CUSTOMER_CODE_PATTERN.test(context)) candidate.semantic_role = "identifier";
  if (["consumo_luce_kwh", "consumo_gas_smc"].includes(candidate.field)
    && evidenceSignalsAnnual(candidate)
    && !BILLED_PERIOD_CONSUMPTION_LABEL_PATTERN.test(context)) {
...
function effectiveConfidenceThreshold(candidate) {
  ...
  if (field === "pod" && EXPLICIT_POD_LABEL_PATTERN.test(context) && normalizeIdentifier(field, candidate.normalized_value)) return 90;
  if (field === "pdr" && EXPLICIT_PDR_LABEL_PATTERN.test(context) && normalizeIdentifier(field, candidate.normalized_value)) return 90;
  if (field === "codice_fiscale" && EXPLICIT_TAX_LABEL_PATTERN.test(context) && normalizeIdentifier(field, candidate.normalized_value)) return 90;
  if (field === "codice_cliente" && EXPLICIT_CUSTOMER_CODE_PATTERN.test(context) && normalizeIdentifier(field, candidate.normalized_value)) return 88;
  if (["codice_offerta", "codice_offerta_luce", "codice_offerta_gas"].includes(field)
    && OFFICIAL_OFFER_CODE_LABEL_PATTERN.test(context)
    && normalizeIdentifier(field, candidate.normalized_value)) return 85;
  if (DATE_FIELDS.has(field) && ECONOMIC_DATE_LABEL_PATTERN.test(context) && normalizeDate(candidate.normalized_value)) return 80;
...
function candidateRejection(candidate, aiConflictFields) {
  ...
  if (["prezzo_luce_eur_kwh", "prezzo_gas_eur_smc"].includes(candidate.field)
    && (candidate.semantic_role === "billing_period" || AVERAGE_UNIT_COST_LABEL_PATTERN.test(visualContext))) {
...
function candidateDiagnostic(candidate, value) {
  return {
    ...
    method: "openai_visual_semantic_acceptance_fix",
  };
}
...
export function mergeCompletedPdfAiForReview(base, ai, policy = { reason: "unrecognized_after_parser_and_ocr" }) {
  ...
  for (const [field, entries] of grouped) {
    ...
    const best = [...values.values()][0].sort((a, b) => Number(b.candidate.confidence) - Number(a.candidate.confidence))[0];
