...
export const PDF_AI_RASTER_BATCH_VERSION = "2.4.9.1-economic-activation";


const CRITICAL_RECOVERY_FIELDS = Object.freeze([
  "fornitore", "kind", "commodity", "customer_type",
  "intestatario", "codice_fiscale", "codice_cliente",
  "pod", "pdr", "indirizzo_fornitura", "indirizzo_fornitura_luce", "indirizzo_fornitura_gas",
  ...
  "codice_prodotto_fornitore_luce", "codice_prodotto_fornitore_gas",
    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          ...
          semantic_role: { type: "string", enum: ["actual_customer_value", "offer_value", "billing_period", "contract_period", "identifier", "classification", "unknown"] },
...
const CRITICAL_RECOVERY_PROMPT = `You are a precision transcription pass for Italian electricity and gas bills.
Read only the attached pages. Preserve the existing critical-field recovery and additionally recover the economic core needed for comparison.

Rules for existing critical fields:
- Copy exact visible characters. Never repair a value by arithmetic or by combining two different rows.
- POD: return only a clearly labelled POD/punto di prelievo and re-check every character.
- PDR: return only a clearly labelled PDR/punto di riconsegna and re-check all 14 digits.
- Tax ID: return only a clearly labelled codice fiscale/P.IVA and re-check all characters.
- Annual consumption: return only a value whose same visible line or box literally says Consumo annuo, Consumo annuale, ultimi 12 mesi or equivalent. Never return billed-period consumption as annual.
- "Codice offerta" is an official offer code. "Codice prodotto" is a supplier product code. Do not swap them.
- Return an offer name only when the page explicitly identifies it as the offer/product name.

Rules for comparable economic fields:
- Electricity unit price: return prezzo_luce_eur_kwh only from the exact sales-component row, such as "di cui spesa per la vendita di energia elettrica", "materia energia" or an explicit contractual offer price. Do not use the total quota per consumi.
- Gas unit price: return prezzo_gas_eur_smc only from the exact sales-component row, such as "di cui spesa per la vendita di gas naturale", "materia prima gas" or an explicit contractual offer price. Do not use the total quota per consumi.
- Never return a row labelled prezzo medio/costo medio, rete, oneri, trasporto, distribuzione, contatore, potenza, imposte, accise or IVA as a comparable sales price.
- Fixed sales fee: under the quota fissa/commercializzazione section, return only the sales row (vendita energia elettrica or vendita gas naturale), never the network row and never quota potenza.
- For a monthly fixed sales fee, keep the visible monthly number in value_number and use unit EUR/mese. Do not multiply it; the application performs the transparent x12 conversion and keeps the original value for review.
- Return indice_riferimento_luce/gas, periodicita_aggiornamento_indice_luce/gas, formula_prezzo_luce/gas and spread only when explicitly visible. Never invent a spread by subtracting bill values.
- If an explicit PUN/PSV/index formula is visible, tipo_prezzo_luce/gas may be variabile; otherwise omit the type rather than guessing.
- Economic-condition dates are secondary. Return them only when their literal evidence includes condizioni economiche, validità, decorrenza or scadenza.
- Evidence must quote the visible section, row label, value and unit, not a paraphrase.
...
async function buildCriticalRecoveryRequest({ imageFiles, filename, model, pageCount }) {
  ...
  content.push({
    type: "input_text",
    text: `Original file: ${filename || "documento.pdf"}. Recover only the critical fields explicitly visible on these pages.`
  });
  return {
    model,
    store: false,
  ...
  try {
    ...
    const normalized = {
      ...
      page_map: imageFiles.map((item) => ({
        ...
        summary: "Focused precision pass for identifiers, annual consumption and comparable economic sales components."
      })),
      conflicts: [],
      review_reasons: ["Focused critical and economic-field recovery executed after the general visual pass."]
    };
...
export async function runPdfAiFallbackImages(options = {}) {
  ...
  const focusedTimeoutMs = Math.min(14_000, focusedBudget);
