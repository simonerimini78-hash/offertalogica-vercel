...
export async function archivePdfAnalysis({
  ...
  const record = {
    ...
    source_context: {
      ...
      archiveSource: String(context?.archiveSource || "original_pdf").slice(0, 80),
      originalBytes: Number(context?.originalBytes || 0) || null,
      rasterBytes: Number(context?.rasterBytes || 0) || null,
      sourcePageCount: Number(context?.sourcePageCount || 0) || null,
    },
...
export async function updatePdfAnalysisRuntime(id, { normalized = null, shadow = null, error = null } = {}) {
  if (!id) return null;
  const config = staffConfig();
