...
function aiShadowNeedsArchive(aiShadow) {
  if (!aiShadow || typeof aiShadow !== "object") return false;
  if (["error", "timeout"].includes(aiShadow.status)) return true;
  if (aiShadow.status !== "observed") return false;
  const summary = aiShadow?.observation?.review_plan?.summary || {};
  return [
    "review_field_count",
    "conflict_count",
...
export function shouldArchivePdf({ normalized, shadow, aiShadow, error, mode = pdfArchiveConfig().mode } = {}) {
  if (mode === "all") return true;
  if (mode !== "problematic") return false;
  return normalizedStatus(normalized, error) !== "complete"
    || shadowNeedsArchive(shadow)
    || aiShadowNeedsArchive(aiShadow);
}

export function buildArchivedNormalizedData(normalized, shadow, aiShadow) {
  const base = normalized && typeof normalized === "object" ? { ...normalized } : {};
  if (shadow?.enabled === true) base._reader_shadow = shadow;
  if (aiShadow && typeof aiShadow === "object") base._ai_shadow = aiShadow;
  return base;
}
...
async function supabaseFetch(path, init = {}, config = pdfArchiveConfig()) {
...
async function supabaseRequest(path, init = {}, config = pdfArchiveConfig()) {
  const response = await supabaseFetch(path, init, config);
  if (response.status === 204) return null;
  const contentType = response.headers.get("content-type") || "";
  return contentType.includes("application/json") ? response.json() : response.text();
}

async function supabaseBufferRequest(path, init = {}, config = pdfArchiveConfig()) {
  const response = await supabaseFetch(path, init, config);
...
const TEMPORARY_PDF_PREFIX = "incoming-preview";
const TEMPORARY_PDF_UPLOAD_ID = /^[a-f0-9-]{20,80}$/i;
const MAX_TEMPORARY_PDF_CHUNKS = 12;

function normalizedTemporaryUploadId(value) {
  const uploadId = String(value || "").trim();
  if (!TEMPORARY_PDF_UPLOAD_ID.test(uploadId)) throw new Error("temporary_upload_id_invalid");
  return uploadId;
}

function normalizedTemporaryChunkCount(value) {
  const count = Number(value);
  if (!Number.isInteger(count) || count < 1 || count > MAX_TEMPORARY_PDF_CHUNKS) {
    throw new Error("temporary_chunk_count_invalid");
  }
  return count;
}

function normalizedTemporaryChunkIndex(value, chunkCount) {
  const index = Number(value);
  if (!Number.isInteger(index) || index < 0 || index >= chunkCount) {
    throw new Error("temporary_chunk_index_invalid");
  }
  return index;
}

function temporaryPdfChunkPath(uploadId, chunkIndex) {
  return `${TEMPORARY_PDF_PREFIX}/${uploadId}/${String(chunkIndex).padStart(3, "0")}.part`;
}

export function temporaryPdfUploadDescriptor({ uploadId, chunkIndex = 0, chunkCount = 1 } = {}) {
  const safeUploadId = normalizedTemporaryUploadId(uploadId);
  const safeChunkCount = normalizedTemporaryChunkCount(chunkCount);
  const safeChunkIndex = normalizedTemporaryChunkIndex(chunkIndex, safeChunkCount);
  return Object.freeze({
    uploadId: safeUploadId,
    chunkIndex: safeChunkIndex,
    chunkCount: safeChunkCount,
    storagePath: temporaryPdfChunkPath(safeUploadId, safeChunkIndex),
  });
}

export async function storeTemporaryPdfChunk({ uploadId, chunkIndex, chunkCount, buffer } = {}) {
  const config = pdfArchiveConfig();
  const descriptor = temporaryPdfUploadDescriptor({ uploadId, chunkIndex, chunkCount });
  const bytes = Buffer.isBuffer(buffer) ? buffer : Buffer.from(buffer || []);
  if (!bytes.length || bytes.length > 3_200_000) throw new Error("temporary_chunk_size_invalid");
  await supabaseRequest(
    `/storage/v1/object/${encodeURIComponent(config.bucket)}/${encodeStoragePath(descriptor.storagePath)}`,
    {
      method: "POST",
      headers: {
        // Il bucket privato è configurato per accettare soltanto PDF. I blocchi
        // sono porzioni dello stesso PDF e devono quindi mantenere il MIME PDF;
        // application/octet-stream viene respinto da allowed_mime_types.
  ...
  return { stored: true, ...descriptor, bytes: bytes.length };
}

export async function assembleTemporaryPdfUpload({ uploadId, chunkCount, expectedBytes = 0 } = {}) {
  const config = pdfArchiveConfig();
  const safeUploadId = normalizedTemporaryUploadId(uploadId);
  const safeChunkCount = normalizedTemporaryChunkCount(chunkCount);
  const chunks = [];
  ...
  for (let index = 0; index < safeChunkCount; index += 1) {
    const storagePath = temporaryPdfChunkPath(safeUploadId, index);
    const chunk = await supabaseBufferRequest(
      `/storage/v1/object/authenticated/${encodeURIComponent(config.bucket)}/${encodeStoragePath(storagePath)}`,
      { method: "GET" },
      config,
    );
    if (!chunk.length) throw new Error("temporary_chunk_missing");
    totalBytes += chunk.length;
    chunks.push(chunk);
  }
  const expected = Number(expectedBytes || 0);
  if (Number.isFinite(expected) && expected > 0 && totalBytes !== expected) {
    throw new Error("temporary_upload_size_mismatch");
  }
  return Buffer.concat(chunks, totalBytes);
}

export async function deleteTemporaryPdfUpload({ uploadId, chunkCount } = {}) {
  const config = pdfArchiveConfig();
  const safeUploadId = normalizedTemporaryUploadId(uploadId);
  const safeChunkCount = normalizedTemporaryChunkCount(chunkCount);
  const prefixes = Array.from({ length: safeChunkCount }, (_, index) => temporaryPdfChunkPath(safeUploadId, index));
  await supabaseRequest(
  ...
  return { deleted: true, count: prefixes.length };
}
...
export async function archivePdfAnalysis({
  ...
  if (!shouldArchivePdf({ normalized, shadow, aiShadow, error, mode: config.mode })) return { stored: false, reason: "mode_filtered" };
  ...
  const record = {
    ...
    normalized_data: buildArchivedNormalizedData(normalized, shadow, aiShadow),
