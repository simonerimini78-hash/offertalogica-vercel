...
const DEFAULT_MAX_PDF_BYTES = 8_000_000;
...
function parseUploadTicket(ticket, config = pdfArchiveConfig(), { allowExpired = false, now = Date.now() } = {}) {
  ...
  if (!Number.isFinite(fileSize) || fileSize <= 0 || fileSize > configuredMaxPdfBytes()) {
    throw new Error("pdf_upload_invalid_ticket");
  }
...
export async function createPdfDirectUpload({
  ...
  const maxFileSize = configuredMaxPdfBytes();
  if (!Number.isFinite(size) || size <= 0) throw new Error("pdf_upload_invalid_size");
  if (size > maxFileSize) throw new Error("pdf_upload_too_large");
...
export async function downloadPdfDirectUpload({ ticket, destinationPath, now = Date.now() } = {}) {
  ...
  if (declaredLength > configuredMaxPdfBytes()) throw new Error("pdf_upload_too_large");
  const buffer = Buffer.from(await response.arrayBuffer());
  if (buffer.length > configuredMaxPdfBytes()) throw new Error("pdf_upload_too_large");
