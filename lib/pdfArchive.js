import crypto from "node:crypto";
import fs from "node:fs/promises";

const DEFAULT_BUCKET = "pdf-test-archive";
const DEFAULT_RETENTION_DAYS = 180;
const DEFAULT_MAX_PDF_BYTES = 20_000_000;
const PDF_UPLOAD_TICKET_VERSION = 1;
const PDF_UPLOAD_TTL_SECONDS = 2 * 60 * 60;
const ACCEPTED_PDF_MIME_TYPES = new Set([
  "application/pdf",
  "application/x-pdf",
  "application/octet-stream",
]);

function boundedRequestTimeout(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed > 0 ? Math.max(500, Math.min(15_000, parsed)) : null;
}

async function fetchWithTimeout(url, init = {}, { timeoutMs = null, timeoutCode = "pdf_archive_request_timeout" } = {}) {
  const safeTimeoutMs = boundedRequestTimeout(timeoutMs);
  if (safeTimeoutMs === null) return fetch(url, init);
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), safeTimeoutMs);
  try {
    return await fetch(url, { ...init, signal: controller.signal });
  } catch (error) {
    if (controller.signal.aborted) throw new Error(timeoutCode);
    throw error;
  } finally {
    clearTimeout(timeoutId);
  }
}

function cleanEnv(value) {
  return String(value || "").trim().replace(/\/+$/g, "");
}

export function pdfArchiveConfig() {
  const mode = String(process.env.PDF_ARCHIVE_MODE || "off").trim().toLowerCase();
  const requestedRetention = Number(process.env.PDF_ARCHIVE_RETENTION_DAYS || DEFAULT_RETENTION_DAYS);
  const retentionDays = Number.isFinite(requestedRetention)
    ? Math.max(1, Math.min(3650, requestedRetention))
    : DEFAULT_RETENTION_DAYS;
  return {
    mode: ["off", "all", "problematic"].includes(mode) ? mode : "off",
    bucket: String(process.env.PDF_ARCHIVE_BUCKET || DEFAULT_BUCKET).trim(),
    retentionDays,
    supabaseUrl: cleanEnv(process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL),
    serviceRoleKey: cleanEnv(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY),
    uploadTicketSecret: String(process.env.PDF_UPLOAD_TICKET_SECRET || "").trim(),
  };
}

function archiveConfigured(config = pdfArchiveConfig()) {
  return Boolean(config.mode !== "off" && config.bucket && config.supabaseUrl && config.serviceRoleKey);
}

export function pdfArchiveConfigured(config = pdfArchiveConfig()) {
  return archiveConfigured(config);
}


export function pdfMaxBytes() {
  const parsed = Number(process.env.MAX_PDF_BYTES || DEFAULT_MAX_PDF_BYTES);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : DEFAULT_MAX_PDF_BYTES;
}

function pdfUploadTooLargeError(actualBytes, maxBytes = pdfMaxBytes()) {
  const error = new Error("pdf_upload_too_large");
  error.actualBytes = Number(actualBytes || 0);
  error.maxBytes = Number(maxBytes || 0);
  return error;
}

function storageConfigured(config = pdfArchiveConfig()) {
  return Boolean(config.bucket && config.supabaseUrl && config.serviceRoleKey);
}

function requireStorageConfig(config = pdfArchiveConfig()) {
  if (!storageConfigured(config)) throw new Error("pdf_upload_not_configured");
  return { ...config, mode: config.mode === "off" ? "all" : config.mode };
}

function requireUploadTicketSecret(config = pdfArchiveConfig()) {
  const secret = String(config.uploadTicketSecret || "").trim();
  if (Buffer.byteLength(secret, "utf8") < 32) throw new Error("pdf_upload_ticket_secret_not_configured");
  return secret;
}

function base64UrlJson(value) {
  return Buffer.from(JSON.stringify(value), "utf8").toString("base64url");
}

function uploadTicketSignature(encodedPayload, secret) {
  return crypto
    .createHmac("sha256", secret)
    .update(`offertalogica-pdf-upload:${encodedPayload}`)
    .digest("base64url");
}

function createUploadTicket(payload, secret) {
  const encoded = base64UrlJson(payload);
  return `${encoded}.${uploadTicketSignature(encoded, secret)}`;
}

function parseUploadTicket(ticket, config = pdfArchiveConfig(), { allowExpired = false, now = Date.now() } = {}) {
  const safeConfig = requireStorageConfig(config);
  const [encoded, signature, extra] = String(ticket || "").split(".");
  if (!encoded || !signature || extra) throw new Error("pdf_upload_invalid_ticket");
  const expected = Buffer.from(uploadTicketSignature(encoded, requireUploadTicketSecret(safeConfig)), "utf8");
  const actual = Buffer.from(signature, "utf8");
  if (expected.length !== actual.length || !crypto.timingSafeEqual(expected, actual)) {
    throw new Error("pdf_upload_invalid_ticket");
  }
  let payload;
  try {
    payload = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
  } catch {
    throw new Error("pdf_upload_invalid_ticket");
  }
  if (payload?.v !== PDF_UPLOAD_TICKET_VERSION) throw new Error("pdf_upload_invalid_ticket");
  if (!/^pending\/\d{4}\/\d{2}\/[0-9a-f-]{36}\.pdf$/i.test(String(payload.path || ""))) {
    throw new Error("pdf_upload_invalid_ticket");
  }
  if (!allowExpired && Number(payload.exp || 0) <= Number(now)) throw new Error("pdf_upload_expired");
  const fileSize = Number(payload.size || 0);
  if (!Number.isFinite(fileSize) || fileSize <= 0 || fileSize > pdfMaxBytes()) {
    throw pdfUploadTooLargeError(fileSize, pdfMaxBytes());
  }
  return { ...payload, size: fileSize, config: safeConfig };
}

function directUploadStoragePath(now = new Date()) {
  const year = String(now.getUTCFullYear());
  const month = String(now.getUTCMonth() + 1).padStart(2, "0");
  return `pending/${year}/${month}/${crypto.randomUUID()}.pdf`;
}

function signedStorageUrl(value, config) {
  const raw = String(value || "");
  if (!raw) throw new Error("pdf_upload_sign_failed");
  if (/^https?:\/\//i.test(raw)) return raw;
  return `${config.supabaseUrl}/storage/v1${raw.startsWith("/") ? "" : "/"}${raw}`;
}

export async function createPdfDirectUpload({
  originalFilename = "documento.pdf",
  mimeType = "application/pdf",
  fileSize,
  now = Date.now(),
  requestTimeoutMs = null,
} = {}) {
  const config = requireStorageConfig();
  const uploadTicketSecret = requireUploadTicketSecret(config);
  const size = Number(fileSize || 0);
  const maxFileSize = pdfMaxBytes();
  if (!Number.isFinite(size) || size <= 0) throw new Error("pdf_upload_invalid_size");
  if (size > maxFileSize) throw pdfUploadTooLargeError(size, maxFileSize);
  const normalizedMime = String(mimeType || "application/pdf").toLowerCase();
  if (!ACCEPTED_PDF_MIME_TYPES.has(normalizedMime)) throw new Error("pdf_upload_invalid_mime");
  const storagePath = directUploadStoragePath(new Date(now));
  const result = await supabaseRequest(
    `/storage/v1/object/upload/sign/${encodeURIComponent(config.bucket)}/${encodeStoragePath(storagePath)}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-upsert": "false",
      },
      body: "{}",
    },
    config,
    { timeoutMs: requestTimeoutMs, timeoutCode: "pdf_upload_sign_timeout" },
  );
  const expiresAt = Number(now) + PDF_UPLOAD_TTL_SECONDS * 1000;
  const payload = {
    v: PDF_UPLOAD_TICKET_VERSION,
    path: storagePath,
    name: safeFileName(originalFilename),
    mime: normalizedMime,
    size,
    exp: expiresAt,
  };
  return {
    uploadUrl: signedStorageUrl(result?.signedURL || result?.signedUrl || result?.url, config),
    uploadTicket: createUploadTicket(payload, uploadTicketSecret),
    expiresAt: new Date(expiresAt).toISOString(),
    maxFileSize,
  };
}

export async function downloadPdfDirectUpload({
  ticket,
  destinationPath,
  now = Date.now(),
  requestTimeoutMs = null,
} = {}) {
  if (!destinationPath) throw new Error("pdf_upload_destination_required");
  const payload = parseUploadTicket(ticket, pdfArchiveConfig(), { now });
  const response = await fetchWithTimeout(
    `${payload.config.supabaseUrl}/storage/v1/object/${encodeURIComponent(payload.config.bucket)}/${encodeStoragePath(payload.path)}`,
    { method: "GET", headers: supabaseHeaders(payload.config), cache: "no-store" },
    { timeoutMs: requestTimeoutMs, timeoutCode: "pdf_upload_download_timeout" },
  );
  if (response.status === 404) throw new Error("pdf_upload_missing");
  if (!response.ok) throw new Error(`pdf_upload_download_failed:${response.status}`);
  const declaredLength = Number(response.headers.get("content-length") || 0);
  if (declaredLength > pdfMaxBytes()) throw pdfUploadTooLargeError(declaredLength, pdfMaxBytes());
  const buffer = Buffer.from(await response.arrayBuffer());
  if (buffer.length > pdfMaxBytes()) throw pdfUploadTooLargeError(buffer.length, pdfMaxBytes());
  if (buffer.length !== payload.size) throw new Error("pdf_upload_size_mismatch");
  await fs.writeFile(destinationPath, buffer, { flag: "wx" });
  return {
    storagePath: payload.path,
    originalFilename: payload.name,
    mimeType: payload.mime,
    fileSize: payload.size,
  };
}

export async function deletePdfDirectUpload(ticket, { requestTimeoutMs = null } = {}) {
  const payload = parseUploadTicket(ticket, pdfArchiveConfig(), { allowExpired: true });
  await deleteStorageObject(payload.path, payload.config, { requestTimeoutMs, timeoutCode: "pdf_upload_cleanup_timeout" });
  return { deleted: true };
}

function normalizedStatus(normalized, error) {
  if (error) return "failed";
  if (!normalized?.recognized) return "unrecognized";
  const diagnostics = Array.isArray(normalized?.diagnostics) ? normalized.diagnostics : [];
  const missingCount = diagnostics.filter((item) => item.status === "missing").length;
  const reviewCount = diagnostics.filter((item) => item.status === "review").length;
  if (normalized?.needsReview || (normalized?.warnings || []).length > 0 || reviewCount > 0 || missingCount > 0) return "partial";
  return "complete";
}

function shadowNeedsArchive(shadow) {
  if (shadow?.enabled !== true) return false;
  if (shadow.error) return true;
  if (["failed", "unavailable", "skipped"].includes(shadow?.ai?.status)) return true;
  const counts = shadow?.arbitration?.counts || {};
  return ["needs_review", "blocked", "missing", "rejected"].some((key) => Number(counts[key] || 0) > 0);
}

export function shouldArchivePdf({ normalized, shadow, error, mode = pdfArchiveConfig().mode } = {}) {
  if (mode === "all") return true;
  if (mode !== "problematic") return false;
  return normalizedStatus(normalized, error) !== "complete" || shadowNeedsArchive(shadow);
}

export function buildArchivedNormalizedData(normalized, shadow) {
  const base = normalized && typeof normalized === "object" ? { ...normalized } : {};
  if (shadow?.enabled === true) base._reader_shadow = shadow;
  return base;
}

function supabaseHeaders(config, extra = {}) {
  return {
    apikey: config.serviceRoleKey,
    Authorization: `Bearer ${config.serviceRoleKey}`,
    ...extra,
  };
}

async function supabaseRequest(
  path,
  init = {},
  config = pdfArchiveConfig(),
  { timeoutMs = null, timeoutCode = "pdf_archive_request_timeout" } = {},
) {
  if (!archiveConfigured({ ...config, mode: config.mode === "off" ? "all" : config.mode })) {
    throw new Error("Archivio PDF non configurato");
  }
  const response = await fetchWithTimeout(`${config.supabaseUrl}${path}`, {
    ...init,
    headers: supabaseHeaders(config, init.headers || {}),
  }, { timeoutMs, timeoutCode });
  if (!response.ok) {
    const body = await response.text().catch(() => "");
    throw new Error(`Supabase PDF archive ${response.status}: ${body.slice(0, 300)}`);
  }
  if (response.status === 204) return null;
  const contentType = response.headers.get("content-type") || "";
  return contentType.includes("application/json") ? response.json() : response.text();
}

function safeFileName(value) {
  const base = String(value || "documento.pdf").split(/[\\/]/).pop() || "documento.pdf";
  return base.replace(/[\u0000-\u001f\u007f]/g, "").slice(0, 180) || "documento.pdf";
}

function storageObjectPath(fileHash) {
  const now = new Date();
  const year = String(now.getUTCFullYear());
  const month = String(now.getUTCMonth() + 1).padStart(2, "0");
  return `${year}/${month}/${fileHash}.pdf`;
}

function encodeStoragePath(path) {
  return String(path).split("/").map(encodeURIComponent).join("/");
}

async function uploadPdf(buffer, storagePath, config, requestTimeoutMs = null) {
  return supabaseRequest(
    `/storage/v1/object/${encodeURIComponent(config.bucket)}/${encodeStoragePath(storagePath)}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/pdf",
        "x-upsert": "true",
        "cache-control": "private, max-age=0, no-store",
      },
      body: buffer,
    },
    config,
    { timeoutMs: requestTimeoutMs, timeoutCode: "pdf_archive_upload_timeout" },
  );
}

async function insertPdfAnalysis(record, config, requestTimeoutMs = null) {
  const result = await supabaseRequest(
    "/rest/v1/pdf_analyses",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Prefer: "return=representation",
      },
      body: JSON.stringify(record),
    },
    config,
    { timeoutMs: requestTimeoutMs, timeoutCode: "pdf_archive_insert_timeout" },
  );
  return Array.isArray(result) ? result[0] : result;
}

export async function archivePdfAnalysis({
  filePath,
  originalFilename,
  mimeType = "application/pdf",
  fileSize,
  normalized = null,
  shadow = null,
  error = null,
  context = {},
  requestTimeoutMs = null,
} = {}) {
  const config = pdfArchiveConfig();
  if (!archiveConfigured(config)) return { stored: false, reason: "disabled" };
  if (!shouldArchivePdf({ normalized, shadow, error, mode: config.mode })) return { stored: false, reason: "mode_filtered" };
  if (!filePath) return { stored: false, reason: "missing_file" };

  const buffer = await fs.readFile(filePath);
  const fileHash = crypto.createHash("sha256").update(buffer).digest("hex");
  const storagePath = storageObjectPath(fileHash);
  const analysisId = crypto.randomUUID();
  const status = normalizedStatus(normalized, error);
  const createdAt = new Date();
  const expiresAt = new Date(createdAt.getTime() + config.retentionDays * 86_400_000);

  await uploadPdf(buffer, storagePath, config, requestTimeoutMs);
  const record = {
    id: analysisId,
    created_at: createdAt.toISOString(),
    expires_at: expiresAt.toISOString(),
    parser_version: normalized?.parser_version || "unknown",
    archive_mode: config.mode,
    status,
    review_status: "pending",
    original_file_name: safeFileName(originalFilename),
    storage_bucket: config.bucket,
    storage_path: storagePath,
    file_sha256: fileHash,
    file_size: Number(fileSize || buffer.length),
    mime_type: mimeType || "application/pdf",
    kind: normalized?.kind || "unknown",
    commodity: normalized?.commodity || "unknown",
    provider: normalized?.fornitore || "",
    recognized: Boolean(normalized?.recognized),
    confidence: normalized?.confidence || "low",
    text_length: Number(normalized?.textExtracted || 0),
    page_count: Number(normalized?.page_count || 0) || null,
    warnings: normalized?.warnings || [],
    normalized_data: buildArchivedNormalizedData(normalized, shadow),
    diagnostics: normalized?.diagnostics || [],
    confirmed_data: {},
    correction_summary: {},
    staff_notes: "",
    source_context: {
      sessionId: String(context?.sessionId || "").slice(0, 120),
      staffMode: Boolean(context?.staffMode),
      customerType: String(context?.customerType || "").slice(0, 40),
      uploadSequence: Number(context?.uploadSequence || 0) || null,
    },
    error_code: error ? String(error?.code || error?.name || "pdf_analysis_error").slice(0, 120) : "",
    error_message: error ? String(error?.message || "Errore analisi PDF").slice(0, 500) : "",
  };

  const saved = await insertPdfAnalysis(record, config, requestTimeoutMs);
  return {
    stored: true,
    analysisId: saved?.id || analysisId,
    status,
    expiresAt: saved?.expires_at || expiresAt.toISOString(),
  };
}

function staffConfig() {
  const config = pdfArchiveConfig();
  if (!config.supabaseUrl || !config.serviceRoleKey) throw new Error("Supabase non configurato");
  return { ...config, mode: config.mode === "off" ? "all" : config.mode };
}

export async function listPdfAnalyses({ limit = 100, status = "", provider = "", reviewStatus = "" } = {}) {
  const config = staffConfig();
  const params = new URLSearchParams({
    select: "id,created_at,expires_at,parser_version,status,review_status,original_file_name,storage_path,file_sha256,file_size,kind,commodity,provider,recognized,confidence,text_length,page_count,warnings,normalized_data,diagnostics,confirmed_data,correction_summary,staff_notes,error_code,error_message,source_context",
    order: "created_at.desc",
    limit: String(Math.max(1, Math.min(500, Number(limit) || 100))),
  });
  if (status) params.set("status", `eq.${status}`);
  if (provider) params.set("provider", `ilike.*${provider.replace(/[*,]/g, "")}*`);
  if (reviewStatus) params.set("review_status", `eq.${reviewStatus}`);
  return supabaseRequest(`/rest/v1/pdf_analyses?${params.toString()}`, { method: "GET" }, config);
}

export async function getPdfAnalysis(id) {
  const config = staffConfig();
  const params = new URLSearchParams({ select: "*", id: `eq.${id}`, limit: "1" });
  const rows = await supabaseRequest(`/rest/v1/pdf_analyses?${params.toString()}`, { method: "GET" }, config);
  return Array.isArray(rows) ? rows[0] || null : null;
}

export async function updatePdfAnalysis(id, { confirmedData = {}, correctionSummary = {}, reviewStatus = "reviewed", staffNotes = "" } = {}) {
  const config = staffConfig();
  const allowedReviewStatus = ["pending", "reviewed", "test_case", "discarded"].includes(reviewStatus) ? reviewStatus : "reviewed";
  const params = new URLSearchParams({ id: `eq.${id}` });
  const rows = await supabaseRequest(
    `/rest/v1/pdf_analyses?${params.toString()}`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Prefer: "return=representation" },
      body: JSON.stringify({
        confirmed_data: confirmedData && typeof confirmedData === "object" ? confirmedData : {},
        correction_summary: correctionSummary && typeof correctionSummary === "object" ? correctionSummary : {},
        review_status: allowedReviewStatus,
        staff_notes: String(staffNotes || "").slice(0, 4000),
        reviewed_at: new Date().toISOString(),
      }),
    },
    config,
  );
  return Array.isArray(rows) ? rows[0] || null : rows;
}

async function deleteStorageObject(storagePath, config, { requestTimeoutMs = null, timeoutCode = "pdf_archive_delete_timeout" } = {}) {
  return supabaseRequest(
    `/storage/v1/object/${encodeURIComponent(config.bucket)}`,
    {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prefixes: [storagePath] }),
    },
    config,
    { timeoutMs: requestTimeoutMs, timeoutCode },
  );
}

export async function deletePdfAnalysis(id) {
  const config = staffConfig();
  const analysis = await getPdfAnalysis(id);
  if (!analysis) return { deleted: false, reason: "not_found" };

  const query = new URLSearchParams({
    select: "id",
    storage_path: `eq.${analysis.storage_path}`,
    id: `neq.${id}`,
    limit: "1",
  });
  const otherRows = await supabaseRequest(`/rest/v1/pdf_analyses?${query.toString()}`, { method: "GET" }, config);
  const params = new URLSearchParams({ id: `eq.${id}` });
  await supabaseRequest(`/rest/v1/pdf_analyses?${params.toString()}`, { method: "DELETE", headers: { Prefer: "return=minimal" } }, config);
  if (!Array.isArray(otherRows) || otherRows.length === 0) {
    await deleteStorageObject(analysis.storage_path, config).catch(() => {});
  }
  return { deleted: true };
}

export async function deletePdfAnalyses(ids = []) {
  const uniqueIds = [...new Set((Array.isArray(ids) ? ids : [])
    .map(value => String(value || "").trim())
    .filter(value => /^[0-9a-f-]{36}$/i.test(value)))].slice(0, 500);
  const results = [];
  for (const id of uniqueIds) {
    results.push({ id, ...(await deletePdfAnalysis(id)) });
  }
  return {
    requested: uniqueIds.length,
    deleted: results.filter(result => result.deleted).length,
    results,
  };
}

export async function createPdfSignedUrl(id, expiresIn = 300) {
  const config = staffConfig();
  const analysis = await getPdfAnalysis(id);
  if (!analysis?.storage_path) return null;
  const result = await supabaseRequest(
    `/storage/v1/object/sign/${encodeURIComponent(analysis.storage_bucket || config.bucket)}/${encodeStoragePath(analysis.storage_path)}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ expiresIn: Math.max(60, Math.min(1800, Number(expiresIn) || 300)) }),
    },
    config,
  );
  const signedPath = result?.signedURL || result?.signedUrl || result?.url;
  if (!signedPath) return null;
  return signedPath.startsWith("http") ? signedPath : `${config.supabaseUrl}/storage/v1${signedPath.startsWith("/") ? "" : "/"}${signedPath}`;
}

export async function cleanupExpiredPdfAnalyses({ limit = 100 } = {}) {
  const config = staffConfig();
  const params = new URLSearchParams({
    select: "id",
    expires_at: `lt.${new Date().toISOString()}`,
    order: "expires_at.asc",
    limit: String(Math.max(1, Math.min(500, Number(limit) || 100))),
  });
  const rows = await supabaseRequest(`/rest/v1/pdf_analyses?${params.toString()}`, { method: "GET" }, config);
  const results = [];
  for (const row of rows || []) {
    results.push(await deletePdfAnalysis(row.id));
  }
  return { processed: results.length, deleted: results.filter((item) => item.deleted).length };
}
