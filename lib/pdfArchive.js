...
export async function downloadPdfDirectUpload({ ticket, destinationPath, now = Date.now() } = {}) {
...
export async function deletePdfDirectUpload(ticket) {
  const payload = parseUploadTicket(ticket, pdfArchiveConfig(), { allowExpired: true });
  await deleteStorageObject(payload.path, payload.config);
  return { deleted: true };
}
...
export async function listPdfAnalyses({ limit = 100, status = "", provider = "", reviewStatus = "", id = "" } = {}) {
  ...
  if (id) params.set("id", `eq.${String(id).trim()}`);
