...
export const PDF_AI_ADAPTER_VERSION = "1.0.0";
...
function boundedTimeout(value) {
  ...
  return Math.max(2_000, Math.min(15_000, Number.isFinite(parsed) ? parsed : 12_000));
}
...
export function pdfAiMode() {
  return String(process.env.PDF_AI_MODE || "off").trim().toLowerCase() === "shadow" ? "shadow" : "off";
}
  ...
  return {
    ...
    input: [
      { role: "system", content: SYSTEM_PROMPT },
      {
        role: "user",
        content: [
          { type: "input_file", filename, file_data: `data:application/pdf;base64,${bytes.toString("base64")}`, detail },
...
export async function runPdfAiShadow({ filePath, filename, legacyNormalized = {}, parserCandidates = [], deadlineAt = null, transport = defaultTransport, apiKey = process.env.OPENAI_API_KEY, model = process.env.PDF_AI_MODEL || PDF_AI_PRIMARY_MODEL } = {}) {
  if (pdfAiMode() !== "shadow") return { status: "disabled", model: null, candidates: [] };
  if (!apiKey) return { status: "unavailable", reason: "missing_openai_api_key", model, candidates: [] };

  const configuredTimeout = boundedTimeout(process.env.PDF_AI_TIMEOUT_MS);
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs < 2_000) return { status: "skipped", reason: "insufficient_time_budget", model, candidates: [] };
