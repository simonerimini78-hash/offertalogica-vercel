    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          ...
          semantic_role: { type: "string", enum: ["actual_customer_value", "identifier", "classification", "unknown"] },
          confidence: { type: "integer", minimum: 0, maximum: 100 },
        },
      },
    },
...
- The module uses codice_fiscale as the canonical tax-id field: return an explicitly labelled 11-digit P.IVA or a 16-character Italian codice fiscale in that field. Prefer semantic_role identifier; actual_customer_value is also valid when the visible label clearly identifies the customer's tax/VAT value.
...
- A supplier-internal product code labelled "Codice prodotto", "Product code" or equivalent must use codice_prodotto_fornitore_luce or codice_prodotto_fornitore_gas. Do not map it to codice_offerta_luce/gas unless the document explicitly labels it as an official offer code or economic-conditions code.
...
- For annual consumption, return the actual annual or rolling-12-month customer value, never the billed-period consumption as an annual value.
- Observation checklist: when the document visibly prints billed-period consumption such as “Consumi fatturati” or “Consumo del periodo”, return the literal value as consumo_luce_kwh or consumo_gas_smc with semantic_role billing_period, preserving label, unit, page and evidence. This is observation-only and must not be presented as annual consumption.
- For contractual sales prices and fixed fees, exclude taxes, network charges, billed totals and average bill costs unless the label explicitly identifies the requested sales component.
- Observation checklist: when the document visibly prints “Costo medio unitario materia energia”, “Costo medio unitario bolletta” or an equivalent average unit cost in EUR/kWh or EUR/Smc, always return that literal value as prezzo_luce_eur_kwh or prezzo_gas_eur_smc with semantic_role billing_period, preserving the exact nearby label, unit, page and evidence. These candidates are observation-only and must not be presented as contractual sales prices.
- If evidence is absent or ambiguous, return no candidate.
- Return JSON matching the supplied schema and no prose.`;

const EMERGENCY_SYSTEM_PROMPT = `You are the emergency visual reader inside OffertaLogica. Read only the first page of an Italian electricity or gas bill.

The main visual request already timed out. Return a small, fast, evidence-grounded result sufficient to recognize the document and recover the main customer/supply identifiers.
- Return direct visible values only.
- Prioritize: fornitore, kind, commodity, customer_type, intestatario, codice_fiscale/P.IVA, codice_cliente, POD or PDR, indirizzo_fornitura.
- Never return consumption, prices, totals, fees, estimates or calculated values.
- Use kind=bill for a bill and commodity=electricity, gas or dual.
- Every candidate must include page, nearby label, short literal evidence, semantic role and confidence.
- If a value is not clearly readable, omit it.
- Return JSON matching the supplied schema and no prose.`;
...
function validateAiOutput(parsed, { profile = "full", pageCount = 0 } = {}) {
  if (!parsed || typeof parsed !== "object") throw new Error("openai_invalid_output_object");
  if (!parsed.document) throw new Error("openai_missing_document_metadata");
  if (!Array.isArray(parsed.candidates)) throw new Error("openai_invalid_candidates");

  if (profile === "emergency") {
    const resolvedPageCount = Math.max(1, Number(parsed.document.page_count || 0), Number(pageCount || 0));
    return {
      ...parsed,
      quality: {
      ...
      page_map: Array.from({ length: resolvedPageCount }, (_, index) => ({
        page: index + 1,
        role: index === 0 ? "summary" : "unknown",
        summary: index === 0 ? "Emergency visual identity recovery from the first rasterized page." : "Page not processed by emergency recovery.",
      })),
      conflicts: [],
      review_reasons: ["Recovered with emergency first-page visual reading after the main visual request timed out."],
    };
  }

  if (!parsed.quality) throw new Error("openai_missing_document_metadata");
  for (const field of ["page_map", "conflicts", "review_reasons"]) {
...
async function runPdfAi({
  ...
  const timeoutMs = resolvePdfAiTimeoutMs({ value: timeoutValue ?? env.PDF_AI_TIMEOUT_MS, deadlineAt });
...
export async function runPdfAiFallbackImages(options = {}) {
  ...
  const remainingMs = deadlineAt ? deadlineAt - Date.now() - 1_000 : 0;
  const configuredRecoveryMs = Number(options.env?.PDF_AI_EMERGENCY_TIMEOUT_MS || process.env.PDF_AI_EMERGENCY_TIMEOUT_MS || 10_000);
  const recoveryTimeoutMs = Math.min(
    12_000,
    Number.isFinite(configuredRecoveryMs) ? Math.max(2_000, configuredRecoveryMs) : 10_000,
    remainingMs,
  );
  if (!Number.isFinite(recoveryTimeoutMs) || recoveryTimeoutMs < 5_000 || !imageFiles.length) {
    return {
      ...primary,
      attempts: 1,
  ...
  const recovery = await runPdfAi({
    ...
    imageProfile: "emergency",
    timeoutValue: recoveryTimeoutMs,
  });
  if (recovery.status === "completed") {
    return {
      ...recovery,
      attempts: 2,
      recovered_from: "openai_timeout",
      primary_timeout_ms: primary.timeout_ms || null,
      recovery_timeout_ms: recovery.timeout_ms || recoveryTimeoutMs,
    };
  }
  ...
  return {
    ...
    recovery_reason: recovery.reason || recovery.status || "emergency_recovery_failed",
    recovery_timeout_ms: recovery.timeout_ms || recoveryTimeoutMs,
  };
}
