...
export const PDF_AI_ADAPTER_VERSION = "2.0.0";
...
async function runPdfAi({
  ...
  if (requiredMode === "fallback" && !consentGranted) {
    return { status: "consent_required", reason: "explicit_user_consent_required", model, candidates: [] };
  }
...
export async function runPdfAiFallback(options = {}) {
  return runPdfAi({ ...options, requiredMode: "fallback", consentGranted: Boolean(options.consentGranted) });
}
