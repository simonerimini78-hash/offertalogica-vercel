...
export const PDF_AI_ADAPTER_VERSION = "step8-clean-reader-v3-minimal-general-map";
...
- For each supplied page, state only its main role and a short summary that can route later electricity and gas passes.
- Distinguish electricity, gas, shared customer identity, economic conditions, totals/taxes and unknown pages.
- The text after each image gives its original page number. Preserve that number in page_map.
- If a page contains both electricity and gas, say so explicitly.
- Return JSON matching the supplied schema and no prose.`;
...
- Put every readable electricity consumption value in consumption_observations. Classify annual, billing-period, monthly and meter-reading values separately. Do not put consumption in candidates.
- Reconstruct every readable electricity economic row in economic_rows before choosing its role. Keep quantity, total amount and printed unit rate in separate columns.
- Transcribe parent totals and indented child rows separately. A child row labelled "di cui" must not be merged into its parent.
- Classify sales_variable only for the electricity sales/material component; classify average prices, totals, network, power, taxes and other components separately.
- For fixed sales rows preserve the printed monthly or annual unit. Never annualize in the model.
- Return price/index/spread/offer candidates only when explicitly labelled and visually supported.
- Omit gas-only values.`,
  critical_gas: `${SYSTEM_PROMPT}

Focus only on gas and shared customer identity.
- Re-check all 14 PDR digits.
- Put every readable gas consumption value in consumption_observations. Classify annual, billing-period, monthly and meter-reading values separately. Do not put consumption in candidates.
- Reconstruct every readable gas economic row in economic_rows before choosing its role. Keep quantity, total amount and printed unit rate in separate columns.
- Transcribe parent totals and indented child rows separately. A child row labelled "di cui" must not be merged into its parent.
- Classify sales_variable only for the gas sales/material component; classify average prices, totals, network, taxes and other components separately.
- For fixed sales rows preserve the printed monthly or annual unit. Never annualize in the model.
- Return price/index/spread/offer candidates only when explicitly labelled and visually supported.
...
export async function buildPdfAiRequest({
  ...
  return {
    model,
    store: false,
    max_output_tokens: inputDetail === "high" ? 6_500 : 4_200,
    input: [
...
function outputSchemaForProfile(profile) {
  if (profile === "general") return GENERAL_OUTPUT_SCHEMA;
...
export async function buildPdfAiImageRequest({
  ...
  return {
    model,
    store: false,
    max_output_tokens: critical ? 5_200 : profile === "general" ? 1_200 : 4_200,
    input: [
      {
        role: "system",
