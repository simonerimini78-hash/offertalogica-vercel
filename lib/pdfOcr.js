...
export async function recognizeBmpWithTemporaryFile(worker, bmp, {
  deadlineAt,
  label = "ocr_recognize",
  temporaryDirectory = os.tmpdir(),
} = {}) {
  if (!worker || typeof worker.recognize !== "function") throw new TypeError("ocr_worker_invalid");
  const bytes = Buffer.isBuffer(bmp) ? bmp : Buffer.from(bmp || []);
  if (bytes.length < 54 || bytes.subarray(0, 2).toString("ascii") !== "BM") {
    throw new TypeError("ocr_bitmap_invalid");
  }
  const temporaryPath = path.join(
    temporaryDirectory,
    `offertalogica-ocr-${process.pid}-${crypto.randomUUID()}.bmp`,
  );
  await fs.writeFile(temporaryPath, bytes);
  try {
    // Tesseract.js su Node legge il percorso dal worker: il bitmap non viene
    // enumerato e serializzato come un enorme oggetto nel thread secondario.
    return await withDeadline(worker.recognize(temporaryPath), deadlineAt, label);
  } finally {
  ...
  try {
    ...
    for (const pageIndex of pageIndexes) {
      ...
      try {
        ...
        const result = await recognizeBmpWithTemporaryFile(worker, bmp, { deadlineAt, label: "ocr_recognize" });
        ...
        if (shouldRunUnoenergyHeaderCrop(text) && remainingMs(deadlineAt) >= minimumRemainingMs * 2) {
          ...
          const headerResult = await recognizeBmpWithTemporaryFile(worker, headerBmp, { deadlineAt, label: "ocr_header_crop" });
        ...
        if (shouldRunReceiptCrop(text) && remainingMs(deadlineAt) >= minimumRemainingMs * 2) {
          ...
          const cropResult = await recognizeBmpWithTemporaryFile(worker, cropBmp, { deadlineAt, label: "ocr_receipt_crop" });
