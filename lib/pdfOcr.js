...
function resolveItalianLanguageData() {
  const trainedData = require.resolve("@tesseract.js-data/ita/4.0.0/ita.traineddata.gz");
  return {
    code: "ita",
    gzip: true,
...
export async function runControlledPdfOcr(filePath, {
  ...
  const configuredMinimum = Number.parseInt(env.PDF_OCR_MIN_REMAINING_MS || "4500", 10);
  const minimumRemainingMs = Number.isFinite(configuredMinimum)
    ? Math.max(2500, Math.min(10_000, configuredMinimum))
    : 4500;
