...
export const PDF_AI_RASTER_BATCH_VERSION = "2.4.8-consolidated-critical-recovery";
...
Read only the attached pages and recover missing or previously conflicting critical fields.
...
- A date is an economic-conditions date only when the visible evidence itself includes condizioni economiche, validità, decorrenza or scadenza. A generic billing interval "dal ... al ..." is not an economic-conditions date.
...
- Average bill prices and average unit costs are not contractual sales prices; omit them from this recovery.
- Fixed monthly charges must not be annualized.
- Evidence must quote the visible label and value from the page, not a paraphrase.
  ...
  try {
    ...
    const normalized = {
      ...
      page_map: imageFiles.map((item) => ({
        ...
        summary: "Focused precision pass for identifiers, annual consumption and offer metadata."
      })),
      conflicts: [],
      review_reasons: ["Focused critical-field recovery executed after the general visual pass."]
    };
