...
export const PDF_AI_RASTER_BATCH_VERSION = "step8-clean-targeted-raster-v5-semantic-page-routing";
...
function pageEvidenceForProfile(results, profile) {
  ...
  for (const result of results || []) {
    ...
    for (const item of result?.page_map || []) {
      ...
      const explicitTarget = domains.has(targetCommodity) || targetPattern.test(context);
      const explicitOpposite = domains.has(oppositeCommodity) || oppositePattern.test(context);
