...
export const PDF_AI_RASTER_BATCH_VERSION = "step8-clean-targeted-raster-v5-3-label-question-routing";
...
function batchDiagnostics(plan, results) {
  return plan.calls.map((entry) => {
    ...
    return {
      ...
      targeted_answer_count: Array.isArray(result?.targeted_answers) ? result.targeted_answers.length : 0,
      targeted_answers: Array.isArray(result?.targeted_answers)
        ? result.targeted_answers.map((item) => ({
          query_id: compact(item?.query_id, 100) || null,
          field: compact(item?.field, 100) || null,
          found: Boolean(item?.found),
          section_literal: compact(item?.section_literal, 160) || null,
          label_literal: compact(item?.label_literal, 180) || null,
          value_literal: compact(item?.value_literal, 180) || null,
          unit_literal: compact(item?.unit_literal, 80) || null,
          page: Number(item?.page || 0) || null,
          evidence_literal: compact(item?.evidence_literal, 260) || null,
          confidence: Number(item?.confidence || 0),
          not_found_reason: compact(item?.not_found_reason, 160) || null,
        }))
        : [],
...
export async function runDeterministicPdfAiRaster({
  ...
  const targetedAnswers = completed.flatMap((item) => item.targeted_answers || []);
