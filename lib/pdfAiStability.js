export const PDF_AI_STABILITY_VERSION = "v106.8.8.3.1-evidence-alignment-1";
...
function candidateContext(candidate) {
  return `${candidate?.label || ""} ${candidate?.evidence || ""}`.replace(/\s+/g, " ").trim();
}

function validAiIdentifier(ai, field) {
  const labelPattern = field === "pod"
    ? /(?:\bpod\b|punto\s+di\s+prelievo)/i
    : /(?:\bpdr\b|punto\s+di\s+riconsegna)/i;
  return (ai?.candidates || []).some((candidate) => {
    if (candidate?.field !== field) return false;
    if (!normalizeIdentifier(field, candidateValue(candidate))) return false;
    if (!labelPattern.test(candidateContext(candidate))) return false;
    return Number(candidate?.confidence || 0) >= 80;
  });
}
...
export function inferCommodityFromEvidence({ merged = {}, selected = [], ai = {} } = {}) {
  ...
  const hasPod = Boolean(
    normalizeIdentifier("pod", merged.pod)
    || validSelectedIdentifier(selected, "pod")
    || validAiIdentifier(ai, "pod")
  );
  const hasPdr = Boolean(
    normalizeIdentifier("pdr", merged.pdr)
    || validSelectedIdentifier(selected, "pdr")
    || validAiIdentifier(ai, "pdr")
  );
  if (hasPod) electricity = Math.max(electricity, 6);
  if (hasPdr) gas = Math.max(gas, 6);
  ...
  // POD e PDR formalmente validi sono evidenze strutturali forti. Possono
  // correggere una classificazione IA incoerente, ma non cancellano dati né
  // trasformano da soli una bolletta mono-commodity in dual.
  if (hasPod && hasPdr) return "dual";
  if (electricity >= 3 && gas >= 3) return "dual";
  if (hasPod && gas < 3) return "luce";
  if (hasPdr && electricity < 3) return "gas";
  if (electricity >= 4 && gas < 2) return "luce";
  if (gas >= 4 && electricity < 2) return "gas";
  if (current === "dual" && electricity >= 2 && gas >= 2) return "dual";
