export const PDF_FIELD_VALIDATION_VERSION = "v102.5-safe-variable-formula";
...
function typeStatus(normalized, commodity) {
  ...
  const indexMismatch = commodity === "luce" ? /\b(?:PSV|PSBG)\b/.test(index || formula) : /\bPUN\b/.test(index || formula);
  ...
  if (type === "variabile" && !index && !/\b(?:PUN|PSV|PSBG)\b/.test(formula)) return { status: "parziale", reason: "prezzo_variabile_senza_indice" };
  if (type === "ibrido") {
    ...
    if (!index && !/\b(?:PUN|PSV|PSBG)\b/.test(formula)) return { status: "parziale", reason: "prezzo_ibrido_senza_indice" };
  }
  return { status: "completo", reason: null };
}
...
function indexStatus(normalized, commodity) {
  ...
  if (commodity === "luce" && /\b(?:PSV|PSBG)\b/i.test(index)) return { status: "da_verificare", reason: "indice_gas_su_luce" };
...
function usableVariableFormula(normalized, fieldStatus, commodity) {
  ...
  const simpleFormula = formula
    ...
    .replace(/^PREZZO\s*=\s*/, "")
    .trim();
  const parseableFormula = /^(?:PUN(?: INDEX GME)?|PSV|PSBG)(?:\s*[X×*]\s*\d+(?:[.,]\d+)?)?(?:\s*[+-]\s*\d+(?:[.,]\d+)?\s*(?:€|EUR)?\s*\/?\s*(?:KWH|SMC)?)?$/.test(simpleFormula);
...
function comparisonReadiness(normalized, fieldStatus, commodity) {
  ...
  const fixedField = isLuce ? "quota_fissa_vendita_luce_eur_anno" : "quota_fissa_vendita_gas_eur_anno";
...
export function buildPdfFieldValidation(input = {}) {
  ...
  for (const commodity of ["luce", "gas"]) {
    ...
    const fixedResult = statusForSignedFixedNumber(normalized[fixedField]);
    setStatus(fieldStatus, fixedField, fixedResult.status, fixedResult.reason);
    ...
    const reviewFields = [addressField, supplyField, consumptionField, priceField, fixedField, `tipo_prezzo_${commodity}`, `indice_riferimento_${commodity}`, multiplierField, `validita_condizioni_economiche_${commodity}`];
