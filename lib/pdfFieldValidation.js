export const PDF_FIELD_VALIDATION_VERSION = "v102.3-dual-address-provenance-guard";
...
export function completeDualSupplyAddresses(input = {}) {
  ...
  if (normalized.commodity === "dual") {
    const hasSpecificAddress = Boolean(luce || gas);
    // A genuinely generic-only dual address may be shared by both supplies.
    // When one commodity already has a specific address, the generic value may
    // have been derived from it by an earlier validation pass and must not be
    // copied into the other commodity without independent evidence.
    if (!hasSpecificAddress && generic) {
      luce = generic;
      gas = generic;
...
export function buildPdfFieldValidation(input = {}) {
  ...
  for (const [field, explicit] of Object.entries(input.field_status || {})) {
    if (!fieldStatus[field] || !PDF_FIELD_STATES.includes(explicit?.status)) continue;
    setStatus(fieldStatus, field, explicit.status, explicit.reason || null, explicit.evidence || null);
    if (explicit.status === "da_verificare") addIssue(validationIssues, field, "review", explicit.reason || "verifica_utente_richiesta");
    if (explicit.status === "parziale") addIssue(validationIssues, field, "partial", explicit.reason || "dato_parziale");
  }
