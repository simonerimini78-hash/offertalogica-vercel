export const PDF_FIELD_VALIDATION_VERSION = "v102.6-fixed-monthly";
...
function statusForNonNegativeNumber(value, { max = Infinity } = {}) {
  if (!hasValue(value)) return { status: "mancante", reason: "valore_non_estratto" };
  const number = Number(value);
  if (!Number.isFinite(number) || number < 0 || number > max) return { status: "da_verificare", reason: "valore_fuori_intervallo" };
  return { status: "completo", reason: null };
...
function statusForSignedFixedNumber(value, { maxAbs = 10_000 } = {}) {
  if (!hasValue(value)) return { status: "mancante", reason: "valore_non_estratto" };
  const number = Number(value);
  if (!Number.isFinite(number) || Math.abs(number) > maxAbs) return { status: "da_verificare", reason: "valore_fuori_intervallo" };
  return { status: "completo", reason: null };
}
...
function typeStatus(normalized, commodity) {
  ...
  if (type === "fisso" && (index || finiteNonZero(spread))) return { status: "da_verificare", reason: "prezzo_fisso_con_indice_o_spread" };
...
function nonApplicableReadiness(reason = "non_applicabile_al_tipo_documento") {
  return {
...
function completeStatus(fieldStatus, field) {
  return fieldStatus[field]?.status === "completo";
}

function usableLightBandPriceSet(fieldStatus) {
  if (completeStatus(fieldStatus, "prezzo_luce_f0_eur_kwh")) return { ready: true, mode: "f0" };
  if (completeStatus(fieldStatus, "prezzo_luce_f1_eur_kwh") && completeStatus(fieldStatus, "prezzo_luce_f23_eur_kwh")) return { ready: true, mode: "f1_f23" };
  if (completeStatus(fieldStatus, "prezzo_luce_f1_eur_kwh") && completeStatus(fieldStatus, "prezzo_luce_f2_eur_kwh") && completeStatus(fieldStatus, "prezzo_luce_f3_eur_kwh")) return { ready: true, mode: "f1_f2_f3" };
  return { ready: false, mode: null };
}

function usableLightBandConsumptionSet(fieldStatus, priceMode) {
  if (priceMode === "f0") return completeStatus(fieldStatus, "consumo_luce_kwh");
  if (priceMode === "f1_f23") return completeStatus(fieldStatus, "consumo_luce_f1_kwh") && completeStatus(fieldStatus, "consumo_luce_f23_kwh");
  if (priceMode === "f1_f2_f3") return completeStatus(fieldStatus, "consumo_luce_f1_kwh") && completeStatus(fieldStatus, "consumo_luce_f2_kwh") && completeStatus(fieldStatus, "consumo_luce_f3_kwh");
  return false;
}

function usableVariableFormula(normalized, fieldStatus, commodity) {
  const suffix = commodity === "luce" ? "luce" : "gas";
  const type = compact(normalized[`tipo_prezzo_${suffix}`]).toLowerCase();
  if (!["variabile", "ibrido"].includes(type)) return false;
  const indexReady = completeStatus(fieldStatus, `indice_riferimento_${suffix}`);
  const formula = compact(normalized[`formula_prezzo_${suffix}`]);
  const spreadField = commodity === "luce" ? "spread_luce_eur_kwh" : "spread_gas_eur_smc";
  const explicitSpread = hasValue(normalized[spreadField]) && Number.isFinite(Number(normalized[spreadField]));
  const simpleFormula = formula
    .toUpperCase()
    .replace(/\s+/g, " ")
    .replace(/^PREZZO\s*=\s*/, "")
    .trim();
  const parseableFormula = /^(?:PUN(?: INDEX GME)?|PSV)(?:\s*[X×*]\s*\d+(?:[.,]\d+)?)?(?:\s*[+-]\s*\d+(?:[.,]\d+)?\s*(?:€|EUR)?\s*\/?\s*(?:KWH|SMC)?)?$/.test(simpleFormula);
  return indexReady && (explicitSpread || parseableFormula);
}

function comparisonReadiness(normalized, fieldStatus, commodity) {
  const isLuce = commodity === "luce";
  const fixedAnnualField = isLuce ? "quota_fissa_vendita_luce_eur_anno" : "quota_fissa_vendita_gas_eur_anno";
  const fixedMonthlyField = isLuce ? "quota_fissa_confrontabile_luce_eur_mese" : "quota_fissa_confrontabile_gas_eur_mese";
  const typeField = `tipo_prezzo_${commodity}`;
  const nameField = `nome_offerta_${commodity}`;
  const codeField = `codice_offerta_${commodity}`;
  const validityField = `validita_condizioni_economiche_${commodity}`;
  const simplePriceField = isLuce ? "prezzo_luce_eur_kwh" : "prezzo_gas_eur_smc";
  const missing = [];
  const review = [];
  const partial = [];
  const missingRecommended = [];
  ...
  if (!completeStatus(fieldStatus, fixedAnnualField) && !completeStatus(fieldStatus, fixedMonthlyField)) {
    const annualStatus = fieldStatus[fixedAnnualField]?.status || "mancante";
    const monthlyStatus = fieldStatus[fixedMonthlyField]?.status || "mancante";
    if (annualStatus === "da_verificare" || monthlyStatus === "da_verificare") review.push(fixedAnnualField);
    else if (annualStatus === "parziale" || monthlyStatus === "parziale") partial.push(fixedAnnualField);
    else missing.push(fixedAnnualField);
  }
  if (normalized.kind === "scheda_offerta") requireField(nameField);
  else {
    requireField(isLuce ? "consumo_luce_kwh" : "consumo_gas_smc");
    recommendField(nameField);
  }

  let pricingReady = completeStatus(fieldStatus, simplePriceField);
  let pricingMode = pricingReady ? "single_price" : null;
  if (!pricingReady && usableVariableFormula(normalized, fieldStatus, commodity)) {
    pricingReady = true;
    pricingMode = "variable_formula";
  }
  if (isLuce && !pricingReady) {
    const bandSet = usableLightBandPriceSet(fieldStatus);
    if (bandSet.ready) {
      if (normalized.kind === "scheda_offerta" || usableLightBandConsumptionSet(fieldStatus, bandSet.mode)) {
        pricingReady = true;
        pricingMode = bandSet.mode;
      } else {
        missing.push("consumi_annui_per_fascia_luce");
      }
    }
  }
  if (!pricingReady && !missing.includes("consumi_annui_per_fascia_luce")) missing.push(simplePriceField);

  recommendField(typeField);
  recommendField(codeField);
  recommendField(validityField);
  ...
  return {
    ...
    partial: [...new Set(partial)],
    review: [...new Set(review)],
    missing_recommended: [...new Set(missingRecommended)],
    partial_recommended: [...new Set(partialRecommended)],
...
export function buildPdfFieldValidation(input = {}) {
  ...
  for (const commodity of ["luce", "gas"]) {
    if (!commodityPresent(normalized, commodity)) {
      for (const field of [
        ...
        ...(commodity === "luce" ? ["consumo_luce_f1_kwh", "consumo_luce_f2_kwh", "consumo_luce_f3_kwh", "consumo_luce_f23_kwh"] : []),
        commodity === "luce" ? "prezzo_luce_eur_kwh" : "prezzo_gas_eur_smc",
        ...(commodity === "luce" ? ["prezzo_luce_f0_eur_kwh", "prezzo_luce_f1_eur_kwh", "prezzo_luce_f2_eur_kwh", "prezzo_luce_f3_eur_kwh", "prezzo_luce_f23_eur_kwh"] : []),
        commodity === "luce" ? "quota_fissa_vendita_luce_eur_anno" : "quota_fissa_vendita_gas_eur_anno",
        commodity === "luce" ? "quota_fissa_confrontabile_luce_eur_mese" : "quota_fissa_confrontabile_gas_eur_mese",
        `nome_offerta_${commodity}`,
        `codice_offerta_${commodity}`,
        `tipo_prezzo_${commodity}`,
        `indice_riferimento_${commodity}`,
        commodity === "luce" ? "moltiplicatore_indice_luce" : "moltiplicatore_indice_gas",
    ...
    const fixedAnnualField = commodity === "luce" ? "quota_fissa_vendita_luce_eur_anno" : "quota_fissa_vendita_gas_eur_anno";
    const fixedMonthlyField = commodity === "luce" ? "quota_fissa_confrontabile_luce_eur_mese" : "quota_fissa_confrontabile_gas_eur_mese";
    ...
    if (commodity === "luce") {
      const consumptionBandFields = [
        "consumo_luce_f1_kwh",
        "consumo_luce_f2_kwh",
        "consumo_luce_f3_kwh",
        "consumo_luce_f23_kwh",
      ];
      const hasConsumptionBands = consumptionBandFields.some((field) => hasValue(normalized[field]));
      for (const field of consumptionBandFields) {
        if (!hasConsumptionBands || !hasValue(normalized[field])) setStatus(fieldStatus, field, "non_applicabile", null);
        else {
          const result = statusForNonNegativeNumber(normalized[field], { max: 100_000_000 });
          setStatus(fieldStatus, field, result.status, result.reason);
        }
      }
      ...
      const hasPriceBands = priceBandFields.some((field) => hasValue(normalized[field]));
      for (const field of priceBandFields) {
        if (!hasPriceBands || !hasValue(normalized[field])) setStatus(fieldStatus, field, "non_applicabile", null);
        else {
          const result = statusForPositiveNumber(normalized[field], { max: 5 });
          setStatus(fieldStatus, field, result.status, result.reason);
        }
      }
      if (!hasValue(normalized.prezzo_luce_eur_kwh) && usableLightBandPriceSet(fieldStatus).ready) {
        setStatus(fieldStatus, "prezzo_luce_eur_kwh", "non_applicabile", null);
      }
    }
    const hasFixedAnnual = hasValue(normalized[fixedAnnualField]);
    const hasFixedMonthly = hasValue(normalized[fixedMonthlyField]);
    if (hasFixedAnnual) {
      const fixedAnnualResult = statusForNonNegativeNumber(normalized[fixedAnnualField], { max: 10_000 });
      setStatus(fieldStatus, fixedAnnualField, fixedAnnualResult.status, fixedAnnualResult.reason);
    } else setStatus(fieldStatus, fixedAnnualField, hasFixedMonthly ? "non_applicabile" : "mancante", hasFixedMonthly ? null : "valore_non_estratto");
    if (hasFixedMonthly) {
      const fixedMonthlyResult = statusForNonNegativeNumber(normalized[fixedMonthlyField], { max: 1_000 });
      setStatus(fieldStatus, fixedMonthlyField, fixedMonthlyResult.status, fixedMonthlyResult.reason);
    } else setStatus(fieldStatus, fixedMonthlyField, hasFixedAnnual ? "non_applicabile" : "mancante", hasFixedAnnual ? null : "valore_non_estratto");
    ...
    const multiplierField = commodity === "luce" ? "moltiplicatore_indice_luce" : "moltiplicatore_indice_gas";
    if (hasValue(normalized[multiplierField])) {
      const multiplierResult = statusForPositiveNumber(normalized[multiplierField], { max: 5 });
      setStatus(fieldStatus, multiplierField, multiplierResult.status, multiplierResult.reason);
    } else {
      setStatus(fieldStatus, multiplierField, "non_applicabile", null);
    }
    ...
    const reviewFields = [addressField, supplyField, consumptionField, priceField, fixedAnnualField, fixedMonthlyField, `tipo_prezzo_${commodity}`, `indice_riferimento_${commodity}`, multiplierField, `validita_condizioni_economiche_${commodity}`];
    if (commodity === "luce") reviewFields.push(
      "consumo_luce_f1_kwh", "consumo_luce_f2_kwh", "consumo_luce_f3_kwh", "consumo_luce_f23_kwh",
      "prezzo_luce_f0_eur_kwh", "prezzo_luce_f1_eur_kwh", "prezzo_luce_f2_eur_kwh", "prezzo_luce_f3_eur_kwh", "prezzo_luce_f23_eur_kwh",
    );
  ...
  for (const commodity of ["luce", "gas"]) {
    ...
    readiness.confronto[commodity] = comparisonReadiness(normalized, fieldStatus, commodity);
    ...
    if (normalized.kind === "scheda_offerta") {
      readiness.dati_bolletta[commodity] = nonApplicableReadiness("scheda_offerta_senza_dati_cliente");
      readiness.attivazione[commodity] = nonApplicableReadiness("scheda_offerta_non_usata_per_attivazione_cliente");
    } else {
