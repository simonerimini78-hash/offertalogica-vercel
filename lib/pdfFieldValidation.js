...
export function buildPdfFieldValidation(input = {}) {
  ...
  for (const commodity of ["luce", "gas"]) {
    ...
    for (const [field, options] of [
      ...
      const result = field === priceField
        ? statusForNonNegativeNumber(normalized[field], options)
        : statusForPositiveNumber(normalized[field], options);
    ...
    if (commodity === "luce") {
      ...
      for (const field of priceBandFields) {
        if (!hasPriceBands || !hasValue(normalized[field])) setStatus(fieldStatus, field, "non_applicabile", null);
          ...
          const result = statusForNonNegativeNumber(normalized[field], { max: 5 });
          setStatus(fieldStatus, field, result.status, result.reason);
        }
      }
