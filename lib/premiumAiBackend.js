...
function normalizePremiumCustomerSegment(value) {
  return trimText(value, 32).toLowerCase() === "business" ? "business" : "consumer";
}

function premiumSubscriptionProductCode(subscription) {
  const segment = normalizePremiumCustomerSegment(subscription?.customer_segment);
  return trimText(subscription?.product_code, 80)
    || (segment === "business" ? "premium_business" : "premium_casa");
}

function assertPremiumBillSubscriptionContext(bill, subscription) {
  if (!subscription) return;
  const expectedSegment = normalizePremiumCustomerSegment(subscription.customer_segment);
  const expectedProductCode = premiumSubscriptionProductCode(subscription);
  const billSegment = trimText(bill?.customer_segment, 32).toLowerCase();
  const billProductCode = trimText(bill?.product_code, 80);

  // Legacy Premium data predates Business and is therefore Casa/consumer.
  // Business records, instead, must always carry an explicit dimensional snapshot.
  if (expectedSegment === "business" && !billSegment) throw new Error("premium_bill_segment_missing");
  if (billSegment && normalizePremiumCustomerSegment(billSegment) !== expectedSegment) {
    throw new Error("premium_bill_segment_mismatch");
  }
  if (expectedSegment === "business" && !billProductCode) throw new Error("premium_bill_product_missing");
  if (billProductCode && billProductCode !== expectedProductCode) {
    throw new Error("premium_bill_product_mismatch");
  }
}
...
function canonicalProviderText(value) {
  return canonicalText(trimText(value, 240).replace(/\./g, ""))
    .replace(/\b(?:societa per azioni|societa a responsabilita limitata|spa|srl)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function providerNamesEquivalent(actual, expected) {
  const a = canonicalProviderText(actual);
  const e = canonicalProviderText(expected);
  if (!a || !e) return false;
  if (a === e) return true;
...
export async function verifyPremiumCustomer({ config, accessToken, fetchImpl = fetch } = {}) {
  ...
  const subscriptionQuery = new URLSearchParams({
    select: "id,user_id,status,plan_code,customer_segment,product_code,current_period_end,included_bills_per_year,created_at",
    user_id: `eq.${user.id}`,
...
export async function loadPremiumCustomerBill({ config, billId, userId, subscription = null, fetchImpl = fetch } = {}) {
  const normalizedBillId = trimText(billId, 80);
  if (!normalizedBillId) throw new Error("premium_bill_id_required");
  const query = new URLSearchParams({
    select: "id,user_id,utility_id,contract_id,commodity,customer_segment,product_code,plan_code_snapshot,original_file_name,file_size,storage_bucket,storage_path,processing_status,customer_status,automatic_screening_status,deleted_at,created_at",
  ...
  assertPremiumBillSubscriptionContext(bill, subscription);
  ...
  if (!["uploaded", "failed", "analyzing"].includes(bill.processing_status)) throw new Error("premium_bill_not_auto_analyzable");
  return bill;
}
...
export async function createPremiumAnalysisRun({ config, check = null, bill, staffUserId = null, requestedByUserId = null, origin = "staff_manual", staleAfterMs = 90000, fetchImpl = fetch } = {}) {
  const activeQuery = new URLSearchParams({
    select: "id,status,run_number,started_at",
    bill_id: `eq.${bill.id}`,
    status: "in.(queued,running)",
    limit: "1",
  });
  const active = await serviceRequest(config, `/rest/v1/premium_analysis_runs?${activeQuery}`, { method: "GET" }, fetchImpl);
  const activeRun = Array.isArray(active) ? active[0] : null;
  if (activeRun) {
    const startedAt = new Date(activeRun.started_at || 0).getTime();
    const staleLimit = Math.max(60000, Number(staleAfterMs) || 90000);
    const stale = Number.isFinite(startedAt) && startedAt > 0 && Date.now() - startedAt >= staleLimit;
    if (!stale) throw new Error("premium_analysis_already_running");
    ...
    await patchPremiumAnalysisRun({
      ...
      values: {
        ...
        error_code: "premium_analysis_stale_recovered",
        error_message: "Analisi precedente interrotta e chiusa automaticamente prima del nuovo tentativo.",
      },
    });
  }
...
export function createMeteredOpenAiTransport({ meter, fetchImpl = fetch, onUsage = null } = {}) {
  return async ({ request, apiKey, signal, attempt, profile }) => {
    ...
    if (typeof onUsage === "function") {
      await onUsage({ body, attempt, profile, totals: meter?.totals || null });
    }
    return body;
  };
}
...
export function customerVisibleAnalysisData(normalized = {}) {
  const source = normalized && typeof normalized === "object" ? normalized : {};
  const text = (value, max = 300) => {
    const cleaned = trimText(value, max);
    return cleaned || null;
  };
  const payload = {
    commodity: text(source.commodity, 20),
    fornitore: text(source.fornitore, 240),
    fornitore_luce: text(source.fornitore_luce ?? source.fornitore, 240),
    fornitore_gas: text(source.fornitore_gas ?? source.fornitore, 240),
    nome_offerta_luce: text(source.nome_offerta_luce ?? source.nome_offerta, 300),
    nome_offerta_gas: text(source.nome_offerta_gas ?? source.nome_offerta, 300),
    codice_offerta_luce: text(source.codice_offerta_luce ?? source.codice_offerta, 180),
    codice_offerta_gas: text(source.codice_offerta_gas ?? source.codice_offerta, 180),
    pod: text(source.pod, 50),
    pdr: text(source.pdr, 50),
    billing_period_start: validIsoDate(source.billing_period_start),
    billing_period_end: validIsoDate(source.billing_period_end),
    issue_date: validIsoDate(source.issue_date),
    due_date: validIsoDate(source.due_date),
    total_amount_eur: finiteNumber(source.total_amount_eur),
    consumo_luce_kwh: finiteNumber(source.consumo_luce_kwh),
    consumo_luce_f1_kwh: finiteNumber(source.consumo_luce_f1_kwh),
    consumo_luce_f2_kwh: finiteNumber(source.consumo_luce_f2_kwh),
    consumo_luce_f3_kwh: finiteNumber(source.consumo_luce_f3_kwh),
    consumo_luce_f23_kwh: finiteNumber(source.consumo_luce_f23_kwh),
    prezzo_luce_eur_kwh: finiteNumber(source.prezzo_luce_eur_kwh),
    prezzo_luce_f0_eur_kwh: finiteNumber(source.prezzo_luce_f0_eur_kwh),
    prezzo_luce_f1_eur_kwh: finiteNumber(source.prezzo_luce_f1_eur_kwh),
    prezzo_luce_f2_eur_kwh: finiteNumber(source.prezzo_luce_f2_eur_kwh),
    prezzo_luce_f3_eur_kwh: finiteNumber(source.prezzo_luce_f3_eur_kwh),
    prezzo_luce_f23_eur_kwh: finiteNumber(source.prezzo_luce_f23_eur_kwh),
    quota_fissa_vendita_luce_eur_anno: finiteNumber(source.quota_fissa_vendita_luce_eur_anno),
    potenza_impegnata_kw: finiteNumber(source.potenza_impegnata_kw),
    tipo_prezzo_luce: text(source.tipo_prezzo_luce, 80),
    indice_riferimento_luce: text(source.indice_riferimento_luce, 80),
    spread_luce_eur_kwh: finiteNumber(source.spread_luce_eur_kwh),
    formula_prezzo_luce: text(source.formula_prezzo_luce, 600),
    scadenza_condizioni_economiche_luce: validIsoDate(source.scadenza_condizioni_economiche_luce),
    consumo_gas_smc: finiteNumber(source.consumo_gas_smc),
    prezzo_gas_eur_smc: finiteNumber(source.prezzo_gas_eur_smc),
    quota_fissa_vendita_gas_eur_anno: finiteNumber(source.quota_fissa_vendita_gas_eur_anno),
    tipo_prezzo_gas: text(source.tipo_prezzo_gas, 80),
    indice_riferimento_gas: text(source.indice_riferimento_gas, 80),
    spread_gas_eur_smc: finiteNumber(source.spread_gas_eur_smc),
    formula_prezzo_gas: text(source.formula_prezzo_gas, 600),
    scadenza_condizioni_economiche_gas: validIsoDate(source.scadenza_condizioni_economiche_gas),
  };
...
function priceDiagnostic(normalized = {}, field = "") {
  const diagnostics = Array.isArray(normalized?.diagnostics) ? normalized.diagnostics : [];
  return [...diagnostics].reverse().find((item) => trimText(item?.field, 120) === field) || null;
}

function contractPriceComparability(normalized = {}, field = "", pricingType = "") {
  // Un prezzo di periodo di un'offerta indicizzata/mista non è direttamente
  // confrontabile con un valore statico salvato nel contratto: per questi casi
  // restano validi i controlli specifici su indice, spread e formula.
  if (["indexed", "mixed"].includes(trimText(pricingType, 40))) return "dynamic";

  const diagnostic = priceDiagnostic(normalized, field);
  if (!diagnostic) return "inconclusive";

  const label = canonicalText(diagnostic.label);
  const evidence = canonicalText(`${diagnostic.label || ""} ${diagnostic.source_snippet || ""}`);

  // Prezzi medi/aggregati o testi che includono componenti regolatorie/fiscali
  // non sono omogenei al prezzo commerciale unitario dell'offerta. La regola è
  // volutamente indipendente dal fornitore e vale allo stesso modo per luce e gas.
  const nonComparable = /prezzo medio|spesa per la vendita|costo per la vendita|quota per consumi|totale quota|prezzo medio totale|rete|oneri|trasport|distribuz|impost| iva |accis|addizional|tribut|fiscal|perequaz|sistema/;
  if (nonComparable.test(` ${evidence} `)) return "aggregate";

  // Il rosso sul prezzo è ammesso solo quando l'evidenza identifica una
  // componente commerciale unitaria direttamente confrontabile con il contratto.
  const directlyComparable = /prezzo materia prima|materia prima gas|materia gas naturale|materia energia|prezzo applicato|corrispettivo gas|corrispettivo energia|componente gas|componente energia|prezzo fisso|prezzo unitario gas|prezzo unitario energia/;
  if (directlyComparable.test(label) || directlyComparable.test(evidence)) return "direct";
  return "inconclusive";
}

function applyContractUnitPriceCheck({ normalized, contract, commodity, red, yellow }) {
  const isLight = commodity === "luce";
  const actualField = isLight ? "prezzo_luce_eur_kwh" : "prezzo_gas_eur_smc";
  const actual = normalized?.[actualField];
  const expected = isLight ? contract?.electricity_price_eur_kwh : contract?.gas_price_eur_smc;
  const absoluteFloor = isLight ? 0.005 : 0.02;
  if (!numericMismatch(actual, expected, absoluteFloor)) return;

  const mode = contractPriceComparability(normalized, actualField, contract?.pricing_type);
  if (mode === "dynamic") return;

  const label = isLight ? "luce" : "gas";
  if (mode === "direct") {
    red.push(screeningReason(
      `prezzo_${label}_diverso_dal_contratto`,
      `Prezzo ${label} diverso`,
      `Il prezzo unitario ${label} direttamente confrontabile differisce dal contratto oltre la tolleranza.`,
      "high",
      "contract",
      { trafficLight: "red" },
    ));
  ...
  yellow.push(screeningReason(
    `prezzo_${label}_non_confrontabile_direttamente`,
    `Prezzo ${label} non confrontabile direttamente`,
    `Il valore unitario letto in bolletta è medio, aggregato o non ha evidenza sufficiente per confrontarlo direttamente con il prezzo commerciale dell'offerta. Non viene considerato una difformità.`,
    "low",
    "contract",
    { trafficLight: "yellow" },
  ));
...
function offerMatchWarning(normalized = {}, contract = null) {
  if (contract?.customer_confirmation_status === "confirmed") return null;
  ...
  if (["matched", "partial", "ambiguous"].includes(status)) {
    return screeningReason(
      "versione_offerta_non_verificata",
      "Versione offerta non verificata",
      "Il nome o alcuni dati dell’offerta sono compatibili con lo storico disponibile, ma la specifica versione economica non è identificata con certezza. Nessun prezzo di una versione simile viene usato per segnalare una difformità.",
      "low",
      "offer_match",
      { trafficLight: "yellow" },
    );
...
export function classifyPremiumAutomaticAnalysis(normalized = {}, {
  ...
  const matchReason = offerMatchWarning(normalized, contract);
  ...
  if (contract) {
    ...
    if (provider && contract.provider_name && !providerNamesEquivalent(provider, contract.provider_name)) {
    ...
    applyContractUnitPriceCheck({ normalized, contract, commodity: "luce", red, yellow });
    applyContractUnitPriceCheck({ normalized, contract, commodity: "gas", red, yellow });
...
export function premiumBillValuesFromAnalysis(normalized = {}, screening, runId, completedAt = new Date().toISOString()) {
  ...
  return {
    ...
    customer_analysis_data: customerVisibleAnalysisData(normalized),
...
export async function insertPremiumAiCostEvent({
  config,
  bill,
  check,
  run,
  ...
  const cost = finiteNumber(estimatedCostEur);
  if (cost === null || cost < 0) throw new Error("premium_ai_cost_required");
  if (!run?.id || !bill?.id || !bill?.user_id) throw new Error("premium_ai_cost_context_invalid");
  ...
  const record = {
    ...
    metadata: {
      ...
      pricing_source: pricingMetadata?.pricing_mode || "openai_usd_x_ecb",
      pricing_verified_eur: pricingMetadata?.pricing_verified_eur === true,
      pricing_version: pricingMetadata?.pricing_version || null,
      usd_to_eur_rate: finiteNumber(pricingMetadata?.usd_to_eur_rate),
      eur_to_usd_rate: finiteNumber(pricingMetadata?.eur_to_usd_rate),
      ecb_reference_date: trimText(pricingMetadata?.ecb_reference_date, 20) || null,
      ecb_rate_stale: Boolean(pricingMetadata?.ecb_rate_stale),
      token_rates_usd_per_million: pricingMetadata?.token_rates_usd_per_million || null,
      web_search_calls: integer(pricingMetadata?.web_search_calls),
      web_search_cost_eur: finiteNumber(pricingMetadata?.web_search_cost_eur),
      web_search_rate_eur_per_1k: finiteNumber(pricingMetadata?.web_search_rate_eur_per_1k),
      web_search_rate_usd_per_1k: finiteNumber(pricingMetadata?.web_search_rate_usd_per_1k),
      accounting_version: trimText(pricingMetadata?.accounting_version, 80) || null,
  ...
  const existing = await serviceRequest(
    config,
    `/rest/v1/premium_cost_events?${existingQuery}`,
    { method: "GET" },
    fetchImpl,
  );
  ...
  if (existingId) {
    ...
    return serviceRequest(config, `/rest/v1/premium_cost_events?${patchQuery}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Prefer: "return=minimal" },
      body: JSON.stringify(record),
    }, fetchImpl);
...
export async function reconcilePremiumAiCostEvents({ config, limit = 250, fetchImpl = fetch } = {}) {
  const safeLimit = Math.max(1, Math.min(500, integer(limit) || 250));
  ...
  const eligible = (Array.isArray(runs) ? runs : []).filter(run => {
    ...
    const cost = finiteNumber(run?.estimated_cost_eur);
    return run?.id && run?.bill_id && run?.user_id && cost !== null && cost >= 0 && usage.pricing_verified_eur === true;
  });
  ...
  const events = await serviceRequest(config, `/rest/v1/premium_cost_events?${eventQuery}`, { method: "GET" }, fetchImpl);
  const present = new Set((Array.isArray(events) ? events : []).map(event => String(event.analysis_run_id || "")).filter(Boolean));
  let restored = 0;

  for (const run of eligible) {
    if (present.has(run.id)) continue;
    const details = run.usage_details && typeof run.usage_details === "object" ? run.usage_details : {};
    const usage = {
      inputTokens: integer(details.input_tokens ?? run.input_tokens),
      cachedInputTokens: integer(details.cached_input_tokens),
      outputTokens: integer(details.output_tokens ?? run.output_tokens),
      reasoningTokens: integer(details.reasoning_tokens),
      totalTokens: integer(details.total_tokens ?? (integer(run.input_tokens) + integer(run.output_tokens))),
    ...
    await insertPremiumAiCostEvent({
      ...
      run,
      usage,
      estimatedCostEur: run.estimated_cost_eur,
      model: run.model,
      pricingMetadata: details,
      fetchImpl,
    });
    restored += 1;
  }
  ...
  return {
    scanned: Array.isArray(runs) ? runs.length : 0,
    eligible: eligible.length,
    restored,
    alreadyPresent: eligible.length - restored,
  };
}
...
export function publicPremiumAiError(error) {
  ...
  if (/premium_ai_pricing_unavailable|premium_ai_cost_required|premium_ai_cost_context_invalid/.test(message)) return { status: 503, code: "PREMIUM_AI_ACCOUNTING_UNAVAILABLE", error: "Analisi temporaneamente sospesa perché il costo IA non è verificabile. Riprova tra poco." };
