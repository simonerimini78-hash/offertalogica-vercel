...
export async function requireStaffSession(
  ...
  try {
    ...
    identity = await verifyPremiumStaff({ config, accessToken, fetchImpl });
  } catch (error) {
    const message = String(error?.message || "");
    if (message.includes("premium_staff_access_required")) {
      json(res, 403, {
        ok: false,
        error: "Account staff non autorizzato",
        code: "staff_access_required",
      });
      return null;
    }
    if (message.includes("premium_invalid_session") || message.includes("premium_auth_required")) {
      json(res, 401, {
        ok: false,
    ...
    console.error("[staff-auth] identity-check-failed", {
      code: String(message || "staff_identity_check_failed").split(":")[0].slice(0, 120),
    });
    json(res, 503, {
      ok: false,
      error: "Verifica backend sessione Staff non disponibile",
      code: "staff_identity_check_failed",
    });
    return null;
  }
  ...
  if (requestedPermissions.length) {
    ...
    try {
      ...
      const message = String(error?.message || "");
      console.error("[staff-auth] permission-check-failed", {
        code: String(message || "staff_permission_check_failed").split(":")[0].slice(0, 120),
      });
      json(res, 503, {
        ok: false,
        error: "Controllo permessi Staff non disponibile",
        code: "staff_permission_check_failed",
      });
      return null;
    }
