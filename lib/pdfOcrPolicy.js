export const PDF_OCR_PIPELINE_VERSION = "v105.8-ocr-file-transport-page-coverage-1";
...
export function ocrMaxRenderPixels(env = process.env) {
  const requested = Number.parseInt(env.PDF_OCR_MAX_RENDER_PIXELS || "3000000", 10);
  if (!Number.isFinite(requested)) return 3_000_000;
  return Math.max(1_500_000, Math.min(6_000_000, requested));
}

export function ocrRenderScale(pageSize = {}, env = process.env) {
  const requestedScale = ocrScale(env);
  ...
  const maxPixels = ocrMaxRenderPixels(env);
  if (sourcePixels * requestedScale * requestedScale <= maxPixels) return requestedScale;
  // Le fotografie telefoniche possono avere pagine PDF da oltre 3.000 x 4.000 punti.
  // Ridurre la scala prima del BMP evita copie da centinaia di MB nel worker Tesseract.
  return Math.min(requestedScale, Math.sqrt(maxPixels / sourcePixels));
}
...
export function selectOcrPageIndexes(pageCount, maxPages = 2) {
  ...
  // Le bollette raster multipagina collocano spesso il riepilogo nella prima
  // pagina e i dettagli della seconda fornitura nelle ultime due.
