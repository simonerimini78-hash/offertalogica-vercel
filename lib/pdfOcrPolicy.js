export const PDF_OCR_PIPELINE_VERSION = "v105.7-ocr-photo-memory-guard-1";
...
export function ocrMaxRenderPixels(env = process.env) {
  const requested = Number.parseInt(env.PDF_OCR_MAX_RENDER_PIXELS || "6000000", 10);
  if (!Number.isFinite(requested)) return 6_000_000;
  return Math.max(3_000_000, Math.min(12_000_000, requested));
}
