export const PDF_OCR_PIPELINE_VERSION = "v105.6-ocr-large-photo-scaling-1";
...
export function ocrMaxRenderPixels(env = process.env) {
  const requested = Number.parseInt(env.PDF_OCR_MAX_RENDER_PIXELS || "12000000", 10);
  if (!Number.isFinite(requested)) return 12_000_000;
  return Math.max(4_000_000, Math.min(16_000_000, requested));
}
