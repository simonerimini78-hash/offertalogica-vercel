...
export const PDF_HYBRID_POLICY_VERSION = "v106.8.6-offer-code-separation-1";
...
const OFFICIAL_OFFER_CODE_FIELDS = new Set(["codice_offerta", "codice_offerta_luce", "codice_offerta_gas"]);
const SUPPLIER_PRODUCT_CODE_FIELDS = new Set([
  "codice_prodotto_fornitore",
  "codice_prodotto_fornitore_luce",
  "codice_prodotto_fornitore_gas",
]);
...
function codeCommodity(field, evidence, commodity) {
  if (String(field).endsWith("_luce")) return "luce";
  if (String(field).endsWith("_gas")) return "gas";
  const quote = compact(evidence, 700);
  const light = /\b(?:luce|energia\s+elettrica|pod)\b/i.test(quote);
  const gas = /\b(?:gas(?:\s+naturale)?|pdr)\b/i.test(quote);
  if (light !== gas) return light ? "luce" : "gas";
  return ["luce", "gas"].includes(commodity) ? commodity : null;
}

function offerCodeField(kind, field, evidence, commodity) {
  const suffix = codeCommodity(field, evidence, commodity);
  const base = kind === "official" ? "codice_offerta" : "codice_prodotto_fornitore";
  return suffix ? `${base}_${suffix}` : base;
}

function normalizeOfferCode(value, kind) {
  const normalized = compact(value, 140).toUpperCase().replace(/\s+/g, "");
  const minimum = kind === "official" ? 12 : 4;
  return new RegExp(`^[A-Z0-9_.-]{${minimum},100}$`).test(normalized) ? normalized : null;
}

function resolveOfferCodeCandidate(field, value, evidence, commodity) {
  const officialInput = OFFICIAL_OFFER_CODE_FIELDS.has(field);
  const productInput = SUPPLIER_PRODUCT_CODE_FIELDS.has(field);
  if (!officialInput && !productInput) return { accepted: true, field, value, remapped: false };

  const quote = compact(evidence, 900);
  const productLabel = SUPPLIER_PRODUCT_CODE_LABEL_PATTERN.test(quote);
  const officialLabel = OFFICIAL_OFFER_CODE_LABEL_PATTERN.test(quote);
  if (productLabel && officialLabel) {
    return { accepted: false, field, value, reason: "ambiguous_offer_code_labels" };
  }

  let kind = officialInput ? "official" : "product";
  if (productLabel) kind = "product";
  else if (officialLabel) kind = "official";
  else {
    return {
      accepted: false,
      field,
      value,
      reason: officialInput ? "official_offer_code_label_missing" : "supplier_product_code_label_missing",
    };
  }

  const normalized = normalizeOfferCode(value, kind);
  if (!normalized) {
    return {
      accepted: false,
      field,
      value,
      reason: kind === "official" ? "invalid_official_offer_code" : "invalid_supplier_product_code",
    };
  }
  const targetField = offerCodeField(kind, field, quote, commodity);
  return {
    accepted: true,
    field: targetField,
    value: normalized,
    remapped: targetField !== field,
    reason: targetField !== field
      ? (kind === "official" ? "official_offer_code_label_reclassification" : "supplier_product_code_label_reclassification")
      : null,
  };
}
...
export function mergeOcrResult(base = {}, ocrNormalized = {}, ocrMeta = {}) {
  ...
  for (const [rawField, rawValue] of Object.entries(ocrNormalized || {})) {
    const sourceField = String(rawField || "");
    if (!sourceField || CONTROL_FIELDS.has(sourceField) || missing(rawValue)) continue;
    let field = sourceField;
    let value = candidateValue(field, rawValue);
    if (missing(value)) continue;
    let evidence = ocrEvidence(ocrNormalized, ocrMeta, sourceField);
    const codeResolution = resolveOfferCodeCandidate(field, value, evidence.quote, normalized.commodity);
    if (!codeResolution.accepted) {
      rejectCandidate(diagnostics, sourceField, value, codeResolution.reason, evidence);
      if (missing(base[sourceField])) normalized[sourceField] = null;
      continue;
    }
    if (codeResolution.remapped) {
      diagnostics.remappedFields.push({ from: sourceField, to: codeResolution.field, value: codeResolution.value, reason: codeResolution.reason });
      field = codeResolution.field;
      value = codeResolution.value;
...
export function mergeAiResult(base = {}, aiResult = {}, sourceContext = {}) {
  ...
  for (const [rawField, rawValue] of Object.entries(fields)) {
    ...
    let value = candidateValue(field, rawValue);
    if (missing(value)) continue;
    let itemEvidence = evidence[sourceField] || { field: sourceField, page: null, quote: "", confidence: null };
    const codeResolution = resolveOfferCodeCandidate(field, value, itemEvidence.quote, normalized.commodity);
    if (!codeResolution.accepted) {
      rejectCandidate(diagnostics, sourceField, value, codeResolution.reason, itemEvidence);
      continue;
    }
    if (codeResolution.remapped) {
      diagnostics.remappedFields.push({ from: sourceField, to: codeResolution.field, value: codeResolution.value, reason: codeResolution.reason });
      field = codeResolution.field;
      value = codeResolution.value;
      itemEvidence = { ...itemEvidence, field };
    }
...
export function applyCrossSourceConsensus({
  ...
  for (const field of allFields) {
    ...
    let ocrValue = candidateValue(field, ocrNormalized?.[field]);
    let aiValue = candidateValue(field, aiFields?.[field]);
    if (missing(ocrValue) || missing(aiValue)) continue;
    const evidence = consensusEvidence(field, ocrNormalized, aiResult, sourceContext);
    const ocrCode = resolveOfferCodeCandidate(field, ocrValue, evidence, result.commodity);
    const aiCode = resolveOfferCodeCandidate(field, aiValue, evidence, result.commodity);
    if (!ocrCode.accepted || !aiCode.accepted) {
      diagnostics.rejected.push({
        field,
        ocr_value: ocrValue,
        ai_value: aiValue,
        reason: !ocrCode.accepted ? ocrCode.reason : aiCode.reason,
      });
      continue;
    }
    if (ocrCode.field !== aiCode.field) {
      diagnostics.rejected.push({ field, ocr_value: ocrValue, ai_value: aiValue, reason: "offer_code_semantic_target_disagreement" });
      continue;
    }
    targetField = ocrCode.field;
    ocrValue = ocrCode.value;
    aiValue = aiCode.value;

    if (!equivalent(targetField, ocrValue, aiValue)) {
      diagnostics.rejected.push({ field: targetField, source_field: field, ocr_value: ocrValue, ai_value: aiValue, reason: "ocr_ai_disagreement" });
      continue;
    }
    if (/^nome_offerta(?:_|$)/.test(targetField) && !safeOfferName(aiValue)) {
      diagnostics.rejected.push({ field: targetField, source_field: field, value: aiValue, reason: "invalid_offer_name_semantics" });
      continue;
    }
    if (ECONOMIC_FIELDS.has(targetField)) {
      const safety = safeEconomicEvidence(targetField, evidence, { allowConsensus: true });
      if (!safety.safe) {
        diagnostics.rejected.push({ field: targetField, source_field: field, value: aiValue, reason: safety.reason });
        continue;
      }
    }

    diagnostics.agreements.push(targetField);
    const before = result[targetField];
    const nativeValue = nativeNormalized?.[targetField];
    if (!missing(nativeValue) && !equivalent(targetField, nativeValue, aiValue)) {
      diagnostics.rejected.push({ field: targetField, source_field: field, native_value: nativeValue, agreed_value: aiValue, reason: "consensus_cannot_override_native" });
      continue;
    }
    if (missing(before) || !equivalent(targetField, before, aiValue)) {
      result[targetField] = aiValue;
      diagnostics.corrections.push({
        field: targetField,
        source_field: targetField === field ? null : field,
        previous_value: missing(before) ? null : before,
        value: aiValue,
        reason: targetField === field ? "ocr_ai_agreement" : "ocr_ai_agreement_with_offer_code_reclassification",
      });
    }
    appendSource(result, targetField, "ocr");
    appendSource(result, targetField, "ai");
  }
