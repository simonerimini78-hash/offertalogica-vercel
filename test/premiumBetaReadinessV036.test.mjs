...
test("v0.36 presenta il servizio come beta riservata e aggiorna la cache PWA", () => {
  assert.match(app, /BETA RISERVATA/);
  assert.doesNotMatch(app, /IN SVILUPPO/);
  assert.match(app, /APP Premium v0\.36/);
  assert.match(sw, /offertalogica-premium-v036/);
  assert.match(bills, /app_version: "0\.36"/);
});
