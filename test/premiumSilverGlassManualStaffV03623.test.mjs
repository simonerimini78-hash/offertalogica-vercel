...
test("v0.36.28 usa un fondo argento neutro e mantiene verdi solo gli elementi di marca", async () => {
  const app = await read("public/app.html");
  assert.match(app, /v0\.36\.28 — fondo argento/);
...
test("v0.36.28 separa l'aggiornamento automatico app da quello manuale staff", async () => {
  ...
  assert.equal(JSON.parse(manifest).version, "0.36.28");
});
