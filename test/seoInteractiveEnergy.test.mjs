...
test("SEO interattiva: PUN PSV distingue dato mensile e pagine giornaliere",()=>{const html=read(pages[0]);assert.match(html,/ultimo (?:valore|dato) mensile ufficiale/i);assert.match(html,/href="\/pun-oggi\.html"/);assert.match(html,/href="\/psv-gas-oggi\.html"/);assert.match(html,/data-ol-market-monthly/);const js=read(path.join("assets","seo-energy-interactions.js"));assert.match(js,/\/data\/calcolo-parametri\.json/);assert.match(html,/from=seo-indici#pdf-upload-panel/);});
