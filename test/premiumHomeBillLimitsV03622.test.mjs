...
test("Liquid Glass e testi piano sono più leggibili senza cambiare layout", async () => {
  const app=await read("public/app.html");
  assert.match(app,/v0\.36\.22 — Liquid Glass marcato/);
  assert.match(app,/rgba\(4,91,42,\.94\)/);
  assert.match(app,/rgba\(132,209,38,\.69\)/);
...
test("app automatica e staff manuale usano il manifest anti-cache", async () => {
  ...
  assert.equal(JSON.parse(manifest).version,"0.36.22");
...
test("v0.36.22 resta entro 12 funzioni Vercel", async () => {
