-- OffertaLogica Premium - schema condiviso sito/app, versione 0.1
...
-- Crea i profili mancanti per eventuali utenti Auth gia presenti.
insert into public.premium_profiles (id, full_name, phone)
select
  users.id,
...
using (user_id = (select auth.uid()) and verification_status in ('unverified', 'needs_review'))
with check (
  user_id = (select auth.uid())
...
using (user_id = (select auth.uid()) and processing_status = 'uploaded');
...
using (user_id = (select auth.uid()) and direction in ('staff_to_user', 'system_to_user'))
with check (user_id = (select auth.uid()));
