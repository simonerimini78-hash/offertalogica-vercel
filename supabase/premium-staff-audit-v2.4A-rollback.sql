-- OffertaLogica Staff v2.4A - rollback.
-- Ripristina premium_owner_add_staff alla V2.3B2.1,
-- premium_owner_update_staff alla V2.3B1 e rimuove gli oggetti Audit V2.4A.
...
begin
  ...
  select staff.role into v_existing_role
...
drop function if exists public.premium_owner_list_audit(integer, integer);
drop function if exists public.premium_staff_audit_insert(
  uuid, text, text, text, text, text, text, jsonb, text
);
drop table if exists public.premium_staff_audit_events;
