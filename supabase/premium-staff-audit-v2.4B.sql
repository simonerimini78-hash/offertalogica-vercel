-- OffertaLogica Staff v2.4B - Audit operazioni critiche
-- Base: Staff v2.4A.
-- Ambito: cancellazioni definitive Premium + supporto al log Stripe server-side.
-- Non modifica frontend, omaggi, pratiche operative, lead o analytics.
...
declare
  ...
  v_raw_role text := coalesce(public.premium_staff_raw_role(), '');
...
begin
  ...
  -- Audit nella stessa transazione della cancellazione:
  -- se la scrittura Audit fallisse, anche la cancellazione viene annullata.
  perform public.premium_staff_audit_insert(
    v_admin_id,
    v_raw_role,
    'premium_records_deleted',
    v_resource,
    case when cardinality(v_ids) = 1 then v_ids[1]::text else null end,
    'success',
    '',
    jsonb_build_object(
      'requested_count', cardinality(v_ids),
      'deleted_count', v_target_count,
      'requested_ids', to_jsonb(v_ids)
    ),
    'rpc:premium_staff_delete_records'
  );
...
comment on function public.premium_staff_delete_records(text, uuid[]) is
  'Staff v2.4B: eliminazione amministrativa atomica con Audit nella stessa transazione. I file Storage vengono rimossi separatamente dal client admin.';
