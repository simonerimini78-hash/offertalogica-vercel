-- OffertaLogica Staff v0.36.66
-- Baseline economica Owner-only: consente di ripartire da zero senza cancellare lo storico.
-- Il dashboard continua a includere tutte le fonti economiche, ma non considera eventi anteriori alla baseline attiva.
...
create or replace function public.premium_owner_reset_economic_baseline()
returns jsonb
language plpgsql
security definer
set search_path = ''
...
revoke all on function public.premium_owner_reset_economic_baseline() from public, anon;
grant execute on function public.premium_owner_reset_economic_baseline() to authenticated, service_role;

comment on table public.premium_economic_baselines is
  'Storico delle baseline del Cruscotto economico. Ogni azzeramento conserva i dati precedenti e sposta solo l’inizio dei conteggi.';
comment on function public.premium_owner_reset_economic_baseline() is
  'Owner-only: crea una nuova baseline economica senza cancellare o modificare eventi storici.';
...
begin
  ...
  select max(b.baseline_at)
    into v_baseline_at
  from public.premium_economic_baselines b;

  v_since := case
    when v_baseline_at is null then v_requested_since
    else greatest(v_requested_since, v_baseline_at)
  end;
  ...
  select jsonb_build_object(
    'generated_at', now(),
    'days', v_days,
    'requested_from', v_requested_since,
    'baseline_at', v_baseline_at,
    'baseline_active', (v_baseline_at is not null and v_baseline_at > v_requested_since),
    'from', v_since,
...
comment on function public.premium_owner_economic_dashboard(integer) is
  'Cruscotto economico Owner-only; applica l’ultima baseline ai conteggi senza cancellare lo storico e separa i costi IA per origine.';
