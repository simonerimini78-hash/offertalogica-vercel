(() => {
  ...
  let mounted = false;
  let state = { settings:null, schedule:[], targets:[], snapshots:[], opportunities:[], plans:[] };
  ...
  function buildShell(){
    ...
    details.innerHTML=`
      ...
      <div class="ol-collapsible-content ol-panel-body">
        <div class="ol-autopilot-status"><div><strong>Fase 1 · struttura e controllo manuale</strong><p>Le impostazioni vengono salvate, ma nessun articolo o post viene ancora generato o pubblicato automaticamente.</p></div><span class="ol-autopilot-badge">Motore non attivo</span></div>
  ...
  function render(){
    ...
    box.innerHTML=`<form data-autopilot-form novalidate>
      <div class="ol-autopilot-grid">
        <section class="ol-card ol-autopilot-card"><h3>Regole editoriali</h3><p>Valori modificabili. Il limite iniziale resta un articolo alla settimana.</p>
          <div class="ol-autopilot-fields">
            ${field("Modalità futura",`<select name="execution_mode"><option value="draft"${s.execution_mode==="draft"?" selected":""}>Solo bozza</option><option value="approval"${s.execution_mode==="approval"?" selected":""}>Genera e chiedi approvazione</option><option value="automatic"${s.execution_mode==="automatic"?" selected":""}>Automatico completo</option></select>`,`Non attiva nulla in questa fase.`)}
      ...
      <div class="ol-autopilot-toolbar"><p class="ol-autopilot-save-state" data-autopilot-save-state>Configurazione caricata. Il motore resta disattivato.</p><button class="ol-button ol-button-primary" type="submit">Salva configurazione Autopilota</button></div>
    </form>`;
    box.querySelector("[data-autopilot-form]")?.addEventListener("submit",saveAll);
  }

  async function loadAll(){
    const [settings,schedule,targets,snapshots,opportunities,plans]=await Promise.all([
    ...
    state={settings:settings?.[0]||{},schedule:schedule||[],targets:targets||[],snapshots:snapshots||[],opportunities:opportunities||[],plans:plans||[]};render();
  }
  ...
  async function saveAll(event){
    ...
    try{
      ...
      if(stateText)stateText.textContent="Configurazione salvata. Nessun automatismo è stato avviato.";showMessage("","Configurazione Autopilota salvata.");await loadAll();
