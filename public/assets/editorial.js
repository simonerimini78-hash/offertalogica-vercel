(() => {
  ...
  function renderSessionAuthor(kind,author,fallbackName="Autore"){
    const name=String(author?.display_name||fallbackName||"Autore").trim()||"Autore";
    const nameEl=document.querySelector(`[data-${kind}-name]`);if(nameEl)nameEl.textContent=name;
    const img=document.querySelector(`[data-${kind}-avatar]`);const fallback=document.querySelector(`[data-${kind}-avatar-fallback]`);const safe=/^https:\/\//i.test(String(author?.avatar_url||""))?String(author.avatar_url):"";
    if(img){if(safe){img.src=safe;img.hidden=false;}else{img.removeAttribute("src");img.hidden=true;}}
    if(fallback){fallback.textContent=(name.match(/[A-Za-zÀ-ÖØ-öø-ÿ0-9]/)?.[0]||"A").toUpperCase();fallback.hidden=Boolean(safe);}
  }
  ...
  async function staffEndpoint(action,{token="",body=null}={}){const response=await fetch(`/api/staff-analytics?action=${encodeURIComponent(action)}`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify(body||{}),cache:"no-store"});const payload=await response.json().catch(()=>null);if(!response.ok||payload?.ok===false){const error=new Error(payload?.error||`Errore ${response.status}`);error.status=response.status;throw error;}return payload;}
  ...
  async function initWorkspace(){
    ...
    const ownProfile=setupOwnAuthorProfile(document.querySelector("[data-own-author-profile]"),{getSession:()=>session,getAuthor:()=>context?.author,onSaved:author=>{if(context)context.author=author;renderSessionAuthor("member",author,"Collaboratore");}});
  ...
  async function initReview(){
    ...
    const ownProfile=setupOwnAuthorProfile(document.querySelector("[data-own-author-profile]"),{getSession:()=>session,getAuthor:()=>ownAuthor,onSaved:author=>{ownAuthor=author;renderSessionAuthor("review",author,"Redazione OffertaLogica Informa");}});
    ...
    function setActionState(){
      ...
      const deleteButton=form.querySelector("[data-review-delete]");if(deleteButton){const allowed=member?.role==="admin"&&Boolean(fields.id.value)&&status==="draft";deleteButton.hidden=!allowed;deleteButton.disabled=!allowed;deleteButton.title=allowed?"Elimina definitivamente questa bozza e i contenuti Autopilota collegati":"È possibile eliminare definitivamente solo una bozza";}
    ...
    async function deleteDraftArticle(){
      clearMessages();
      if(member?.role!=="admin"){show(validation,"Solo un amministratore può eliminare definitivamente una bozza.");return;}
      const articleId=safeId(fields.id.value);if(!articleId||fields.status.value!=="draft"){show(validation,"È possibile eliminare definitivamente solo una bozza già salvata.");return;}
      const title=text(fields.title.value,140)||"questa bozza";
      const confirmed=window.confirm(`Eliminare definitivamente “${title}”? Verranno rimossi anche i post statici ancora in bozza e le immagini generate dall’Autopilota collegate. L’opportunità editoriale resterà disponibile ma verrà scollegata dall’articolo.`);
      if(!confirmed)return;
      const button=form.querySelector("[data-review-delete]");button.disabled=true;show(saveMessage,"Eliminazione della bozza e dei contenuti collegati…");
      try{
        const payload=await staffEndpoint("delete-editorial-draft-article",{token:session.access_token,body:{article_id:articleId}});
        dirty=false;currentArticle=null;fields.id.value="";
        if(payload?.result?.storage_warning)window.alert("Bozza eliminata. Una o più immagini generate nello storage non sono state rimosse automaticamente; il resto della cancellazione è completato.");
        location.replace(queueUrl());
      }catch(error){show(saveMessage,"");show(validation,`Eliminazione non riuscita: ${error.message}`);button.disabled=false;}
    }
    async function transition(target){
      ...
      try{await db("rpc/editorial_staff_transition_article",{method:"POST",token:session.access_token,body:{p_article_id:article.id,p_status:target,p_feedback:note,p_visibility:visibility}});fields.review_note.value="";dirty=false;if(["in_review","changes_requested","approved","published","draft","archived"].includes(target))filter.value=target;history.replaceState(null,"",queueUrl(article.id));const messages={in_review:"Articolo inviato in revisione.",changes_requested:"Modifiche richieste all’autore.",approved:"Articolo approvato.",published:"Articolo pubblicato. La versione HTML SEO sarà rigenerata al prossimo deploy.",archived:"Articolo archiviato."};show(saveMessage,messages[target]||"Stato aggiornato.");await loadArticle(article.id);await loadQueue();}
