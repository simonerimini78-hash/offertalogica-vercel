...
export const PDF_AI_RASTER_BATCH_VERSION = "step8-clean-targeted-raster-v4-strict-page-routing";
...
function pageEvidenceForProfile(results, profile) {
  ...
  for (const result of results || []) {
    ...
    const batchMatchesTarget = batchCommodity === targetCommodity;
    const batchMatchesOpposite = batchCommodity === oppositeCommodity;
    ...
    for (const item of result?.page_map || []) {
      ...
      const explicitTarget = targetPattern.test(context);
      const explicitOpposite = oppositePattern.test(context);
      const identity = IDENTITY_CONTEXT.test(context);
      const economic = ECONOMIC_CONTEXT.test(context);
      const identityOnly = identity && !economic && !explicitTarget && !explicitOpposite;

      if (explicitTarget) entry.target += 10;
      if (explicitOpposite) entry.opposite += 10;
      ...
      if (!explicitOpposite && batchMatchesTarget && !identityOnly) entry.target += 5;
      if (!explicitTarget && batchMatchesOpposite) entry.opposite += 5;
    }
  }
  return evidence;
}
...
export function selectCriticalPageDecision({
  ...
  const rankedTargets = [...evidence.values()]
    .filter((item) => available.has(item.page) && item.target > 0 && item.target > item.opposite)
    .sort((left, right) => {
      const leftScore = left.target + left.economic;
      const rightScore = right.target + right.economic;
  ...
  const selected = rankedTargets.slice(0, Math.max(1, maxPages));
